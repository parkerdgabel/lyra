...
impl SecureStdlibWrapper {
    ...
    fn estimate_value_memory(&self, value: &Value) -> i64 {
        match value {
            ...
            Value::Object(map) => 24 + map.iter().map(|(k, v)| k.len() as i64 + self.estimate_value_memory(v)).sum::<i64>(),
        }
    }
}
...
/// Register security wrapper functions
pub fn register_security_wrapper_functions() -> std::collections::HashMap<String, fn(&[Value]) -> VmResult<Value>> {
    let functions = std::collections::HashMap::new();
    
    // For now, this module provides infrastructure but doesn't register functions directly
    // The SecureStdlibWrapper is used to wrap existing stdlib functions
