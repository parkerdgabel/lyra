        ...
        match value {
            ...
            Value::Object(map) => {
                ...
                for (i, (key, value)) in map.iter().enumerate() {
                    result.push_str(&format!("{}{}: {}", "  ".repeat(indent + 1), key, Self::format_structure(value, indent + 1)));
                    if i < map.len() - 1 {
                        result.push(',');
                    }
                    result.push('\n');
...
fn estimate_value_size(value: &Value) -> usize {
    match value {
        ...
        Value::Object(map) => 24 + map.iter().map(|(k, v)| k.len() + estimate_value_size(v)).sum::<usize>(),
    }
}
