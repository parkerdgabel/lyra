...
use tower_lsp::lsp_types::{
    self as lsp, CodeAction, CodeActionKind, CodeActionOrCommand, CodeActionParams,
    CodeActionResponse, Command, CompletionItem, CompletionItemKind, CompletionParams,
    CompletionResponse, CreateFile, CreateFileOptions, Diagnostic, DiagnosticSeverity,
    DocumentChangeOperation, DocumentChanges, DocumentSymbolParams, DocumentSymbolResponse,
    ExecuteCommandOptions, ExecuteCommandParams, GotoDefinitionParams, GotoDefinitionResponse,
    Hover, HoverContents, HoverParams, HoverProviderCapability, InitializeParams, InitializeResult,
    InitializedParams, Location, OneOf, OptionalVersionedTextDocumentIdentifier,
    ParameterInformation, Position, Range, ResourceOp, ServerCapabilities, SignatureHelp,
    SignatureHelpOptions, SignatureInformation, SymbolInformation, SymbolKind,
...
#[derive(Clone)]
struct DocIndex {
    map: Arc<Mutex<HashMap<String, (String, Vec<String>)>>>,
} // name -> (summary, params)
...
#[derive(Clone, Debug)]
struct ServerConfig {
    module_skeleton: String,
    organize_on_save: bool,
    organize_on_save_globs: Vec<String>,
}

impl Default for ServerConfig {
    fn default() -> Self {
        ServerConfig {
            module_skeleton: String::from("Exported[{}];\n\n"),
            organize_on_save: false,
            organize_on_save_globs: Vec::new(),
        }
    }
}
...
impl LanguageServer for Backend {
        ...
        Ok(InitializeResult {
            capabilities: ServerCapabilities {
                ...
                execute_command_provider: Some(ExecuteCommandOptions {
                    commands: vec![
                        "lyra.createModule".into(),
                        "lyra.organizeImports".into(),
                        "lyra.reloadProject".into(),
                    ],
                    work_done_progress_options: Default::default(),
                }),
                ..Default::default()
    ...
    async fn did_change(&self, params: lsp::DidChangeTextDocumentParams) {
        if let Some(text) = apply_changes(
            self.docs.lock().get(&params.text_document.uri).cloned(),
        ...
        let should_org = {
            ...
            if let Ok(path) = uri.to_file_path() {
                if let Some(root) = discover_project_root(&path) {
                    ...
                    if let Some(cfg) = self.project_configs.lock().get(&key) {
                        flag = cfg.organize_on_save;
                        globs = cfg.organize_on_save_globs.clone();
                    }
                }
                // If globs present, require a match
                if !globs.is_empty() {
                    let pstr = path.to_string_lossy().to_string();
                    if !matches_any_glob(&globs, &pstr) {
                        flag = false;
                    }
                }
            }
    ...
    async fn hover(&self, params: HoverParams) -> LspResult<Option<Hover>> {
        ...
        let text = match self.docs.lock().get(&uri) {
            Some(t) => t.clone(),
            None => return Ok(None),
        };
        ...
        if let Some((summary, params)) = self.doc_index.map.lock().get(&word).cloned() {
            ...
            if !params.is_empty() {
                parts.push(format!("{}[{}]", word, params.join(", ")));
            }
            if !summary.is_empty() {
                parts.push(summary);
            }
            ...
            return Ok(Some(Hover {
                contents: HoverContents::Markup(lsp::MarkupContent {
        ...
        for name in self.builtins.iter() {
            if score(name, &prefix).is_some() {
                ...
                if let Some((summary, params)) = self.doc_index.map.lock().get(name).cloned() {
                    it.detail = Some(if params.is_empty() {
                        summary.clone()
                    } else {
                        format!("{}[{}] — {}", name, params.join(", "), summary)
                    });
                    it.documentation =
            ...
            if let Some(mods) = self.projects.lock().get(&root.to_string_lossy().to_string()) {
                for (mname, _path) in mods.iter() {
                    if score(mname, &prefix).is_some() {
                        let mut it =
                            CompletionItem::new_simple(mname.clone(), String::from("(module)"));
        ...
        if let Some(map) = self.sym_index.lock().get(&uri) {
            if let Some(defpos) = map.get(&word) {
                let loc = Location {
                    ...
                    range: Range {
                        start: *defpos,
                        end: Position::new(defpos.line, defpos.character + word.len() as u32),
                    },
                };
        ...
        if let Some((head, arg_idx)) = current_call_context_at(&text, pos) {
            if let Some((summary, params)) = self.doc_index.map.lock().get(&head).cloned() {
                ...
                let help = SignatureHelp {
                    ...
                    active_parameter: Some(arg_idx as u32),
                };
                return Ok(Some(help));
            }
        }
    ...
    async fn code_action(&self, params: CodeActionParams) -> LspResult<Option<CodeActionResponse>> {
        ...
        if let Some(root) = uri.to_file_path().ok().and_then(|p| discover_project_root(&p)) {
                ...
                if let Some(mods) = self.projects.lock().get(&key) {
                    if mods.contains_key(&name) {
                        ...
                        if !text.contains(&format!("Using[\"{}\"", name)) {
                            ...
                            actions.push(CodeActionOrCommand::CodeAction(CodeAction {
                                title: format!("Insert Using[\"{}\"]", name),
                                kind: Some(CodeActionKind::QUICKFIX),
                                diagnostics: None,
                                edit: Some(edit),
                                command: None,
                        ...
                        if let Some(path) = mods.get(&name) {
                            if !std::path::Path::new(path).exists() {
                                let cmd = Command::new(
                                    format!("Create module {}", name),
                                    "lyra.createModule".into(),
                                    Some(vec![serde_json::json!({"path": path, "name": name})]),
                                );
            ...
            let ca = CodeAction {
                ...
                diagnostics: None,
                edit: edits_opt.map(|edits| lsp::WorkspaceEdit {
                    changes: Some(std::iter::once((uri.clone(), edits)).collect()),
                    document_changes: None,
                    ..Default::default()
                }),
                command: Some(Command::new(
                    String::from("Organize Imports"),
                    "lyra.organizeImports".into(),
        ...
        match params.command.as_str() {
            "lyra.createModule" => {
                ...
                if let Some(first) = args.into_iter().next() {
                    if let (Some(path), name_opt) = (
                        first.get("path").and_then(|v| v.as_str()).map(|s| s.to_string()),
                        first.get("name").and_then(|v| v.as_str()).map(|s| s.to_string()),
                    ) {
                        ...
                        let skel = {
                            ...
                            if let Some(root) = discover_project_root(p) {
                                let key = root.to_string_lossy().to_string();
                                if let Some(cfg) = self.project_configs.lock().get(&key) {
                                    applied = Some(cfg.module_skeleton.clone());
                                }
                            }
                            applied.unwrap_or_else(|| {
                                self.default_config.lock().module_skeleton.clone()
                            })
                        };
                        let nm = name_opt.unwrap_or_else(|| {
                        ...
                        let uri = match StdUrl::from_file_path(&p) {
                            Ok(u) => Url::from(u),
                            Err(_) => {
                                return Ok(Some(
            ...
            "lyra.reloadProject" => {
                ...
                let key_opt =
                    params.arguments.into_iter().next().and_then(|v| {
                        v.get("path").and_then(|s| s.as_str()).map(|s| s.to_string())
                    });
                if let Some(path) = key_opt {
                    let pb = std::path::Path::new(&path).to_path_buf();
                    if let Some(root) = discover_project_root(&pb) {
                        let fake_uri = match StdUrl::from_file_path(root.join("project.lyra")) {
                            Ok(u) => Url::from(u),
                            Err(_) => {
                                return Ok(Some(
                                    serde_json::json!({"ok": false, "error": "invalid path"}),
            ...
            "lyra.organizeImports" => {
                // args: { uri: string }
                let uri = params.arguments.into_iter().next().and_then(|v| {
                    v.get("uri").and_then(|s| s.as_str()).and_then(|s| Url::parse(s).ok())
                });
                if let Some(u) = uri {
                    ...
                    if let Some(text) = text_opt {
                        if let Some(edits) = build_organize_imports_edits(&text, &u) {
                            let we = lsp::WorkspaceEdit {
                                changes: Some(std::iter::once((u.clone(), edits)).collect()),
                                document_changes: None,
                                ..Default::default()
                            };
                            let _ = self.client.apply_edit(we).await;
...
impl Backend {
    ...
    async fn ensure_project_for_uri(&self, uri: &Url) {
        ...
        let key = root.to_string_lossy().to_string();
        if self.projects.lock().contains_key(&key) {
            return;
        }
        // Load via stdlib ProjectInfo["<root>"]
        let mut ev = self.evaluator.lock();
        let q = Value::Expr {
            head: Box::new(Value::Symbol("ProjectInfo".into())),
        ...
        let cfg = {
            ...
            let v = ev.eval(Value::Expr {
                head: Box::new(Value::Symbol("ProjectLoad".into())),
                args: vec![Value::String(key.clone())],
            });
                ...
                if let Some(Value::Assoc(t)) = tool {
                    if let Some(Value::Assoc(ls)) = t.get("lyra.lsp").or_else(|| t.get("Lyra.LSP"))
                    {
                        ...
                        if let Some(Value::String(s)) | Some(Value::Symbol(s)) =
                            ls.get("moduleSkeleton").or_else(|| ls.get("ModuleSkeleton"))
                        {
                            cfg.module_skeleton = s.clone();
                        }
                        if let Some(Value::Boolean(b)) =
                            ls.get("organizeOnSave").or_else(|| ls.get("OrganizeOnSave"))
                        {
                            cfg.organize_on_save = *b;
                        }
                        if let Some(Value::List(vs)) =
                            ls.get("organizeOnSaveGlobs").or_else(|| ls.get("OrganizeOnSaveGlobs"))
                        {
        ...
        if let Value::Assoc(m) = val {
            if let Some(Value::Assoc(mods)) = m.get("modules") {
                for (k, v) in mods.iter() {
                    if let Value::String(p) | Value::Symbol(p) = v {
                        modules.insert(k.clone(), p.clone());
                    }
                }
            }
        }
                ...
                for (sym, pos) in defs {
                    if let Ok(u) = StdUrl::from_file_path(path) {
                        let loc = Location {
                            ...
                            range: Range {
                                start: pos,
                                end: Position::new(pos.line, pos.character + sym.len() as u32),
                            },
                        };
        ...
        if let Some(c) = cfg {
            self.project_configs.lock().insert(key.clone(), c);
        }
    }
...
fn score(candidate: &str, pat: &str) -> Option<i64> {
    if candidate.starts_with(pat) {
        return Some(1000 - (candidate.len() as i64 - pat.len() as i64).max(0));
    }
...
fn current_call_context_at(text: &str, pos: Position) -> Option<(String, usize)> {
    ...
    while i >= 0 {
        ...
        if c == '[' {
            if bracket_depth == 0 {
                target_lbracket = Some(i as usize);
                break;
            } else {
                bracket_depth -= 1;
            }
...
fn build_organize_imports_edits(text: &str, _uri: &Url) -> Option<Vec<TextEdit>> {
    ...
    while first_code_line < lines.len() && lines[first_code_line].trim().is_empty() {
        first_code_line += 1;
    }
    ...
    let new_block = uniq
        .into_iter()
        .map(|m| format!("Using[\"{}\", <|Import->All|>];", m))
        .collect::<Vec<_>>()
        .join("\n");
    // Prepare edits:
    ...
    let insert_edit = TextEdit {
        range: Range {
            start: Position::new(first_code_line as u32, 0),
            end: Position::new(first_code_line as u32, 0),
        },
        new_text: format!("{}\n", new_block),
    };
...
fn matches_any_glob(patterns: &Vec<String>, path: &str) -> bool {
    for pat in patterns {
        ...
        regex.push('$');
        if Regex::new(&regex).map(|re| re.is_match(path)).unwrap_or(false) {
            return true;
        }
    }
    false
...
fn build_docs_and_builtins(ev: &mut Evaluator) -> (Arc<Vec<String>>, DocIndex) {
    ...
    for n in names.iter() {
        // Prefer generic Documentation[], then fallback to ToolsDescribe
        let q_doc = Value::Expr {
            head: Box::new(Value::Symbol("Documentation".into())),
            args: vec![Value::String(n.clone())],
        };
        let (mut summary, mut params): (String, Vec<String>) = match ev.eval(q_doc) {
            Value::Assoc(m) => {
                let s = m
                    .get("summary")
                    .and_then(|v| match v {
        ...
        if summary.is_empty() {
            let q = Value::Expr {
                head: Box::new(Value::Symbol("ToolsDescribe".into())),
                args: vec![Value::String(n.clone())],
            };
            if let Value::Assoc(m) = ev.eval(q) {
                summary = m
                    .get("summary")
                    .and_then(|v| match v {
                        Value::String(s) => Some(s.clone()),
