...
fn with_policy(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("WithPolicy".into())), args };
    }
    ...
    if let Value::Assoc(m) = &args[0] {
        if let Some(v) = m.get("Capabilities") {
            match v {
                Value::List(items) => {
                    caps = Some(items.clone());
                }
                Value::String(_) | Value::Symbol(_) => {
                    caps = Some(vec![v.clone()]);
                }
                _ => {}
            }
        }
    ...
    let old_caps = ev.eval(Value::Expr {
        head: Box::new(Value::Symbol("ToolsGetCapabilities".into())),
        args: vec![],
    });
    if let Some(list) = caps {
        let _ = ev.eval(Value::Expr {
            head: Box::new(Value::Symbol("ToolsSetCapabilities".into())),
