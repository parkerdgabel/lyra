                ...
                match audio_buf {
                    AudioBufferRef::F32(buf) => {
                        if sample_buf.is_none() {
                            sample_buf =
                                Some(SampleBuffer::<f32>::new(buf.capacity() as u64, *buf.spec()))
                        }
...
#[cfg(any(feature = "audio_mp3", feature = "audio_ogg_vorbis", feature = "audio_flac"))]
fn ffmpeg_encode_to_bytes(
    mut dec: DecodedAudio,
    fmt: &str,
    ...
    let out_ext = match fmt {
        ...
        "ogg" => "ogg",
        "flac" => "flac",
        _ => return Err("Unsupported ffmpeg fmt".into()),
    };
    ...
    if let Some(m) = opts {
        match fmt {
            "mp3" => {
                ...
                if let Some(Value::Integer(b)) = m.get("bitrateKbps").or(m.get("BitrateKbps")) {
                    cmd.arg("-b:a").arg(format!("{}k", b));
                }
                if let Some(Value::Boolean(vbr)) = m.get("vbr").or(m.get("Vbr")) {
                    if *vbr {
                        if let Some(Value::Integer(q)) = m.get("quality").or(m.get("Quality")) {
            ...
            "ogg" => {
                ...
                if let Some(Value::Real(q)) = m.get("quality").or(m.get("Quality")) {
                    cmd.arg("-q:a").arg(format!("{}", q));
                }
                if let Some(Value::Integer(qi)) = m.get("quality").or(m.get("Quality")) {
                    cmd.arg("-q:a").arg(qi.to_string());
                }
            }
            "flac" => {
                cmd.arg("-c:a").arg("flac");
                if let Some(Value::Integer(l)) =
                    m.get("compressionLevel").or(m.get("CompressionLevel"))
                {
                    cmd.arg("-compression_level").arg(l.to_string());
                }
            }
            _ => {}
        }
    ...
    if !status.success() {
        return Err("ffmpeg failed to encode".into());
    }
...
fn audio_info(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 1 {
        return Value::Expr { head: Box::new(Value::Symbol("AudioInfo".into())), args };
    }
    let bytes = match read_input_bytes(ev, args[0].clone()) {
        Ok(b) => b,
        Err(e) => return failure("Audio::input", &format!("AudioInfo: {}", e)),
    };
...
fn audio_decode(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 1 {
        return Value::Expr { head: Box::new(Value::Symbol("AudioDecode".into())), args };
    }
    let bytes = match read_input_bytes(ev, args[0].clone()) {
        Ok(b) => b,
        Err(e) => return failure("Audio::input", &format!("AudioDecode: {}", e)),
    };
    let dec = match decode_with_symphonia(&bytes) {
        Ok(d) => d,
        Err(e) => return failure("Audio::decode", &format!("AudioDecode: {}", e)),
    };
    ...
    if fmt == "raw" {
        ...
        for s in dec.samples.iter() {
            let v = (s.clamp(-1.0, 1.0) * i16::MAX as f32) as i16;
            pcm_s16.extend_from_slice(&v.to_le_bytes());
        }
        let mut m = std::collections::HashMap::new();
        m.insert("sampleRate".into(), Value::Integer(dec.sample_rate as i64));
        m.insert("channels".into(), Value::Integer(dec.channels as i64));
...
fn audio_encode(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 1 {
        return Value::Expr { head: Box::new(Value::Symbol("AudioEncode".into())), args };
    }
    let raw = match ev.eval(args[0].clone()) {
        Value::Assoc(m) => m,
        _ => return failure("Audio::encode", "Expected raw assoc"),
    };
    let sr = raw
        .get("sampleRate")
    ...
    let pcm_b64 = raw
        .get("pcm")
        .and_then(|v| if let Value::String(s) = v { Some(s.clone()) } else { None })
        .ok_or_else(|| "Missing pcm".to_string());
    let pcm_bytes = match pcm_b64 {
        Ok(s) => base64url_decode(&s),
        Err(e) => return failure("Audio::encode", &e),
    };
    let (samples_f32, ok) = match (pcm_bytes, enc.as_str()) {
        (Ok(bytes), "s16le") => {
            let mut out = Vec::with_capacity(bytes.len() / 2);
            for chunk in bytes.chunks_exact(2) {
                let v = i16::from_le_bytes([chunk[0], chunk[1]]) as f32 / i16::MAX as f32;
                out.push(v);
            }
            (out, true)
        }
...
fn audio_convert(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("AudioConvert".into())), args };
    }
    let bytes = match read_input_bytes(ev, args[0].clone()) {
        Ok(b) => b,
        Err(e) => return failure("Audio::input", &format!("AudioConvert: {}", e)),
    };
    let dec = match decode_with_symphonia(&bytes) {
        Ok(d) => d,
        Err(e) => return failure("Audio::decode", &format!("AudioConvert: {}", e)),
    };
    ...
    match fmt.as_str() {
        ...
        "ogg" | "vorbis" => {
            ...
            #[cfg(not(feature = "audio_ogg_vorbis"))]
            {
                return failure(
                    "Audio::convert",
                    "Ogg/Vorbis encoding not enabled (feature audio_ogg_vorbis)",
                );
            }
        }
        "flac" => {
...
fn audio_trim(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("AudioTrim".into())), args };
    }
    let bytes = match read_input_bytes(ev, args[0].clone()) {
        Ok(b) => b,
        Err(e) => return failure("Audio::input", &format!("AudioTrim: {}", e)),
    };
    let dec = match decode_with_symphonia(&bytes) {
        Ok(d) => d,
        Err(e) => return failure("Audio::decode", &format!("AudioTrim: {}", e)),
    };
    ...
    let start = opts
        ...
        .unwrap_or(0.0);
    let dur_opt = opts.get("durationSec").or(opts.get("DurationSec")).and_then(|v| match v {
        Value::Real(f) => Some(*f),
        Value::Integer(i) => Some(*i as f64),
        _ => None,
    });
    ...
    let max_frames = if let Some(d) = dur_opt {
        (d * dec.sample_rate as f64).round() as usize
    } else {
        total_frames.saturating_sub(frame_start)
    };
    let frame_end = (frame_start + max_frames).min(total_frames);
    let ch = dec.channels as usize;
    let start_i = frame_start * ch;
    let end_i = frame_end * ch;
    let slice = if start_i < end_i && end_i <= dec.samples.len() {
        &dec.samples[start_i..end_i]
    } else {
        &[]
    };
...
fn audio_gain(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("AudioGain".into())), args };
    }
    let bytes = match read_input_bytes(ev, args[0].clone()) {
        Ok(b) => b,
        Err(e) => return failure("Audio::input", &format!("AudioGain: {}", e)),
    };
    let mut dec = match decode_with_symphonia(&bytes) {
        Ok(d) => d,
        Err(e) => return failure("Audio::decode", &format!("AudioGain: {}", e)),
    };
    let opts = match ev.eval(args[1].clone()) {
        Value::Assoc(m) => m,
        _ => std::collections::HashMap::new(),
    };
    let linear = if let Some(Value::Real(db)) = opts.get("db").or(opts.get("dB")) {
        10f32.powf((*db as f32) / 20.0)
    } else if let Some(Value::Integer(db)) = opts.get("db").or(opts.get("dB")) {
        10f32.powf((*db as f32) / 20.0)
    } else if let Some(Value::Real(lin)) = opts.get("linear") {
        *lin as f32
    } else if let Some(Value::Integer(lin)) = opts.get("linear") {
        *lin as f32
    } else {
        1.0
    };
    for s in dec.samples.iter_mut() {
    ...
    for of in 0..out_frames {
        ...
        for c in 0..ch {
            let s0 = if i0 < 0 {
                ...
                samples[i0 as usize * ch + c]
            } else {
                samples[(in_frames - 1) * ch + c]
            };
            let s1 = if i1 < 0 {
                samples[c]
            } else if (i1 as usize) < in_frames {
                samples[i1 as usize * ch + c]
            } else {
                samples[(in_frames - 1) * ch + c]
            };
...
fn mix_channels(samples: &[f32], in_ch: u16, out_ch: u16) -> Vec<f32> {
    ...
    for f in 0..frames {
        ...
        let mut sum = 0.0f32;
        for c in 0..in_ch_usize {
            sum += samples[f * in_ch_usize + c];
        }
        ...
        for c in 0..out_ch_usize {
            ...
            out[f * out_ch_usize + c] = if out_ch_usize >= in_ch_usize {
                samples[f * in_ch_usize + (c % in_ch_usize)]
            } else {
                avg
            };
        }
...
fn audio_resample(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("AudioResample".into())), args };
    }
    let bytes = match read_input_bytes(ev, args[0].clone()) {
        Ok(b) => b,
        Err(e) => return failure("Audio::input", &format!("AudioResample: {}", e)),
    };
    let dec = match decode_with_symphonia(&bytes) {
        Ok(d) => d,
        Err(e) => return failure("Audio::decode", &format!("AudioResample: {}", e)),
    };
    let opts = match ev.eval(args[1].clone()) {
        Value::Assoc(m) => m,
        _ => std::collections::HashMap::new(),
...
fn audio_concat(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("AudioConcat".into())), args };
    }
    let list = match ev.eval(args[0].clone()) {
        Value::List(xs) => xs,
        _ => return failure("Audio::concat", "Expected list of inputs"),
    };
    let opts = if args.len() > 1 {
        match ev.eval(args[1].clone()) {
            Value::Assoc(m) => m,
    ...
    for v in list {
        let b = match read_input_bytes(ev, v) {
            Ok(bb) => bb,
            Err(e) => return failure("Audio::input", &e),
        };
        match decode_with_symphonia(&b) {
            Ok(d) => decoded.push(d),
            Err(e) => return failure("Audio::decode", &e),
        }
    }
    if decoded.is_empty() {
        return failure("Audio::concat", "No inputs");
    }
    let target_sr = opts
        .get("sampleRate")
        .or(opts.get("SampleRate"))
        .and_then(|v| if let Value::Integer(i) = v { Some(*i as u32) } else { None })
        .unwrap_or(decoded[0].sample_rate);
    let target_ch = opts
        .get("channels")
        .or(opts.get("Channels"))
        .and_then(|v| if let Value::Integer(i) = v { Some(*i as u16) } else { None })
        .unwrap_or(decoded[0].channels);
    let mut out: Vec<f32> = Vec::new();
    for mut d in decoded {
        if d.sample_rate != target_sr {
            d.samples = resample_linear(&d.samples, d.channels, d.sample_rate, target_sr);
            d.sample_rate = target_sr;
        }
        if d.channels != target_ch {
            d.samples = mix_channels(&d.samples, d.channels, target_ch);
            d.channels = target_ch;
        }
...
fn audio_fade(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("AudioFade".into())), args };
    }
    let bytes = match read_input_bytes(ev, args[0].clone()) {
        Ok(b) => b,
        Err(e) => return failure("Audio::input", &format!("AudioFade: {}", e)),
    };
    let mut dec = match decode_with_symphonia(&bytes) {
        Ok(d) => d,
        Err(e) => return failure("Audio::decode", &format!("AudioFade: {}", e)),
    };
    let opts = match ev.eval(args[1].clone()) {
        Value::Assoc(m) => m,
        _ => std::collections::HashMap::new(),
...
fn audio_channel_mix(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("AudioChannelMix".into())), args };
    }
    let bytes = match read_input_bytes(ev, args[0].clone()) {
        Ok(b) => b,
        Err(e) => return failure("Audio::input", &format!("AudioChannelMix: {}", e)),
    };
    let mut dec = match decode_with_symphonia(&bytes) {
        Ok(d) => d,
        Err(e) => return failure("Audio::decode", &format!("AudioChannelMix: {}", e)),
    };
    let opts = match ev.eval(args[1].clone()) {
        Value::Assoc(m) => m,
        _ => std::collections::HashMap::new(),
    };
    let to = opts.get("to").or(opts.get("To")).and_then(|v| {
    ...
    let target_ch: u16 = match to.as_deref() {
        ...
        _ => opts
            ...
            .or(opts.get("Channels"))
            .and_then(|v| if let Value::Integer(i) = v { Some(*i as u16) } else { None })
            .unwrap_or(dec.channels),
    };
    if target_ch != dec.channels {
        dec.samples = mix_channels(&dec.samples, dec.channels, target_ch);
        dec.channels = target_ch;
    }
    match encode_wav_s16(dec.sample_rate, dec.channels, &dec.samples) {
        Ok(wav) => Value::String(base64url_encode(&wav)),
        Err(e) => failure("Audio::encode", &e),
...
fn audio_save(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("AudioSave".into())), args };
    }
    ...
    if maybe_assoc {
        ...
        let wav_bytes = match wav_b64 {
            Value::String(s) => base64url_decode(&s),
            _ => Err("AudioSave: invalid raw input".into()),
        };
        let (out_path, mkdirs) = match ev.eval(args[1].clone()) {
            Value::String(s) | Value::Symbol(s) => (s, false),
            Value::Assoc(m) => {
                let p = m.get("path").or(m.get("Path")).and_then(|v| {
                    if let Value::String(s) = v {
                        Some(s.clone())
                    } else {
                        None
                ...
                match p {
                    Some(pp) => (pp, mk),
                    None => return failure("Audio::save", "Missing output path"),
                }
            }
        ...
        return match wav_bytes {
            Ok(b) => match std::fs::write(&out_path, &b) {
                Ok(_) => Value::Assoc(
                    std::iter::once(("path".into(), Value::String(out_path))).collect(),
                ),
                Err(e) => failure("Audio::save", &e.to_string()),
            },
            Err(e) => failure("Audio::save", &e),
        };
    } else {
        let bytes = match read_input_bytes(ev, args[0].clone()) {
            Ok(b) => b,
            Err(e) => return failure("Audio::input", &format!("AudioSave: {}", e)),
        };
        dec = match decode_with_symphonia(&bytes) {
            Ok(d) => d,
            Err(e) => return failure("Audio::decode", &format!("AudioSave: {}", e)),
        };
    }
    let (out_path, mkdirs) = match ev.eval(args[1].clone()) {
        Value::String(s) | Value::Symbol(s) => (s, false),
    ...
    let mut enc_opts = if args.len() > 2 {
    ...
    if !enc_opts.contains_key("format") && !enc_opts.contains_key("Format") {
        if let Some(fmt) = infer_audio_format_from_ext(&out_path) {
            enc_opts.insert("format".into(), Value::String(fmt));
        }
    }
    let fmt = enc_opts
        .get("format")
        .or(enc_opts.get("Format"))
        .and_then(|v| if let Value::String(s) = v { Some(s.to_lowercase()) } else { None })
        .unwrap_or_else(|| "wav".into());
    let bytes = match fmt.as_str() {
        ...
        "mp3" => {
            #[cfg(feature = "audio_mp3")]
            {
                match ffmpeg_encode_to_bytes(dec, "mp3", Some(enc_opts)) {
                    Ok(b) => b,
                    Err(e) => return failure("Audio::save", &e),
                }
            }
            #[cfg(not(feature = "audio_mp3"))]
            {
        ...
        "ogg" | "vorbis" => {
            #[cfg(feature = "audio_ogg_vorbis")]
            {
                match ffmpeg_encode_to_bytes(dec, "ogg", Some(enc_opts)) {
                    Ok(b) => b,
                    Err(e) => return failure("Audio::save", &e),
                }
            }
        ...
        "flac" => {
            #[cfg(feature = "audio_flac")]
            {
                match ffmpeg_encode_to_bytes(dec, "flac", Some(enc_opts)) {
                    Ok(b) => b,
                    Err(e) => return failure("Audio::save", &e),
                }
            }
