...
#[derive(Clone)]
struct SpanRec { id: i64, name: String, attrs: HashMap<String, Value>, start: i64, end: Option<i64> }
...
fn spans() -> &'static Mutex<Vec<SpanRec>> { SPANS.get_or_init(|| Mutex::new(Vec::new())) }
fn stack() -> &'static Mutex<Vec<i64>> { STACK.get_or_init(|| Mutex::new(Vec::new())) }
...
fn span(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    // Span["name", Attrs-><|...|>]
    let name = args.get(0).and_then(|v| match v { Value::String(s)|Value::Symbol(s)=>Some(s.clone()), _=>None }).unwrap_or_else(|| "span".into());
    let mut attrs: HashMap<String, Value> = HashMap::new();
    for a in &args { if let Value::Assoc(m) = a { if let Some(Value::Assoc(am)) = m.get("Attrs") { attrs = am.clone(); } } }
    let id = next_id();
...
fn span_end(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    let id_opt = if args.is_empty() { stack().lock().unwrap().pop() } else { args.get(0).and_then(|v| if let Value::Integer(i)=v { Some(*i) } else { None }) };
    if let Some(id) = id_opt {
        let mut guard = spans().lock().unwrap();
        for s in guard.iter_mut().rev() { if s.id == id && s.end.is_none() { s.end = Some(now_ms()); break; } }
        Value::Boolean(true)
...
fn trace_get(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    ...
    let list: Vec<Value> = spans().lock().unwrap().iter().map(|s| Value::Assoc(HashMap::from([
...
fn trace_export(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    for a in &args { match a {
        ...
        Value::Assoc(m) => { if let Some(Value::String(p)) = m.get("Path") { path = Some(p.clone()); } }
        _=>{}
    }}
    let spans_list = trace_get(&mut Evaluator::new(), vec![]);
    if fmt=="json" {
        if let Some(p) = path { if let Value::List(items) = spans_list {
