...
pub fn register_rag_filtered(ev: &mut Evaluator, pred: &dyn Fn(&str) -> bool) {
    ...
    super::register_if(
        ...
        pred,
        "RAGAssembleContext",
        rag_assemble_context as NativeFn,
        Attributes::empty(),
    );
...
fn assoc_str(m: &HashMap<String, Value>, k: &str) -> Option<String> {
    m.get(k).and_then(as_str)
}

fn rag_chunk(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    // RAGChunk[text, <|Size->400, Overlap->40|>]
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("RAGChunk".into())), args };
    }
    ...
    if let Some(Value::Assoc(m)) = args.get(1) {
        if let Some(Value::Integer(n)) = m.get("Size") {
            size = (*n).max(1) as usize;
        }
        if let Some(Value::Integer(n)) = m.get("Overlap") {
            overlap = (*n).max(0) as usize;
        }
    }
...
fn rag_index(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("RAGIndex".into())), args };
    }
    ...
    if let Value::List(list) = docs {
        for d in list {
            if let Value::Assoc(m) = d {
                ...
                let _meta = m.get("meta").cloned().unwrap_or(Value::Assoc(HashMap::new()));
                ...
                if let Value::List(chs) = chunks {
                    for (idx, ch) in chs.into_iter().enumerate() {
                        if let Value::Assoc(cm) = ch {
                            ...
                            let emb = super::model::embed(
                                ev,
                                vec![Value::Assoc(HashMap::from([(
                                    String::from("Text"),
                                    Value::String(ctext.clone()),
                                )]))],
                            );
...
fn rag_retrieve(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("RAGRetrieve".into())), args };
    }
...
fn rag_assemble_context(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if let Some(Value::Assoc(m)) = args.get(1) {
        if let Some(Value::Integer(n)) = m.get("MaxChars") {
    ...
    if let Value::List(mm) = matches {
        for m in mm {
            if let Value::Assoc(am) = m {
                if let Some(Value::Assoc(meta)) = am.get("meta") {
                    if let Some(Value::String(t)) = meta.get("text") {
...
fn rag_answer(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("RAGAnswer".into())), args };
    }
    let store = args[0].clone();
    ...
    let matches = rag_retrieve(
        ev,
        vec![
            store,
            query.clone(),
            Value::Assoc(HashMap::from([(String::from("K"), Value::Integer(k))])),
        ],
    );
    let context = rag_assemble_context(
        ev,
        vec![
            matches.clone(),
            Value::Assoc(HashMap::from([(String::from("MaxChars"), Value::Integer(2000))])),
        ],
    );
    // Compose a simple prompt and call Chat
    let messages = Value::List(vec![
        Value::Assoc(HashMap::from([
            (String::from("role"), Value::String(String::from("system"))),
            (String::from("content"), Value::String(String::from("You are a helpful assistant."))),
        ])),
        Value::Assoc(HashMap::from([
            (String::from("role"), Value::String(String::from("user"))),
            (
                String::from("content"),
                Value::String(format!(
                    "Context:\n{}\n\nQuestion: {}",
    ...
    let reply = super::model::chat(
        ev,
        vec![Value::Assoc(HashMap::from([(String::from("Messages"), messages)]))],
    );
...
fn hybrid_search(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    let mut a2 = Vec::new();
    a2.push(args.get(0).cloned().unwrap_or(Value::String("mem".into())));
    a2.push(args.get(1).cloned().unwrap_or(Value::String(String::new())));
    a2.push(Value::Assoc(opts));
            ...
            for v in iter {
                if let Value::Assoc(m) = v {
                    ...
                    let text = m
                        ...
                        .unwrap_or_default();
                    let score_s = if with_scores {
                        match m.get("score") {
                            Some(Value::Real(r)) => format!(" (score: {:.3})", r),
                            Some(Value::Integer(i)) => format!(" (score: {})", i),
                            _ => String::new(),
                        }
                    } else {
                        String::new()
...
fn citations(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if let Some(v) = args.get(0) {
        match v {
            Value::Assoc(m) => {
                if let Some(Value::List(ms)) = m.get("matches") {
                    matches = ms.clone();
                } else if let Some(Value::List(cs)) = m.get("citations") {
                    matches = cs.clone();
                }
            }
    ...
    for m in matches.into_iter() {
        if let Value::Assoc(am) = m {
            ...
            if let Some(Value::Real(r)) = am.get("score") {
                score = Some(Value::Real(*r));
            }
            if let Some(Value::Integer(i)) = am.get("score") {
                score = Some(Value::Integer(*i));
            }
            match am.get("meta") {
                Some(Value::Assoc(mm)) => {
                    if let Some(Value::String(s)) = mm.get("text") {
                        text = s.clone();
                    }
                    if let Some(Value::String(u)) = mm.get("url") {
                        url = Some(u.clone());
                    }
                }
                Some(Value::String(s)) => {
            ...
            if let Some(sc) = score {
                m2.insert("score".into(), sc);
            }
            if let Some(u) = url {
                m2.insert("url".into(), Value::String(u));
            }
