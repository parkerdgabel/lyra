...
fn rt_reg() -> &'static Mutex<HashMap<i64, RuntimeState>> {
    RT_REG.get_or_init(|| Mutex::new(HashMap::new()))
}
fn next_rt_id() -> i64 {
    let a = NEXT_RT_ID.get_or_init(|| std::sync::atomic::AtomicI64::new(1));
...
fn get_container(v: &Value) -> Option<(i64, i64)> {
    if let Value::Assoc(m) = v {
        if matches!(m.get("__type"), Some(Value::String(s)) if s=="Container") {
            if let (Some(Value::Integer(rt)), Some(Value::Integer(id))) =
                (m.get("runtime_id"), m.get("id"))
            {
                return Some((*rt, *id));
            }
        }
...
fn connect_containers(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("ConnectContainers".into())), args };
    }
    let dsn = match ev.eval(args[0].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => {
            return Value::Expr {
                head: Box::new(Value::Symbol("ConnectContainers".into())),
                args: vec![other],
            }
        }
    };
    let kind = if dsn.starts_with("docker:") || dsn.starts_with("docker://") {
        RuntimeKind::Docker
    } else if dsn.starts_with("podman:") || dsn.starts_with("podman://") {
        RuntimeKind::Podman
    } else if dsn.starts_with("containerd:") || dsn.starts_with("containerd://") {
        RuntimeKind::Containerd
    } else if dsn.starts_with("k8s:") || dsn.starts_with("k8s://") || dsn.starts_with("kubernetes:")
    {
        RuntimeKind::K8s
    } else {
        RuntimeKind::Mock
...
fn disconnect_containers(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("DisconnectContainers".into())), args };
    }
    if let Some(id) = get_runtime(&args[0]) {
        rt_reg().lock().unwrap().remove(&id);
        Value::Boolean(true)
    } else {
        Value::Expr { head: Box::new(Value::Symbol("DisconnectContainers".into())), args }
    }
}

fn ping_containers(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("PingContainers".into())), args };
    }
...
fn runtime_info(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("RuntimeInfo".into())), args };
    }
...
fn runtime_capabilities(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("RuntimeCapabilities".into())), args };
    }
    match get_runtime(&args[0]) {
        Some(id) => {
            let reg = rt_reg().lock().unwrap();
            if let Some(st) = reg.get(&id) {
                let caps = match st.kind {
                    RuntimeKind::Mock => vec![
                        "Images",
                        "Containers",
                        "Exec",
                        "Logs",
                    ...
                    RuntimeKind::Containerd => {
                        vec!["Images", "Containers", "Exec", "Logs", "Stats", "Events"]
                    }
                    RuntimeKind::K8s => {
                        vec!["Images", "Containers", "Exec", "Logs", "Events", "Networks"]
                    }
                };
                Value::Assoc(HashMap::from([(
                    String::from("capabilities"),
                    Value::List(caps.into_iter().map(|s| Value::String(s.into())).collect()),
                )]))
            } else {
                Value::Assoc(HashMap::new())
            }
...
fn pull_image(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("PullImage".into())), args };
    }
    let rt = match get_runtime(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("PullImage".into())), args },
    };
    let name = match ev.eval(args[1].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => {
            return Value::Expr {
                head: Box::new(Value::Symbol("PullImage".into())),
    ...
    {
        let backend_is_docker = {
            let reg = rt_reg().lock().unwrap();
            reg.get(&rt).map(|s| matches!(s.kind, RuntimeKind::Docker)).unwrap_or(false)
        };
        if backend_is_docker {
            return containers_docker::docker_pull_image(&get_rt_dsn(rt), &name);
        }
    }
    let mut reg = rt_reg().lock().unwrap();
    if let Some(st) = reg.get_mut(&rt) {
        if !st.images.contains(&name) {
            st.images.push(name);
        }
    }
    Value::Boolean(true)
...
fn build_image(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    let rt = match get_runtime(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("BuildImage".into())), args },
    };
    #[cfg(feature = "containers_docker")]
    {
        let is_docker = {
        ...
        if is_docker {
            ...
            let (dockerfile, tag, buildargs, pull) = match args.get(1).cloned() {
                Some(Value::Assoc(m)) => {
                    ...
                    let opts = if args.len() >= 3 {
                        ev.eval(args[2].clone())
                    } else {
                        Value::Assoc(HashMap::new())
                    };
                    let (tag, buildargs, pull) = match opts {
                        Value::Assoc(mm) => {
                            let tag = mm.get("Tags").and_then(|v| match v {
                                Value::List(xs) if !xs.is_empty() => match &xs[0] {
                                    Value::String(s) | Value::Symbol(s) => Some(s.clone()),
                                    other => Some(lyra_core::pretty::format_value(other)),
                                },
                                Value::String(s) | Value::Symbol(s) => Some(s.clone()),
                                _ => None,
                            });
                            let buildargs = mm.get("BuildArgs").and_then(|v| match v {
                                Value::Assoc(am) => Some(am.clone()),
                                _ => None,
                            });
                            let pull = mm
                                .get("Pull")
            ...
            return containers_docker::docker_build_image(
                &get_rt_dsn(rt),
                dockerfile,
                tag,
                buildargs,
...
fn tag_image(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 3 {
        return Value::Expr { head: Box::new(Value::Symbol("TagImage".into())), args };
    }
    let rt = match get_runtime(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("TagImage".into())), args },
    };
    let dst = match ev.eval(args[2].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => {
            return Value::Expr {
                head: Box::new(Value::Symbol("TagImage".into())),
                args: vec![args[0].clone(), args[1].clone(), other],
            }
        }
    };
    #[cfg(feature = "containers_docker")]
    {
        ...
        if is_docker {
            let src = match ev.eval(args[1].clone()) {
                Value::String(s) | Value::Symbol(s) => s,
                _ => String::new(),
            };
...
fn list_images(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    #[cfg(feature = "containers_docker")]
    {
        if let Some(rt_id) = get_runtime(&args[0]) {
            ...
            if is_docker {
                return containers_docker::docker_list_images(&get_rt_dsn(rt_id));
            }
        }
    }
    Value::Expr {
        head: Box::new(Value::Symbol("DatasetFromRows".into())),
        args: vec![Value::List(Vec::new())],
    }
}
...
fn remove_image(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    #[cfg(feature = "containers_docker")]
    {
        if let Some(rt_id) = get_runtime(&args[0]) {
            ...
            if is_docker {
                let image = match ev.eval(args[1].clone()) {
                    Value::String(s) | Value::Symbol(s) => s,
                    other => lyra_core::pretty::format_value(&other),
                };
                let opts = if args.len() >= 3 {
...
fn search_images(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    #[cfg(feature = "containers_docker")]
    {
        if let Some(rt_id) = get_runtime(&args[0]) {
            ...
            if is_docker {
                let term = match ev.eval(args[1].clone()) {
                    Value::String(s) | Value::Symbol(s) => s,
                    other => lyra_core::pretty::format_value(&other),
                };
                let opts = if args.len() >= 3 {
    ...
    #[cfg(feature = "containers_docker")]
    {
        if let Some(rt_id) = get_runtime(&args[0]) {
            ...
            if is_docker {
                ...
                return containers_docker::docker_inspect_registry_image(
                    &get_rt_dsn(rt_id),
                    &image,
                    auth,
                );
...
fn export_images(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    #[cfg(feature = "containers_docker")]
    {
        if let Some(rt_id) = get_runtime(&args[0]) {
            ...
            if is_docker {
                let images: Vec<String> = match ev.eval(args[1].clone()) {
                    Value::List(xs) => xs
                        .into_iter()
                        .map(|v| match v {
                            Value::String(s) | Value::Symbol(s) => s,
...
fn run_container(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("RunContainer".into())), args };
    }
    match get_runtime(&args[0]) {
        Some(rt_id) => {
            #[cfg(feature = "containers_docker")]
            {
                ...
                if is_docker {
                    return containers_docker::docker_run_container(
                        rt_id,
                        &get_rt_dsn(rt_id),
                        args[1].clone(),
                    );
...
fn start_container(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 2 {
        return Value::Expr { head: Box::new(Value::Symbol("StartContainer".into())), args };
    }
    Value::Boolean(true)
}
...
fn restart_container(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 1 {
        return Value::Expr { head: Box::new(Value::Symbol("RestartContainer".into())), args };
    }
    Value::Boolean(true)
}
...
fn pause_container(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 2 {
        return Value::Expr { head: Box::new(Value::Symbol("PauseContainer".into())), args };
    }
    Value::Boolean(true)
}
fn unpause_container(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 2 {
        return Value::Expr { head: Box::new(Value::Symbol("UnpauseContainer".into())), args };
    }
    Value::Boolean(true)
}
fn rename_container(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 3 {
        return Value::Expr { head: Box::new(Value::Symbol("RenameContainer".into())), args };
    }
    Value::Boolean(true)
}
...
fn exec_in_container(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 3 {
        return Value::Expr { head: Box::new(Value::Symbol("ExecInContainer".into())), args };
    }
    ...
    #[cfg(feature = "containers_docker")]
    {
        if let Some((rt_id, cid)) = get_container(&args[1]) {
            ...
            if is_docker {
                return containers_docker::docker_exec_in_container(
                    &get_rt_dsn(rt_id),
                    cid,
                    args[2].clone(),
                );
...
fn copy_to_container(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 4 {
        return Value::Expr { head: Box::new(Value::Symbol("CopyToContainer".into())), args };
    }
    Value::Boolean(true)
}
fn copy_from_container(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 3 {
        return Value::Expr { head: Box::new(Value::Symbol("CopyFromContainer".into())), args };
    }
    Value::String(String::new())
}
...
fn wait_container(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 1 {
        return Value::Expr { head: Box::new(Value::Symbol("WaitContainer".into())), args };
    }
    Value::Assoc(HashMap::from([("status".into(), Value::String("exited".into()))]))
}
...
fn ccur_reg() -> &'static Mutex<HashMap<i64, CCurState>> {
    CCUR_REG.get_or_init(|| Mutex::new(HashMap::new()))
}
fn next_ccur_id() -> i64 {
    let a = NEXT_CCUR_ID.get_or_init(|| std::sync::atomic::AtomicI64::new(1));
    a.fetch_add(1, std::sync::atomic::Ordering::Relaxed)
}
...
fn containers_logs(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("Logs".into())), args };
    }
    let rt_id = match get_runtime(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("Logs".into())), args },
    };
    let reg = rt_reg().lock().unwrap();
    ...
    #[cfg(feature = "containers_docker")]
    {
        if let Some((rtid, cid)) = get_container(&args[1]) {
            let is_docker = {
                let reg = rt_reg().lock().unwrap();
                reg.get(&rtid).map(|s| matches!(s.kind, RuntimeKind::Docker)).unwrap_or(false)
            };
            if is_docker {
                buffer = containers_docker::docker_fetch_logs(&dsn, cid, false);
            }
        }
    }
    ccur_reg().lock().unwrap().insert(
        id,
        CCurState {
            kind: "logs".into(),
            rt_dsn: dsn,
...
fn containers_stats(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("Stats".into())), args };
    }
    let rt_id = match get_runtime(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("Stats".into())), args },
    };
    let reg = rt_reg().lock().unwrap();
    let dsn = reg.get(&rt_id).map(|s| s.dsn.clone()).unwrap_or_default();
    drop(reg);
...
fn containers_events(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 1 {
        return Value::Expr { head: Box::new(Value::Symbol("Events".into())), args };
    }
    let rt_id = match get_runtime(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("Events".into())), args },
    };
    let reg = rt_reg().lock().unwrap();
    let dsn = reg.get(&rt_id).map(|s| s.dsn.clone()).unwrap_or_default();
    drop(reg);
...
fn containers_fetch(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 1 {
        return Value::Expr { head: Box::new(Value::Symbol("ContainersFetch".into())), args };
    }
    let n = if args.len() >= 2 {
        if let Value::Integer(k) = args[1] {
            k.max(0) as usize
        } else {
            100
        }
...
fn containers_close(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("ContainersClose".into())), args };
    }
    if let Value::Assoc(m) = &args[0] {
        if let Some(Value::Integer(id)) = m.get("id") {
            ccur_reg().lock().unwrap().remove(id);
            return Value::Boolean(true);
        }
    }
    Value::Boolean(false)
...
fn list_volumes(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("ListVolumes".into())), args };
    }
    Value::Expr {
        head: Box::new(Value::Symbol("DatasetFromRows".into())),
        args: vec![Value::List(Vec::new())],
...
fn remove_volume(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("RemoveVolume".into())), args };
    }
    Value::Boolean(true)
}
fn inspect_volume(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("InspectVolume".into())), args };
    }
    Value::Assoc(HashMap::new())
}

fn list_networks(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("ListNetworks".into())), args };
    }
    Value::Expr {
        head: Box::new(Value::Symbol("DatasetFromRows".into())),
        args: vec![Value::List(Vec::new())],
...
fn remove_network(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("RemoveNetwork".into())), args };
    }
    Value::Boolean(true)
}
fn inspect_network(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("InspectNetwork".into())), args };
    }
    Value::Assoc(HashMap::new())
}

fn add_registry_auth(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 3 {
        return Value::Expr { head: Box::new(Value::Symbol("AddRegistryAuth".into())), args };
    }
    Value::Boolean(true)
}
fn list_registry_auth(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("ListRegistryAuth".into())), args };
    }
    Value::Expr {
        head: Box::new(Value::Symbol("DatasetFromRows".into())),
        args: vec![Value::List(Vec::new())],
...
#[allow(dead_code)]
fn get_rt_dsn(rt_id: i64) -> String {
    let reg = rt_reg().lock().unwrap();
    reg.get(&rt_id).map(|s| s.dsn.clone()).unwrap_or_default()
}
...
pub mod containers_docker {
    ...
    use bollard::image::{
        CreateImageOptions, ListImagesOptions, PushImageOptions, TagImageOptions,
    };
    use bollard::Docker;
    use futures_util::StreamExt;

    fn docker_client(_dsn: &str) -> Docker {
        Docker::connect_with_local_defaults().expect("Docker socket not available")
    }
    ...
    pub fn docker_pull_image(_dsn: &str, image: &str) -> Value {
        ...
        let res = rt.block_on(async move {
            let mut stream = docker.create_image(
                Some(CreateImageOptions { from_image: img.as_str(), ..Default::default() }),
                None,
                None,
            );
    ...
    pub fn docker_run_container(rt_id: i64, dsn: &str, spec_val: Value) -> Value {
        ...
        if let Value::Assoc(m) = spec_val {
            if let Some(Value::String(s)) | Some(Value::Symbol(s)) = m.get("Image") {
                image = s.clone();
            }
            if let Some(Value::String(s)) | Some(Value::Symbol(s)) = m.get("Name") {
                name = Some(s.clone());
            }
            if let Some(Value::List(xs)) = m.get("Cmd") {
                cmd = Some(
                    xs.iter()
                        .map(|v| match v {
        ...
        if rt.is_err() {
            return Value::Expr {
                head: Box::new(Value::Symbol("RunContainer".into())),
                args: vec![runtime_handle(rt_id)],
            };
        }
        let rt = rt.unwrap();
        let res = rt.block_on(async move {
            ...
            while let Some(_progress) = stream.next().await {}
            let mut cfg =
                ContainerConfig::<String> { image: Some(image.clone()), ..Default::default() };
            if let Some(c) = cmd {
                cfg.cmd = Some(c);
            }
            let create = docker
                .create_container(
                    name.as_deref().map(|n| CreateContainerOptions { name: n, platform: None }),
                    cfg,
                )
                .await;
            let id = match create {
            ...
            if docker.start_container(&id, None::<StartContainerOptions<String>>).await.is_err() {
                return Err(());
            }
            Ok(id)
        });
        match res {
            Ok(id) => {
                let mut reg = rt_reg().lock().unwrap();
                if let Some(st) = reg.get_mut(&rt_id) {
                    let nid = st.next_container_id;
                    st.next_container_id += 1;
                    st.containers.insert(
                        nid,
                        HashMap::from([(String::from("docker_id"), Value::String(id))]),
                    );
                    ...
                    .map(|c| {
                        ...
                        if let Some(names) = c.names {
                            if let Some(n) = names.first() {
                                m.insert(
                                    "name".into(),
                                    Value::String(n.trim_start_matches('/').to_string()),
                                );
                            }
                        }
                        ...
                        m.insert(
                            "created".into(),
                            Value::Integer(c.created.unwrap_or_default() as i64),
                        );
                        Value::Assoc(m)
                    })
                    .collect();
    ...
    pub fn docker_inspect_container(dsn: &str, cid: i64) -> Value {
        let docker_id = {
            let reg = rt_reg().lock().unwrap();
            reg.values().find_map(|st| {
                st.containers.get(&cid).and_then(|m| m.get("docker_id")).and_then(|v| match v {
        ...
        let rt = tokio::runtime::Runtime::new();
        if rt.is_err() {
            return Value::Assoc(HashMap::new());
        }
        let rt = rt.unwrap();
        ...
        match res {
            Ok(info) => {
                ...
                if let Some(name) = info.name {
                    m.insert(
                        "name".into(),
                        Value::String(name.trim_start_matches('/').to_string()),
                    );
                }
                if let Some(cfg) = info.config {
                    if let Some(img) = cfg.image {
                        m.insert("image".into(), Value::String(img));
                    }
                }
                Value::Assoc(m)
            }
                    ...
                    match chunk {
                        LogOutput::StdOut { message } => {
                            ...
                            out.push(Value::Assoc(HashMap::from([
                                (String::from("stream"), Value::String(String::from("stdout"))),
                        ...
                        LogOutput::StdErr { message } => {
                            let msg = String::from_utf8_lossy(&message).to_string();
                            out.push(Value::Assoc(HashMap::from([
                                (String::from("stream"), Value::String(String::from("stderr"))),
                                (String::from("message"), Value::String(msg)),
                            ])));
                        }
                        LogOutput::Console { message } => {
                            let msg = String::from_utf8_lossy(&message).to_string();
                            out.push(Value::Assoc(HashMap::from([
                                (String::from("stream"), Value::String(String::from("console"))),
                                (String::from("message"), Value::String(msg)),
                            ])));
                        }
                        _ => {}
    ...
    pub fn docker_exec_in_container(dsn: &str, cid: i64, spec: Value) -> Value {
        ...
        if cmd.is_empty() {
            return Value::Assoc(HashMap::from([(String::from("exit_code"), Value::Integer(127))]));
        }
        let docker = docker_client(dsn);
        let rt = tokio::runtime::Runtime::new();
        if rt.is_err() {
        ...
        match res {
            Ok((code, out, err)) => Value::Assoc(HashMap::from([
                (String::from("exit_code"), Value::Integer(code as i64)),
                (String::from("stdout"), Value::String(out)),
                (String::from("stderr"), Value::String(err)),
            ])),
            Err(_) => Value::Assoc(HashMap::new()),
        }
    }
        ...
        match res {
            Ok(imgs) => {
                ...
                for img in imgs {
                    ...
                    for t in tags {
                        let (repo, tag) = if let Some((r, tg)) = t.rsplit_once(':') {
                            (r.to_string(), tg.to_string())
                        } else {
                            (t.clone(), "latest".into())
                        };
                        let mut m = HashMap::new();
    ...
    pub fn docker_tag_image(dsn: &str, src: String, dst: String) -> Value {
        let (repo, tag) = if let Some((r, t)) = dst.rsplit_once(':') {
            (r.to_string(), t.to_string())
        } else {
            (dst, String::from("latest"))
        };
        let docker = docker_client(dsn);
        let rt = tokio::runtime::Runtime::new();
        if rt.is_err() {
        ...
        let rt = rt.unwrap();
        let res = rt.block_on(async move {
            docker.tag_image(&src, Some(TagImageOptions { repo, tag })).await
        });
        Value::Boolean(res.is_ok())
    }
    ...
    pub fn docker_push_image(dsn: &str, image: String, opts: Option<Value>) -> Value {
        ...
        if let Some(Value::Assoc(m)) = opts {
            let username = m.get("Username").and_then(|v| match v {
                Value::String(s) | Value::Symbol(s) => Some(s.clone()),
                _ => None,
            });
            let password = m.get("Password").and_then(|v| match v {
                Value::String(s) | Value::Symbol(s) => Some(s.clone()),
                _ => None,
            });
            let serveraddress = m.get("Registry").and_then(|v| match v {
                Value::String(s) | Value::Symbol(s) => Some(s.clone()),
                _ => None,
            });
            if username.is_some() || password.is_some() || serveraddress.is_some() {
                creds = Some(DockerCredentials {
                    username,
                    password,
                    serveraddress,
                    ..Default::default()
        ...
        let res = rt.block_on(async move {
            let mut stream =
                docker.push_image(&repo, Some(PushImageOptions { tag: tag.as_str() }), creds);
            while let Some(_progress) = stream.next().await {}
            Ok::<(), ()>(())
        });
        Value::Boolean(res.is_ok())
    ...
    pub fn docker_build_image(
        ...
        if builder.append(&header, dockerfile.as_bytes()).is_err() {
            return Value::Boolean(false);
        }
        let uncompressed = match builder.into_inner() {
            Ok(v) => v,
        ...
        let buildargs_map: std::collections::HashMap<String, String> = buildargs
            .unwrap_or_default()
            .into_iter()
            .map(|(k, v)| {
                (
        ...
        match res {
            Ok(info) => {
                ...
                if let Some(size) = info.size {
                    m.insert("size".into(), Value::Integer(size as i64));
                }
                if let Some(v) = info.os {
                    m.insert("os".into(), Value::String(v));
                }
                if let Some(v) = info.architecture {
                    m.insert("arch".into(), Value::String(v));
                }
                if let Some(v) = info.created {
                    m.insert("created".into(), Value::String(v));
                }
                if let Some(config) = info.config {
                    if let Some(env) = config.env {
                        m.insert(
                            "env".into(),
                            Value::List(env.into_iter().map(Value::String).collect()),
                        );
                    }
                }
    ...
    pub fn docker_save_image(dsn: &str, image: &str) -> Value {
        ...
        match res {
            Ok(data) => Value::String(base64::engine::general_purpose::STANDARD.encode(&data)),
    ...
    pub fn docker_remove_image(dsn: &str, image: &str, opts: Value) -> Value {
        ...
        let (force, noprune, creds) = match opts {
            Value::Assoc(m) => {
                ...
                let creds = if username.is_some() || password.is_some() || serveraddress.is_some() {
        ...
        let rt = rt.unwrap();
        let res = rt.block_on(async move {
            docker.remove_image(image, Some(RemoveImageOptions { force, noprune }), creds).await
        });
        Value::Boolean(res.is_ok())
    }
    ...
    pub fn docker_prune_images(dsn: &str, opts: Value) -> Value {
        ...
        let rt = rt.unwrap();
        let res = rt.block_on(async move {
            docker.prune_images::<String>(Some(PruneImagesOptions { filters })).await
        });
        match res {
            Ok(resp) => {
                ...
                if let Some(deleted) = resp.images_deleted {
                    let rows: Vec<Value> = deleted
                        .into_iter()
                        .map(|d| {
                            let mut m = HashMap::new();
                            if let Some(u) = d.untagged {
                                m.insert("untagged".into(), Value::String(u));
                            }
                            if let Some(u) = d.deleted {
                                m.insert("deleted".into(), Value::String(u));
                            }
                            Value::Assoc(m)
                        })
                        .collect();
    ...
    pub fn docker_search_images(dsn: &str, term: String, opts: Value) -> Value {
        ...
        let (limit, filters) = match opts {
            Value::Assoc(m) => {
                let limit = m.get("Limit").and_then(|v| match v {
                    Value::Integer(n) => Some(*n as u64),
                    _ => None,
                });
                let filters = if let Some(Value::Assoc(fm)) = m.get("Filters") {
                    fm.iter()
                        .map(|(k, v)| {
                            (
                                k.clone(),
        ...
        let rt = rt.unwrap();
        let res = rt.block_on(async move {
            docker.search_images(SearchImagesOptions { term, limit, filters }).await
        });
        match res {
            Ok(items) => {
                let rows: Vec<Value> = items
                    ...
                    .map(|it| {
                        ...
                        if let Some(s) = it.star_count {
                            m.insert("stars".into(), Value::Integer(s as i64));
                        }
                        if let Some(o) = it.is_official {
                            m.insert("official".into(), Value::Boolean(o));
                        }
                        if let Some(a) = it.is_automated {
                            m.insert("automated".into(), Value::Boolean(a));
                        }
                        Value::Assoc(m)
                    })
                    .collect();
            ...
            Ok(items) => {
                let rows: Vec<Value> = items
                    ...
                    .map(|h| {
                        ...
                        m.insert(
                            "tags".into(),
                            Value::List(h.tags.into_iter().map(Value::String).collect()),
                        );
    ...
    pub fn docker_inspect_registry_image(dsn: &str, image: &str, auth: Value) -> Value {
        ...
        match res {
            Ok(info) => {
                let mut m = HashMap::new();
                if let Some(d) = info.descriptor.digest {
                    m.insert("digest".into(), Value::String(d));
                }
                {
                    let rows: Vec<Value> = info
                        ...
                        .map(|p| {
                            ...
                            if let Some(os) = p.os {
                                mm.insert("os".into(), Value::String(os));
                            }
                            if let Some(arch) = p.architecture {
                                mm.insert("arch".into(), Value::String(arch));
                            }
                            Value::Assoc(mm)
                        })
                        .collect();
...
pub fn register_containers_filtered(ev: &mut Evaluator, pred: &dyn Fn(&str) -> bool) {
    ...
    register_if(
        ...
        pred,
        "RuntimeCapabilities",
        runtime_capabilities as NativeFn,
        Attributes::empty(),
    );
    ...
    register_if(
        ...
        pred,
        "DescribeContainers",
        describe_containers as NativeFn,
        Attributes::empty(),
    );
    ...
    register_if(
        ...
        disconnect_containers as NativeFn,
        Attributes::empty(),
    );
    ...
    register_if(
        ...
        pred,
        "CopyFromContainer",
        copy_from_container as NativeFn,
        Attributes::empty(),
    );
    ...
    register_if(
        ev,
        pred,
        "InspectRegistryImage",
        inspect_registry_image as NativeFn,
        Attributes::empty(),
    );
