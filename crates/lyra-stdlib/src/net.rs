...
pub fn register_net(ev: &mut Evaluator) {
    ...
    #[cfg(feature = "tools")]
    add_specs(vec![
        tool_spec!("HttpGet", summary: "HTTP GET request (http/https)", params: ["url","opts"], tags: ["net","http"], examples: [Value::String("HttpGet[\"https://example.com\"]".into())], effects: ["net.http"]),
        tool_spec!("HttpPost", summary: "HTTP POST request (http/https)", params: ["url","body","opts"], tags: ["net","http"], examples: [Value::String("HttpPost[\"https://example.com/api\", <|\"json\"-><|\"x\"->1|>|>]".into())], effects: ["net.http"]),
...
pub fn register_net_filtered(ev: &mut Evaluator, pred: &dyn Fn(&str) -> bool) {
    ...
    register_if(
        ...
        pred,
        "HttpStreamRequest",
        http_stream_request as NativeFn,
        Attributes::empty(),
    );
    ...
    register_if(
        ...
        pred,
        "HttpDownloadCached",
        http_download_cached as NativeFn,
        Attributes::empty(),
    );
    ...
    register_if(
        ...
        pred,
        "GetResponseCookies",
        get_response_cookies as NativeFn,
        Attributes::empty(),
    );
    #[cfg(feature = "net_https")]
...
enum MultipartValue {
    Text(String),
    File { path: String, filename: Option<String>, content_type: Option<String> },
}
...
fn collect_multipart(ev: &mut Evaluator, v: Value) -> Option<Vec<MultipartPart>> {
    ...
    match v {
        Value::Assoc(m) => {
            for (k, vv) in m.into_iter() {
                if let Some(pv) = to_mpart_value(ev, vv) {
                    out.push(MultipartPart { name: k, value: pv });
                }
            }
        }
        Value::List(items) => {
            for it in items {
                match ev.eval(it) {
                    Value::List(mut pair) if pair.len() == 2 => {
                        ...
                        let kn = value_to_string(&key);
                        if let Some(pv) = to_mpart_value(ev, val) {
                            out.push(MultipartPart { name: kn, value: pv });
                        }
                    }
                    _ => {}
                }
...
fn to_mpart_value(ev: &mut Evaluator, v: Value) -> Option<MultipartValue> {
    ...
    match vv {
        ...
        Value::Expr { head, args } if matches!(&*head, Value::Symbol(s) if s=="File") => {
            let path = args.get(0).and_then(|a| match ev.eval(a.clone()) {
                Value::String(s) | Value::Symbol(s) => Some(s),
                _ => None,
            })?;
            let mut filename: Option<String> = None;
            let mut ctype: Option<String> = None;
            if let Some(Value::Assoc(opts)) = args.get(1).and_then(|a| match ev.eval(a.clone()) {
                Value::Assoc(m) => Some(Value::Assoc(m)),
                _ => None,
            }) {
                if let Some(Value::String(s)) | Some(Value::Symbol(s)) = opts.get("Filename") {
                    filename = Some(s.clone());
                }
                if let Some(Value::String(s)) | Some(Value::Symbol(s)) = opts.get("ContentType") {
                    ctype = Some(s.clone());
                }
            }
...
fn http_get(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("HttpGet".into())), args };
    }
    let url = to_string_arg(ev, args[0].clone());
    let opts = http_opts_from(ev, args.get(1).cloned());
    match http_request("GET", &url, None, &opts) {
        Ok(resp) => resp,
...
fn http_post(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("HttpPost".into())), args };
    }
    let url = to_string_arg(ev, args[0].clone());
    let body = value_to_bytes(ev.eval(args[1].clone()));
    let opts = http_opts_from(ev, args.get(2).cloned());
    match http_request("POST", &url, Some(&body), &opts) {
        Ok(resp) => resp,
        Err(e) => failure("HTTP::error", &e),
    }
}
...
fn http_put(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("HttpPut".into())), args };
    }
    let url = to_string_arg(ev, args[0].clone());
    let body = value_to_bytes(ev.eval(args[1].clone()));
    let opts = http_opts_from(ev, args.get(2).cloned());
    match http_request("PUT", &url, Some(&body), &opts) {
        Ok(resp) => resp,
        Err(e) => failure("HTTP::error", &e),
    }
}
...
fn http_patch(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("HttpPatch".into())), args };
    }
    let url = to_string_arg(ev, args[0].clone());
    let body = value_to_bytes(ev.eval(args[1].clone()));
    let opts = http_opts_from(ev, args.get(2).cloned());
    match http_request("PATCH", &url, Some(&body), &opts) {
        Ok(resp) => resp,
        Err(e) => failure("HTTP::error", &e),
    }
}
...
fn http_delete(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("HttpDelete".into())), args };
    }
    let url = to_string_arg(ev, args[0].clone());
    let opts = http_opts_from(ev, args.get(1).cloned());
    match http_request("DELETE", &url, None, &opts) {
        Ok(resp) => resp,
        Err(e) => failure("HTTP::error", &e),
    }
}
...
fn http_head(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("HttpHead".into())), args };
    }
    let url = to_string_arg(ev, args[0].clone());
    let opts = http_opts_from(ev, args.get(1).cloned());
    match http_request("HEAD", &url, None, &opts) {
        Ok(resp) => resp,
        Err(e) => failure("HTTP::error", &e),
    }
}
...
fn http_request_generic(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("HttpRequest".into())), args };
    }
    ...
    if let Value::Assoc(m) = opts_v {
        ...
        if url.is_empty() {
            return failure("HTTP::request", "Url required");
        }
        let mut body_bytes: Option<Vec<u8>> = None;
        if let Some(b) = m.get("Body") {
            body_bytes = Some(value_to_bytes(ev.eval(b.clone())));
        }
        let opts = http_opts_from(ev, Some(Value::Assoc(m)));
        match http_request(&method, &url, body_bytes.as_deref(), &opts) {
            Ok(resp) => resp,
            Err(e) => failure("HTTP::error", &e),
        }
    } else {
        failure("HTTP::request", "Options must be an association")
    }
}
fn http_options(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("HttpOptions".into())), args };
    }
    let url = to_string_arg(ev, args[0].clone());
    let opts = http_opts_from(ev, args.get(1).cloned());
    match http_request("OPTIONS", &url, None, &opts) {
        Ok(resp) => resp,
        Err(e) => failure("HTTP::error", &e),
    }
}
...
fn download(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("Download".into())), args };
    }
    let url = to_string_arg(ev, args[0].clone());
        ...
        Ok((status, _status_text, _headers, body)) => {
            if status >= 200 && status < 300 {
                match std::fs::write(&path, &body) {
                    ...
                    Err(e) => failure("HTTP::fs", &format!("Download write: {}", e)),
                }
            } else {
                failure("HTTP::status", &format!("Download failed: status {}", status))
            }
        }
        Err(e) => failure("HTTP::error", &e),
    }
...
fn download_stream(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("DownloadStream".into())), args };
    }
    let url = to_string_arg(ev, args[0].clone());
    let path = to_string_arg(ev, args[1].clone());
    let opts = http_opts_from(ev, args.get(2).cloned());
    match download_stream_impl(&url, &path, &opts) {
        Ok(()) => Value::Boolean(true),
        Err(e) => failure("HTTP::download", &e),
    }
}
...
fn build_response(
    ...
    let headers_list_val = Value::List(
        headers
            .iter()
            .map(|(k, v)| {
                Value::Assoc(
                    vec![("name".into(), Value::String(k.clone())), ("value".into(), v.clone())]
                        .into_iter()
                        .collect(),
                )
            })
    ...
    if want_json {
        ...
        match opts.as_kind {
            AsKind::Bytes => {
                map.insert(
                    "bytes".into(),
                    Value::List(body.into_iter().map(|b| Value::Integer(b as i64)).collect()),
                );
            }
            _ => {
                map.insert("body".into(), Value::String(text));
...
fn http_request_raw(
    ...
    body: Option<&[u8]>,
    opts: &HttpOpts,
) -> Result<(u16, String, Vec<(String, Value)>, Vec<u8>), String> {
    // Append query from opts
    let mut url_final = url.to_string();
    if !opts.query.is_empty() {
        url_final = append_query(&url_final, &opts.query);
    }
    let parsed = parse_http_url(&url_final)
        .ok_or_else(|| format!("Unsupported or invalid URL: {}", url_final))?;
    if parsed.scheme != "http" {
        return Err("Only http scheme supported; https not available".into());
    }
    use std::io::Read;
    ...
    let stream = if let Some(ms) = opts.timeout_ms {
        TcpStream::connect_timeout(&addr, std::time::Duration::from_millis(ms))
    } else {
        TcpStream::connect(addr)
    };
    let mut stream = stream.map_err(|e| format!("Connect: {}", e))?;
    if let Some(ms) = opts.timeout_ms {
        let d = Some(std::time::Duration::from_millis(ms));
    ...
    if let Some(parts) = &opts.multipart {
        let (ctype, bytes) = build_multipart_body(parts);
        req.push_str(&format!("Content-Type: {}\r\n", ctype));
        body_bytes = bytes;
    } else if let Some(j) = &opts.json_body {
        let s = sj::to_string(j).unwrap_or_else(|_| String::new());
        body_bytes = s.into_bytes();
        req.push_str("Content-Type: application/json\r\n");
    } else if let Some(form) = &opts.form_body {
        let s = form_urlencode(form);
        body_bytes = s.into_bytes();
...
fn has_header_case(headers: &[(String, String)], name: &str, value_contains: &str) -> bool {
    headers.iter().any(|(k, v)| {
        k.eq_ignore_ascii_case(name)
            && v.to_ascii_lowercase().contains(&value_contains.to_ascii_lowercase())
    })
}

fn read_chunked<R: std::io::Read>(
    reader: &mut R,
    initial: &[u8],
    max: usize,
...
fn parse_request_has_chunked(headers: &[(String, String)]) -> bool {
    has_header_case(headers, "Transfer-Encoding", "chunked")
}
...
fn parse_http_url(url: &str) -> Option<ParsedUrl> {
    ...
    let (scheme, rest) = if let Some(pos) = lower.find("://") {
        (&lower[..pos], &url[pos + 3..])
    } else {
        return None;
    };
    let (authority, path) =
        if let Some(pos) = rest.find('/') { (&rest[..pos], &rest[pos..]) } else { (rest, "/") };
    let (host, port) = if let Some(pos) = authority.rfind(':') {
        (&authority[..pos], authority[pos + 1..].parse::<u16>().ok().unwrap_or(80))
    } else {
        (authority, 80)
    };
...
fn append_query(base: &str, pairs: &Vec<(String, String)>) -> String {
    ...
    let qs = pairs
        .iter()
        .map(|(k, v)| {
            format!("{}={}", percent_encode_component(k, false), percent_encode_component(v, true))
        })
        .collect::<Vec<_>>()
        .join("&");
    format!("{}{}{}", base, sep, qs)
...
fn download_stream_raw(url: &str, path: &str, opts: &HttpOpts) -> Result<(), String> {
    ...
    let _header_bytes = &buf[..header_end_pos];
    let (status, _status_text, headers, _body0) =
        parse_http_response(&buf).map_err(|e| format!("parse: {}", e))?;
    if status < 200 || status >= 300 {
    ...
    if has_header_case(
        ...
        loop {
            ...
            while line_end.is_none() {
                let n = stream.read(&mut tmp).map_err(|e| format!("Read: {}", e))?;
                if n == 0 {
                    return Err("EOF in chunked size".into());
                }
    ...
    } else if let Some((_, Value::String(cl))) =
        headers.iter().find(|(k, _)| k.eq_ignore_ascii_case("Content-Length"))
    {
        ...
        if !body_initial.is_empty() {
            file.write_all(body_initial).map_err(|e| format!("write: {}", e))?;
            written += body_initial.len();
        }
        while written < len {
            let n = stream.read(&mut tmp).map_err(|e| format!("Read: {}", e))?;
            if n == 0 {
...
fn download_stream_reqwest(url: &str, path: &str, opts: &HttpOpts) -> Result<(), String> {
    ...
    if opts.tls_insecure {
        cb = cb.danger_accept_invalid_certs(true).danger_accept_invalid_hostnames(true);
    }
    if let Some(n) = opts.follow_redirects {
        cb = if n == 0 {
            cb.redirect(redirect::Policy::none())
        } else {
            cb.redirect(redirect::Policy::limited(n))
        }
    }
...
fn collect_kv_pairs(ev: &mut Evaluator, v: Value) -> Vec<(String, String)> {
    match ev.eval(v) {
        ...
        Value::List(items) => items
            .into_iter()
            .filter_map(|it| match ev.eval(it) {
                Value::List(mut pair) if pair.len() == 2 => {
                    let b = pair.pop().unwrap();
                    let a = pair.pop().unwrap();
                    Some((value_to_string(&a), value_to_string(&b)))
                }
                _ => None,
            })
            .collect(),
...
use std::sync::{Mutex as Mutex2, OnceLock as OnceLock2};
static STREAM_REG: OnceLock2<
    Mutex2<std::collections::HashMap<i64, Box<dyn std::io::Read + Send>>>,
> = OnceLock2::new();
static STREAM_NEXT: OnceLock2<AtomicI64_2> = OnceLock2::new();
fn sreg() -> &'static Mutex2<std::collections::HashMap<i64, Box<dyn std::io::Read + Send>>> {
    STREAM_REG.get_or_init(|| Mutex2::new(std::collections::HashMap::new()))
}
fn snext() -> i64 {
    STREAM_NEXT
        .get_or_init(|| AtomicI64_2::new(1))
        .fetch_add(1, std::sync::atomic::Ordering::Relaxed)
}

fn http_stream_request(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("HttpStreamRequest".into())), args };
    }
    ...
    #[cfg(feature = "net_https")]
    {
        ...
        let client = match cb.build() {
            Ok(c) => c,
            Err(e) => return failure("HTTP::stream", &e.to_string()),
        };
        let m = match Method::from_bytes(method.as_bytes()) {
            Ok(m) => m,
            Err(e) => return failure("HTTP::stream", &e.to_string()),
        };
        ...
        let id = snext();
        sreg().lock().unwrap().insert(id, reader);
        return Value::Assoc(
            vec![
...
fn http_stream_read(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("HttpStreamRead".into())), args };
    }
                ...
                Value::Assoc(
                    vec![
                        (
                            "chunk".into(),
                            Value::List(
                                buf.into_iter().map(|b| Value::Integer(b as i64)).collect(),
                            ),
                        ),
                        ("done".into(), Value::Boolean(n == 0)),
                    ]
                    .into_iter()
                    .collect(),
                )
            }
            Err(e) => failure("HTTP::stream", &e.to_string()),
        }
    } else {
        failure("HTTP::stream", "Invalid stream handle")
    }
}

fn http_stream_close(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("HttpStreamClose".into())), args };
    }
    let id = match ev.eval(args[0].clone()) {
        Value::Integer(n) => n,
        _ => 0,
...
fn http_download_cached(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("HttpDownloadCached".into())), args };
    }
    let url = to_string_arg(ev, args[0].clone());
    let dest = to_string_arg(ev, args[1].clone());
    let opts = http_opts_from(ev, args.get(2).cloned());
    let _cache_dir = match args.get(2).and_then(|v| {
        if let Value::Assoc(m) = ev.eval(v.clone()) {
            m.get("CacheDir").cloned()
        } else {
            None
        }
    }) {
        Some(Value::String(s)) | Some(Value::Symbol(s)) => Some(s),
        _ => None,
    };
    let ttl_ms = match args.get(2).and_then(|v| {
        ...
        Some(Value::Integer(n)) if n > 0 => Some(n as i64),
        _ => None,
    };
    ...
    if let (Some(ttl), Ok(md)) = (ttl_ms, std::fs::metadata(&dest)) {
        let age = std::time::SystemTime::now()
            .duration_since(md.modified().unwrap_or(std::time::SystemTime::UNIX_EPOCH))
            .map(|d| d.as_millis() as i64)
            .unwrap_or(i64::MAX);
        if age < ttl {
            if let Ok(s) = std::fs::read_to_string(&meta_path) {
                etag_old = Some(s.trim().to_string());
            }
        }
    }
    #[cfg(feature = "net_https")]
    {
        ...
        let client = match cb.build() {
            Ok(c) => c,
            Err(e) => return failure("HTTP::cache", &e.to_string()),
        };
        let mut url_final = url.clone();
        if !opts.query.is_empty() {
            url_final = append_query(&url_final, &opts.query);
        ...
        if resp.status() == reqwest::StatusCode::NOT_MODIFIED {
            return Value::Assoc(
                vec![
                    ...
                    ("bytes_written".into(), Value::Integer(0)),
                    ("from_cache".into(), Value::Boolean(true)),
                    ("etag".into(), Value::String(etag_old.unwrap_or_default())),
                ]
                .into_iter()
                .collect(),
            );
        ...
        let n = match std::io::copy(&mut resp, &mut file) {
            Ok(n) => n,
            Err(e) => return failure("HTTP::cache", &e.to_string()),
        } as i64;
        if let Some(et) = resp.headers().get("ETag").and_then(|v| v.to_str().ok()) {
            let _ = std::fs::write(&meta_path, et.as_bytes());
        }
        return Value::Assoc(
            vec![
                ("path".into(), Value::String(dest)),
                ("bytes_written".into(), Value::Integer(n)),
                ("from_cache".into(), Value::Boolean(false)),
            ]
            .into_iter()
            .collect(),
        );
...
fn http_retry(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("HttpRetry".into())), args };
    }
    let req_map = match ev.eval(args[0].clone()) {
        Value::Assoc(m) => m,
        _ => return failure("HTTP::retry", "Request map required"),
    };
    ...
    let attempts = opts
        .get("Attempts")
        .and_then(|v| if let Value::Integer(n) = v { Some(*n as i32) } else { None })
        .unwrap_or(3)
        .max(1);
    let backoff_min = opts
        .get("BackoffMs")
        .and_then(|v| if let Value::Integer(n) = v { Some(*n as i64) } else { None })
        .unwrap_or(200);
    ...
    for i in 0..attempts {
        ...
        if let Value::Assoc(m) = &resp {
            if let Some(Value::Integer(code)) = m.get("status") {
...
fn http_request_reqwest(
    ...
    if let Some(parts) = &opts.multipart {
        ...
        for p in parts {
            match &p.value {
                MultipartValue::Text(s) => {
                    form = form.text(p.name.clone(), s.clone());
                }
                MultipartValue::File { path, filename, content_type } => {
    ...
    for (k, v) in resp.headers().iter() {
        headers_vec
            .push((k.as_str().to_string(), Value::String(v.to_str().unwrap_or("").to_string())));
    }
...
fn tls_opts_from(ev: &mut Evaluator, v: Option<Value>) -> Result<(ServerOpts, TlsOpts), String> {
    ...
    if let Some(Value::Assoc(m)) = v.map(|x| ev.eval(x)) {
        let cert_path = match m.get("CertPath") {
            Some(Value::String(s)) | Some(Value::Symbol(s)) => s.clone(),
            _ => return Err("CertPath required".into()),
        };
        let key_path = match m.get("KeyPath") {
            Some(Value::String(s)) | Some(Value::Symbol(s)) => s.clone(),
            _ => return Err("KeyPath required".into()),
        };
        Ok((srv, TlsOpts { cert_path, key_path }))
    } else {
        Err("TLS options required".into())
    }
}
...
fn servers_reg() -> &'static Mutex<HashMap<i64, ServerEntry>> {
    SERVERS.get_or_init(|| Mutex::new(HashMap::new()))
}
fn next_srv_id() -> i64 {
    let a = NEXT_SRV_ID.get_or_init(|| AtomicI64::new(1));
...
fn http_serve(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("HttpServe".into())), args };
    }
                ...
                Ok((mut stream, _peer)) => {
                    ...
                    std::thread::spawn(move || {
                        ...
                        loop {
                            ...
                            if let Some(p) = find_subsequence(&buf, b"\r\n\r\n") {
                                header_end = p + 4;
                                header = buf[..header_end].to_vec();
                                if let Some(cl) = parse_content_length(&header) {
                                    body_len_opt = Some(cl);
                                }
                                break;
                            }
                        }
                        ...
                        let _ = parse_request_head(&header).map(|(_, _, hs)| {
                            headers_vec = hs;
                        });
                        ...
                        if parse_request_has_chunked(&headers_vec) {
                            match read_chunked(&mut stream, &body, max_body) {
                                Ok(data) => body = data,
                                Err(_) => {}
                            }
                        } else if let Some(blen) = body_len_opt {
                            while body.len() < blen {
                                match stream.read(&mut tmp) {
                                    Ok(0) => break,
                                    Ok(n) => {
                        ...
                        // Parse request
                        let (method, target, headers) = match parse_request_head(&header) {
                            Some(t) => t,
                            None => {
                                let _ =
                                    write_simple_resp(&mut stream, 400, "Bad Request", &[], b"");
                                return;
                            }
                        };
                        ...
                        for (k, v) in headers {
                            hdr_assoc.insert(k, Value::String(v));
                        }
                        req_map.insert("headers".into(), Value::Assoc(hdr_assoc));
                        req_map.insert(
                            "body".into(),
                            Value::List(body.iter().map(|b| Value::Integer(*b as i64)).collect()),
                        );
                        // Evaluate handler
                        ...
                        if let Some(file_path) = spec.body_file.clone() {
                            ...
                            let _ = write_chunked_resp(
                                &mut stream,
                                status,
                                "OK",
                                hdrs,
                                Box::new(FileChunkIter::new(&file_path)),
                            );
                            ...
                            let _ = write_chunked_resp(
                                ...
                                "OK",
                                hdrs,
                                Box::new(ChannelChunkIter::new(cid)),
                            );
...
fn http_server_stop(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("HttpServerStop".into())), args };
    }
    let id_opt = match &args[0] {
        Value::Expr { head, args } if matches!(&**head, Value::Symbol(s) if s=="ServerId") => {
...
fn http_server_addr(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("HttpServerAddr".into())), args };
    }
    let id_opt = match &args[0] {
        Value::Expr { head, args } if matches!(&**head, Value::Symbol(s) if s=="ServerId") => {
            args.get(0).and_then(|v| if let Value::Integer(i) = v { Some(*i) } else { None })
    ...
    if let Some(id) = id_opt {
        if let Some(entry) = servers_reg().lock().unwrap().get(&id) {
            return Value::String(entry.addr.clone());
        }
    }
    failure("HTTP::server", "Unknown server id")
}
...
fn split_path_query(target: &str) -> (String, HashMap<String, Value>) {
    ...
    let (path, qs) = if let Some(pos) = target.find('?') {
    ...
    for pair in qs.split('&') {
        ...
        if let Some((k, v)) = pair.split_once('=') {
            out.insert(url_decode(k), Value::String(url_decode(v)));
        } else {
            out.insert(url_decode(pair), Value::String(String::new()));
        }
    }
    (path.to_string(), out)
}
...
fn render_response_from_value(v: Value) -> ResponseSpec {
    match v {
        Value::Assoc(m) => {
            ...
            if let Some(Value::Integer(s)) = m.get("status") {
                if *s >= 100 && *s <= 599 {
                    status = *s as u16;
                }
            }
            if let Some(Value::Assoc(h)) = m.get("headers") {
                for (k, vv) in h {
                    if let Value::String(s) | Value::Symbol(s) = vv {
                        headers.insert(k.clone(), s.clone());
                    }
                }
            }
            if let Some(j) = m.get("json") {
                let jv = value_to_json(j);
                if let Ok(s) = serde_json::to_string(&jv) {
                    body = Some(s.into_bytes());
                    headers.entry("Content-Type".into()).or_insert("application/json".into());
                }
            }
            if body.is_none() {
                if let Some(bf) = m.get("bodyFile") {
                    if let Value::String(p) | Value::Symbol(p) = bf {
                        body_file = Some(p.clone());
                        headers
                            .entry("Content-Type".into())
                            .or_insert("application/octet-stream".into());
                    }
                }
            }
                            ...
                            for it in list {
                                match it {
                                    Value::String(s) | Value::Symbol(s) => {
                                        vecs.push(s.clone().into_bytes())
                                    }
                                    Value::List(bytes) => {
                                        let mut out = Vec::with_capacity(bytes.len());
                                        for b in bytes {
                                            if let Value::Integer(i) = b {
                                                out.push(*i as u8);
                                            }
                                        }
                                        vecs.push(out);
                ...
                if let Some(b) = m.get("body") {
                    match b {
                        Value::String(s) | Value::Symbol(s) => {
                            ...
                            headers
                                .entry("Content-Type".into())
                                .or_insert("text/plain; charset=utf-8".into());
                        }
                        Value::List(list) => {
                            let mut tmp = Vec::with_capacity(list.len());
                            for it in list {
                                if let Value::Integer(i) = it {
                                    tmp.push(*i as u8);
                                }
                            }
                            body = Some(tmp);
            ...
            if !headers.contains_key("Content-Type") {
                if let Some(ref body_bytes) = body {
                    if let Ok(s) = std::str::from_utf8(body_bytes) {
                        ...
                        if s_trim.starts_with("<!DOCTYPE html") || s_trim.starts_with("<html") {
                            headers
                                .insert("Content-Type".into(), "text/html; charset=utf-8".into());
                        } else if (s_trim.starts_with('{') || s_trim.starts_with('['))
                            && serde_json::from_str::<serde_json::Value>(s_trim).is_ok()
                        {
        ...
        Value::String(s) | Value::Symbol(s) => ResponseSpec {
            status: 200,
...
fn write_simple_resp(
    ...
    // trait bound already brings Write methods into scope
...
#[cfg(feature = "net_https")]
fn write_simple_resp_tls<S: std::io::Write>(
    stream: &mut S,
    status: u16,
    reason: &str,
    headers: &[(String, String)],
...
fn http_serve_tls(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("HttpServeTls".into())), args };
    }
    let handler = args[0].clone();
    let (opts, tls) = match tls_opts_from(ev, args.get(1).cloned()) {
        Ok(v) => v,
        Err(e) => return failure("HTTP::tls", &e),
    };
    // Load certs/keys
    let certs = match std::fs::File::open(&tls.cert_path).and_then(|f| {
        let mut reader = std::io::BufReader::new(f);
        let certs = rustls_pemfile::certs(&mut reader)?;
        Ok(certs)
    }) {
        Ok(v) => v.into_iter().map(rustls::Certificate).collect::<Vec<_>>(),
        Err(e) => return failure("HTTP::tls", &format!("certs: {}", e)),
    };
    let key = match std::fs::File::open(&tls.key_path).and_then(|f| {
        ...
        keys.into_iter()
            .next()
            .ok_or(std::io::Error::new(std::io::ErrorKind::InvalidData, "no private key"))
    }) {
        Ok(k) => rustls::PrivateKey(k),
        Err(e) => return failure("HTTP::tls", &format!("key: {}", e)),
    };
    ...
    {
        Ok(c) => c,
        Err(e) => return failure("HTTP::tls", &format!("config: {}", e)),
    };
    ...
    let addr_str =
        listener.local_addr().map(|a| a.to_string()).unwrap_or(format!("{}:{}", host, port));
    ...
    std::thread::spawn(move || loop {
        match listener.accept() {
            Ok((stream, _peer)) => {
                ...
                let handler_v = handler.clone();
                std::thread::spawn(move || {
                    let conn = match rustls::ServerConnection::new(acceptor2) {
                        Ok(c) => c,
                        Err(_) => return,
                    };
                    ...
                    use std::io::{Read, Write};
                    if let Some(d) = read_to {
                        let _ = tls_stream.sock.set_read_timeout(Some(d));
                    }
                    if let Some(d) = write_to {
                        let _ = tls_stream.sock.set_write_timeout(Some(d));
                    }
                    let mut buf = Vec::new();
                    let mut tmp = [0u8; 1024];
                    let mut total = 0usize;
                    let mut header = Vec::new();
                    let mut body = Vec::new();
                    let mut body_len_opt = None;
                    loop {
                        match tls_stream.read(&mut tmp) {
                            Ok(0) => break,
                            Ok(n) => {
                                buf.extend_from_slice(&tmp[..n]);
                                total += n;
                    ...
                    if parse_request_has_chunked(&headers_vec) {
                        match read_chunked(&mut tls_stream, &body, max_body) {
                            Ok(data) => body = data,
                            Err(_) => {}
                        }
                    } else if let Some(blen) = body_len_opt {
...
fn path_match(pattern: &str, path: &str) -> Option<HashMap<String, String>> {
    ...
    while i < pseg.len() && j < aseg.len() {
        ...
        if ps == "*" {
            ...
            return Some(params);
        } else if let Some(name) = ps.strip_prefix(':') {
            params.insert(name.to_string(), url_decode(aseg[j]));
            i += 1;
            j += 1;
        } else if ps == aseg[j] {
            i += 1;
    ...
    if i == pseg.len() && j == aseg.len() {
        Some(params)
    } else if i + 1 == pseg.len() && pseg[i] == "*" {
        params.insert("splat".into(), String::new());
        Some(params)
    } else {
        None
    }
...
fn path_match_builtin(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 2 {
        return Value::Expr { head: Box::new(Value::Symbol("PathMatch".into())), args };
    }
    let pat = to_string_arg(ev, args[0].clone());
    let path = to_string_arg(ev, args[1].clone());
    match path_match(&pat, &path) {
        Some(params) => Value::Assoc(HashMap::from([
            (String::from("ok"), Value::Boolean(true)),
            (
                String::from("params"),
                Value::Assoc(params.into_iter().map(|(k, v)| (k, Value::String(v))).collect()),
            ),
        ])),
...
fn respond_file(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("RespondFile".into())), args };
    }
    ...
    if let Some(Value::Assoc(m)) = args
        .get(1)
        .and_then(|a| Some(ev.eval(a.clone())))
        .and_then(|v| if let Value::Assoc(m) = v { Some(Value::Assoc(m)) } else { None })
    {
        if let Some(Value::Boolean(b)) = m.get("Attachment") {
            attach = *b;
        }
        if let Some(Value::String(s)) | Some(Value::Symbol(s)) = m.get("Filename") {
            filename = Some(s.clone());
        }
        if let Some(Value::String(s)) | Some(Value::Symbol(s)) = m.get("ContentType") {
            ctype = Some(s.clone());
        }
        if let Some(Value::Integer(s)) = m.get("Status") {
            status = *s;
...
fn respond_text(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("RespondText".into())), args };
    }
    ...
    headers
        .entry("Content-Type".into())
        .or_insert(Value::String(ctype.unwrap_or_else(|| "text/plain; charset=utf-8".into())));
    Value::Assoc(
        vec![
            ("status".into(), Value::Integer(status)),
            ("headers".into(), Value::Assoc(headers)),
...
fn respond_json(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("RespondJson".into())), args };
    }
    ...
    let body = if pretty {
        serde_json::to_string_pretty(&j).unwrap_or_else(|_| String::from("null"))
    } else {
        serde_json::to_string(&j).unwrap_or_else(|_| String::from("null"))
    };
    Value::Assoc(
        vec![
            ("status".into(), Value::Integer(status)),
...
fn http_serve_routes(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("HttpServeRoutes".into())), args };
    }
    ...
    if let Value::Assoc(m) = routes_val {
        for (k, v) in m {
            // k like "GET /path"
            let ks = k.trim();
            let (method, pat) = if let Some(space) = ks.find(' ') {
                (ks[..space].to_string(), ks[space + 1..].to_string())
            } else {
                ("GET".into(), ks.to_string())
            };
        ...
        return failure(
            "HTTP::routes",
            "Routes must be an association mapping 'METHOD path' -> handler",
        );
    }
    // Build a handler that dispatches
    let _handler =
        Value::Expr { head: Box::new(Value::Symbol("RoutesHandler".into())), args: vec![] };
                    ...
                    std::thread::spawn(move || {
                        ...
                        for (m, pat, hv) in routes2.iter() {
                            if m.eq_ignore_ascii_case(&method) {
                                if let Some(params) = path_match(pat, &path) {
                                    chosen = Some((hv.clone(), params));
                                    break;
                                }
                            }
                        }
                        ...
                        let req_val = Value::Assoc(req_map);
                        let out = if let Some((hv, _)) = chosen {
                            let mut evh = lyra_runtime::Evaluator::new();
                            crate::register_all(&mut evh);
                            let call = Value::Expr { head: Box::new(hv), args: vec![req_val] };
                            evh.eval(call)
                        } else {
                            Value::assoc(vec![
                                ("status", Value::Integer(404)),
                                ("body", Value::String(String::from("Not Found"))),
                            ])
                        };
                        let spec = render_response_from_value(out);
                        let status = spec.status;
...
fn cors(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    // Cors[opts, handler]
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("Cors".into())), args };
    }
    let opts = ev.eval(args[0].clone());
    let handler = args[1].clone();
    // Build PureFunction req -> CorsApply[opts, handler, req]
    let body = Value::Expr {
        head: Box::new(Value::Symbol("CorsApply".into())),
        args: vec![opts, handler, Value::Symbol("req".into())],
    };
...
fn cors_apply(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 3 {
        return Value::Expr { head: Box::new(Value::Symbol("CorsApply".into())), args };
    }
    ...
    let allow_methods = opts
        .get("AllowMethods")
        .and_then(|v| if let Value::String(s) = v { Some(s.clone()) } else { None })
        .unwrap_or("GET,POST,PUT,PATCH,DELETE,OPTIONS".into());
    let allow_headers = opts
        .get("AllowHeaders")
        .and_then(|v| if let Value::String(s) = v { Some(s.clone()) } else { None })
        .unwrap_or("*".into());
    let expose_headers = opts.get("ExposeHeaders").and_then(|v| {
    ...
    if method.eq_ignore_ascii_case("OPTIONS") {
        let mut headers: std::collections::HashMap<String, Value> =
            std::collections::HashMap::new();
        ...
        if let Some(eh) = expose_headers {
            headers.insert("Access-Control-Expose-Headers".into(), Value::String(eh));
        }
        if allow_creds {
            headers.insert("Access-Control-Allow-Credentials".into(), Value::String("true".into()));
        }
        return Value::Assoc(
            vec![
    ...
    if let Value::Assoc(m) = &mut resp {
        let hdrs =
            m.entry("headers".into()).or_insert(Value::Assoc(std::collections::HashMap::new()));
        if let Value::Assoc(hm) = hdrs {
            ...
            if let Some(eh) = expose_headers {
                hm.insert("Access-Control-Expose-Headers".into(), Value::String(eh));
            }
            if allow_creds {
                hm.insert("Access-Control-Allow-Credentials".into(), Value::String("true".into()));
            }
        }
    }
    resp
...
fn auth_jwt(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("AuthJwt".into())), args };
    }
    let opts = ev.eval(args[0].clone());
    let handler = args[1].clone();
    let body = Value::Expr {
        head: Box::new(Value::Symbol("AuthJwtApply".into())),
        args: vec![opts, handler, Value::Symbol("req".into())],
    };
    Value::PureFunction { params: Some(vec!["req".into()]), body: Box::new(body) }
}
...
fn auth_jwt_apply(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 3 {
        return Value::Expr { head: Box::new(Value::Symbol("AuthJwtApply".into())), args };
    }
    let opts = match ev.eval(args[0].clone()) {
        Value::Assoc(m) => m,
        _ => std::collections::HashMap::new(),
    ...
    let bearer = match &req {
        Value::Assoc(m) => {
            if let Some(Value::Assoc(h)) = m.get("headers") {
                if let Some(Value::String(auth)) =
                    h.get("Authorization").or_else(|| h.get("authorization")).cloned()
                {
                    let s = auth.trim();
    ...
    if bearer.is_none() {
        return Value::Assoc(
            vec![
                ("status".into(), Value::Integer(401)),
                ("body".into(), Value::String(String::from("Unauthorized"))),
            ]
            .into_iter()
            .collect(),
        );
    ...
    let mut verify_args =
        vec![Value::String(token), Value::List(vec![Value::String(secret.unwrap_or_default())])];
    let mut verify_opts = std::collections::HashMap::new();
    if let Some(aud) = audience {
        verify_opts.insert("Audience".into(), Value::String(aud));
    }
    if let Some(iss) = issuer {
        verify_opts.insert("Issuer".into(), Value::String(iss));
    }
    verify_args.push(Value::Assoc(verify_opts));
    let ver = ev
        .eval(Value::Expr { head: Box::new(Value::Symbol("JwtVerify".into())), args: verify_args });
    if let Value::Assoc(m) = &ver {
        if let Some(Value::Boolean(true)) = m.get("valid") {
            // Inject claims into req as req.auth.claims
            let mut reqm = match req {
                Value::Assoc(m) => m,
                _ => std::collections::HashMap::new(),
            };
            let mut auth_map = std::collections::HashMap::new();
            if let Some(claims) = m.get("claims") {
                auth_map.insert("claims".into(), claims.clone());
            }
            reqm.insert("auth".into(), Value::Assoc(auth_map));
            return ev
                .eval(Value::Expr { head: Box::new(handler), args: vec![Value::Assoc(reqm)] });
        }
    }
    Value::Assoc(
        vec![
...
fn openapi_generate(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("OpenApiGenerate".into())), args };
    }
    let routes_val = ev.eval(args[0].clone());
    let info = if args.len() >= 2 {
        ev.eval(args[1].clone())
    } else {
    ...
    if let Value::Assoc(m) = routes_val {
        for (k, v) in m.iter() {
            let ks = k.trim();
            let (method, pat) = if let Some(space) = ks.find(' ') {
                (ks[..space].to_lowercase(), ks[space + 1..].to_string())
            } else {
                ("get".into(), ks.to_string())
            };
            // Support richer spec: <|"Handler"->..., "Summary"->..., "Input"->schema, "Output"->schema, "Tags"->{}|>
            let (summary, tags, _input_schema, output_schema) = match v {
                Value::Assoc(hm) => (
                    hm.get("Summary")
                        .and_then(|x| if let Value::String(s) = x { Some(s.clone()) } else { None })
                        .unwrap_or_default(),
                    hm.get("Tags")
                        .and_then(|x| if let Value::List(xs) = x { Some(xs.clone()) } else { None })
                    ...
                    (
                        ...
                        Value::Assoc(
                            vec![(
                                ...
                                Value::Assoc(
                                    vec![
                                        ("description".into(), Value::String(String::from("OK"))),
                                        (
                                            "content".into(),
                                            Value::Assoc(
                                                vec![(
                                                    "application/json".into(),
                                                    Value::Assoc(match output_schema {
                                                        Some(s) => std::collections::HashMap::from(
                                                            [(String::from("schema"), s)],
                                                        ),
            ...
            let path_item =
                paths.entry(pat).or_insert(Value::Assoc(std::collections::HashMap::new()));
            if let Value::Assoc(pm) = path_item {
                pm.insert(method, op);
            }
        }
    }
...
fn write_chunked_resp<W: std::io::Write>(
    mut stream: W,
    status: u16,
    reason: &str,
    mut headers: Vec<(String, String)>,
    mut chunks: Box<dyn Iterator<Item = Vec<u8>>>,
) -> std::io::Result<()> {
    let mut head = format!(
        "HTTP/1.1 {} {}
",
...
",
    ...
    std::io::Write::write_all(&mut stream, head.as_bytes())?;
...
impl FileChunkIter {
    fn new(path: &str) -> Self {
        ...
        Self { file: f, bufsize: 32 * 1024 }
    }
}
impl Iterator for FileChunkIter {
    type Item = Vec<u8>;
...
#[cfg(feature = "net_https")]
fn write_chunked_resp_tls<W: std::io::Write>(
    mut stream: W,
    status: u16,
    reason: &str,
    mut headers: Vec<(String, String)>,
...
struct ChannelChunkIter {
    ev: lyra_runtime::Evaluator,
    chan: Value,
}
impl ChannelChunkIter {
    fn new(id: i64) -> Self {
        let mut ev = lyra_runtime::Evaluator::new();
        crate::register_all(&mut ev);
        let chan = Value::Expr {
...
impl Iterator for ChannelChunkIter {
    type Item = Vec<u8>;
    fn next(&mut self) -> Option<Self::Item> {
        let recv = Value::Expr {
            head: Box::new(Value::Symbol("Receive".into())),
            args: vec![self.chan.clone()],
        };
        let v = self.ev.eval(recv);
        match v {
            Value::Symbol(s) if s == "Null" => None,
            Value::String(s) | Value::Symbol(s) => Some(s.into_bytes()),
            Value::List(bytes) => {
                let mut out = Vec::with_capacity(bytes.len());
                for b in bytes {
                    if let Value::Integer(i) = b {
...
fn respond_bytes(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("RespondBytes".into())), args };
    }
    ...
    match data_v {
        ...
        other => {
            let s = lyra_core::pretty::format_value(&other);
            bytes = s.into_bytes();
        }
    }
    let mut headers: std::collections::HashMap<String, Value> = std::collections::HashMap::new();
    ...
    headers
        .entry("Content-Type".into())
        .or_insert(Value::String(ctype.unwrap_or_else(|| "application/octet-stream".into())));
    Value::Assoc(
        vec![
            ("status".into(), Value::Integer(status)),
            ("headers".into(), Value::Assoc(headers)),
...
fn respond_html(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("RespondHtml".into())), args };
    }
    ...
    headers
        .entry("Content-Type".into())
        .or_insert(Value::String(ctype.unwrap_or_else(|| "text/html; charset=utf-8".into())));
    Value::Assoc(
        vec![
            ("status".into(), Value::Integer(status)),
            ("headers".into(), Value::Assoc(headers)),
...
fn respond_redirect(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("RespondRedirect".into())), args };
    }
    ...
    Value::Assoc(
        vec![("status".into(), Value::Integer(status)), ("headers".into(), Value::Assoc(headers))]
            .into_iter()
            .collect(),
    )
}
...
fn cookies_header(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("CookiesHeader".into())), args };
    }
...
fn get_response_cookies(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("GetResponseCookies".into())), args };
    }
        ...
        if let Some(Value::List(items)) = m.get("headersList") {
            for it in items {
                if let Value::Assoc(h) = it {
                    let name_ok = h
                        ...
                        .map(|s| s.to_ascii_lowercase() == "set-cookie")
                        .unwrap_or(false);
                    if name_ok {
                        if let Some(Value::String(line)) | Some(Value::Symbol(line)) =
                            h.get("value")
                        {
                            out.push(parse_set_cookie_line(&line));
                        }
                    }
                }
            }
            ...
            for (k, v) in hs {
                if k.eq_ignore_ascii_case("set-cookie") {
                    if let Value::String(line) | Value::Symbol(line) = v {
                        out.push(parse_set_cookie_line(&line));
                    }
                }
            }
...
fn parse_set_cookie_line(line: &str) -> Value {
    ...
    let (name, value) = if let Some((n, v)) = nv.split_once('=') {
        (n.trim().to_string(), v.trim().to_string())
    } else {
        (nv.trim().to_string(), String::new())
    };
    ...
    for a in parts {
        ...
        if let Some((k, v)) = a.split_once('=') {
            attrs.insert(k.trim().to_string(), Value::String(v.trim().to_string()));
        } else {
            attrs.insert(a.to_string(), Value::Boolean(true));
        }
    }
    Value::Assoc(
        vec![
