...
fn fuzzy_find_in_text(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 { return Value::Expr { head: Box::new(Value::Symbol("FuzzyFindInText".into())), args } }
    let text = match ev.eval(args[0].clone()) { Value::String(s)=>s, v=> lyra_core::pretty::format_value(&v) };
    let needle = match ev.eval(args[1].clone()) { Value::String(s)=>s, v=> lyra_core::pretty::format_value(&v) };
    ...
    for (idx, line) in text.lines().enumerate() {
        if let Some((score, indices)) = matcher.fuzzy_indices(line, &needle) {
            out.push(Value::Assoc([
                ...
                ("indices".into(), Value::List(indices.into_iter().map(|i| Value::Integer(i as i64)).collect())),
...
fn fuzzy_find_in_list(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 { return Value::Expr { head: Box::new(Value::Symbol("FuzzyFindInList".into())), args } }
    let items = match ev.eval(args[0].clone()) { Value::List(vs)=>vs, v=> return Value::Expr { head: Box::new(Value::Symbol("FuzzyFindInList".into())), args: vec![v] } };
    let needle = match ev.eval(args[1].clone()) { Value::String(s)=>s, v=> lyra_core::pretty::format_value(&v) };
    let opts = args.get(2).map(|v| ev.eval(v.clone())).and_then(|v| match v { Value::Assoc(m)=>Some(m), _=>None }).unwrap_or_default();
    let matcher = matcher_from_opts(&opts);
    let mut out: Vec<(i64, Value)> = Vec::new();
    for it in items {
        let s = match it { Value::String(s)=>s, v=> lyra_core::pretty::format_value(&v) };
...
fn fuzzy_find_in_files(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 { return Value::Expr { head: Box::new(Value::Symbol("FuzzyFindInFiles".into())), args } }
    let input = match super::text::parse_input(ev, args[0].clone()) { Ok(i)=>i, Err(e)=> return super::text::failure("Fuzzy::input", &e) };
    let needle = match ev.eval(args[1].clone()) { Value::String(s)=>s, v=> lyra_core::pretty::format_value(&v) };
    ...
    match input {
        super::text::InputKind::Text(s) => {
            for (lnum, line) in s.lines().enumerate() {
                if let Some((score, indices)) = matcher.fuzzy_indices(line, &needle) {
                    out.push((score as i64, Value::Assoc([
                        ("value".into(), Value::String(line.to_string())),
