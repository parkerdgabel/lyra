        ...
        Value::Expr { head, args } => {
            if let Value::Symbol(name) = &**head {
                match (name.as_str(), args.as_slice()) {
                    ("Plus", list) => {
                        let mut acc = 0.0;
                        for a in list { acc += to_f64_scalar(a)?; }
                        Some(acc)
                    }
                    ("Times", list) => {
                        let mut acc = 1.0;
...
fn floor_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        ...
        other => Value::Expr { head: Box::new(Value::Symbol("Floor".into())), args: other.to_vec() },
    }
}
...
fn ceiling_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        ...
        other => Value::Expr { head: Box::new(Value::Symbol("Ceiling".into())), args: other.to_vec() },
    }
}
...
fn round_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        ...
        other => Value::Expr { head: Box::new(Value::Symbol("Round".into())), args: other.to_vec() },
    }
}
...
fn trunc_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        ...
        other => Value::Expr { head: Box::new(Value::Symbol("Trunc".into())), args: other.to_vec() },
    }
}
...
fn mod_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        [Value::Integer(a), Value::Integer(b)] => if *b == 0 { Value::Expr { head: Box::new(Value::Symbol("Mod".into())), args: vec![Value::Integer(*a), Value::Integer(*b)] } } else { Value::Integer(a.rem_euclid(*b)) },
        [a, b] => match (to_f64_scalar(a), to_f64_scalar(b)) {
            (Some(x), Some(y)) if y != 0.0 => Value::Real(x.rem_euclid(y)),
            _ => Value::Expr { head: Box::new(Value::Symbol("Mod".into())), args: vec![a.clone(), b.clone()] },
        },
...
fn quotient_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        [Value::Integer(a), Value::Integer(b)] => if *b == 0 { Value::Expr { head: Box::new(Value::Symbol("Quotient".into())), args: vec![Value::Integer(*a), Value::Integer(*b)] } } else { Value::Integer(floor_div(*a, *b)) },
        [a, b] => match (to_f64_scalar(a), to_f64_scalar(b)) {
            (Some(x), Some(y)) if y != 0.0 => Value::Integer((x / y).floor() as i64),
            _ => Value::Expr { head: Box::new(Value::Symbol("Quotient".into())), args: vec![a.clone(), b.clone()] },
        },
        other => Value::Expr { head: Box::new(Value::Symbol("Quotient".into())), args: other.to_vec() },
    }
}

fn remainder_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        [Value::Integer(a), Value::Integer(b)] => if *b == 0 { Value::Expr { head: Box::new(Value::Symbol("Remainder".into())), args: vec![Value::Integer(*a), Value::Integer(*b)] } } else { let q = floor_div(*a, *b); Value::Integer(a - b * q) },
        [a, b] => match (to_f64_scalar(a), to_f64_scalar(b)) {
            (Some(x), Some(y)) if y != 0.0 => { let q = (x / y).floor(); Value::Real(x - y * q) }
            _ => Value::Expr { head: Box::new(Value::Symbol("Remainder".into())), args: vec![a.clone(), b.clone()] },
        },
        other => Value::Expr { head: Box::new(Value::Symbol("Remainder".into())), args: other.to_vec() },
    }
}

fn divmod_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        [Value::Integer(a), Value::Integer(b)] => if *b == 0 { Value::Expr { head: Box::new(Value::Symbol("DivMod".into())), args: vec![Value::Integer(*a), Value::Integer(*b)] } } else { let q = floor_div(*a, *b); let r = a - b * q; Value::List(vec![Value::Integer(q), Value::Integer(r)]) },
        [a, b] => match (to_f64_scalar(a), to_f64_scalar(b)) {
            (Some(x), Some(y)) if y != 0.0 => { let q = (x / y).floor(); let r = x - y*q; Value::List(vec![Value::Integer(q as i64), Value::Real(r)]) },
            _ => Value::Expr { head: Box::new(Value::Symbol("DivMod".into())), args: vec![a.clone(), b.clone()] },
        },
        other => Value::Expr { head: Box::new(Value::Symbol("DivMod".into())), args: other.to_vec() },
    }
}

fn sqrt_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    fn isqrt(n: i64) -> Option<i64> { if n < 0 { return None; } let r = (n as f64).sqrt() as i64; if r*r == n { Some(r) } else if (r+1)*(r+1) == n { Some(r+1) } else { None } }
    match args.as_slice() {
        [Value::Integer(n)] => {
            if *n < 0 { Value::Expr { head: Box::new(Value::Symbol("Sqrt".into())), args: vec![Value::Integer(*n)] } }
            else if let Some(r) = isqrt(*n) { Value::Integer(r) } else { Value::Real((*n as f64).sqrt()) }
        }
        [Value::Rational { num, den }] => {
            if *num < 0 { Value::Expr { head: Box::new(Value::Symbol("Sqrt".into())), args: vec![Value::Rational{ num:*num, den:*den }] } }
            else {
                let nr = isqrt(*num);
                let dr = isqrt(*den);
                match (nr, dr) {
...
fn log_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        ...
        [v, base] => match (to_f64_scalar(v), to_f64_scalar(base)) {
            ...
            _ => Value::Expr { head: Box::new(Value::Symbol("Log".into())), args: vec![v.clone(), base.clone()] },
        },
...
fn asin_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        [v] => match to_f64_scalar(v) { Some(x) => Value::Real(x.asin()), _ => map_unary_packed("ASin", v.clone(), |x| x.asin()) },
...
fn acos_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        [v] => match to_f64_scalar(v) { Some(x) => Value::Real(x.acos()), _ => map_unary_packed("ACos", v.clone(), |x| x.acos()) },
...
fn atan_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        [v] => match to_f64_scalar(v) { Some(x) => Value::Real(x.atan()), _ => map_unary_packed("ATan", v.clone(), |x| x.atan()) },
...
fn atan2_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        [y, x] => match (to_f64_scalar(y), to_f64_scalar(x)) { (Some(a), Some(b)) => Value::Real(a.atan2(b)), _ => Value::Expr { head: Box::new(Value::Symbol("ATan2".into())), args: vec![y.clone(), x.clone()] } },
        other => Value::Expr { head: Box::new(Value::Symbol("ATan2".into())), args: other.to_vec() },
    }
}
...
fn nthroot_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        [Value::Integer(x), Value::Integer(n)] if *n > 0 => {
            let n_u = *n as u32;
            if *x == 0 { return Value::Integer(0); }
            if *x > 0 {
                // exact check for perfect power
                let xr = (*x as f64).powf(1.0 / (*n as f64));
                let r = xr.round() as i64;
                let mut acc: i128 = 1;
                for _ in 0..n_u { acc = acc.saturating_mul(r as i128); }
                if acc == *x as i128 { Value::Integer(r) } else { Value::Real((*x as f64).powf(1.0/(*n as f64))) }
            } else {
                if n % 2 != 0 { // odd root of negative is negative root of abs
                    let xr = (-*x as f64).powf(1.0/(*n as f64));
                    Value::Real(-xr)
                } else {
                    Value::Expr { head: Box::new(Value::Symbol("NthRoot".into())), args: vec![Value::Integer(*x), Value::Integer(*n)] }
                }
            }
        }
        [a, b] => match (to_f64_scalar(a), to_f64_scalar(b)) {
            (Some(x), Some(n)) if n != 0.0 => Value::Real(x.powf(1.0/n)),
            _ => Value::Expr { head: Box::new(Value::Symbol("NthRoot".into())), args: vec![a.clone(), b.clone()] },
        },
        other => Value::Expr { head: Box::new(Value::Symbol("NthRoot".into())), args: other.to_vec() },
    }
}
...
fn total_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() == 1 {
        match ev.eval(args[0].clone()) {
            Value::List(items) => {
                if items.is_empty() { return Value::Integer(0); }
                let mut acc = items[0].clone();
                for a in items.into_iter().skip(1) {
                    match add_numeric(acc.clone(), a.clone()) { Some(v)=>acc=v, None=> return Value::Expr { head: Box::new(Value::Symbol("Total".into())), args: vec![Value::List(vec![])] } }
                }
                acc
            }
            other => other,
...
fn mean_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    let (items, n) = if args.len() == 1 {
        match ev.eval(args[0].clone()) {
            Value::List(xs) => { let n = xs.len(); (xs, n) },
            other => (vec![other], 1)
        }
    } else {
        let n = args.len();
    ...
    if n == 0 { return Value::Expr { head: Box::new(Value::Symbol("Mean".into())), args: vec![Value::List(vec![])] }; }
    let sum = total_fn(ev, vec![Value::List(items)]);
    div_numeric(sum, Value::Integer(n as i64)).unwrap_or(Value::Expr { head: Box::new(Value::Symbol("Mean".into())), args: vec![] })
}
...
fn median_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if items.is_empty() { return Value::Expr { head: Box::new(Value::Symbol("Median".into())), args: vec![Value::List(vec![])] }; }
    let mut nums: Vec<(bool, f64, Value)> = Vec::with_capacity(items.len());
    for v in items {
        if let Some(x) = to_f64_scalar(&v) { nums.push((matches!(v, Value::Integer(_) | Value::Rational{..}), x, v)); }
        else { return Value::Expr { head: Box::new(Value::Symbol("Median".into())), args: vec![] }; }
    }
    nums.sort_by(|a,b| a.1.partial_cmp(&b.1).unwrap());
    let n = nums.len();
    if n % 2 == 1 { nums[n/2].2.clone() }
    else {
        // average of two middles; try exact if both are exact ints/rats
        let a = &nums[n/2 - 1].2; let b = &nums[n/2].2;
        match add_numeric(a.clone(), b.clone())
            .and_then(|s| div_numeric(s, Value::Integer(2))) {
            Some(v) => v,
...
fn variance_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if items.is_empty() { return Value::Expr { head: Box::new(Value::Symbol("Variance".into())), args: vec![Value::List(vec![])] }; }
    let mut vals: Vec<f64> = Vec::with_capacity(items.len());
    for v in items { if let Some(x) = to_f64_scalar(&v) { vals.push(x); } else { return Value::Expr { head: Box::new(Value::Symbol("Variance".into())), args: vec![] }; } }
    let n = vals.len() as f64;
    let mean = vals.iter().copied().sum::<f64>() / n;
    let var = vals.iter().map(|x| { let d = *x - mean; d*d }).sum::<f64>() / n; // population variance
    Value::Real(var)
}
...
fn gcd_i64(mut a: i64, mut b: i64) -> i64 { if a==0 { return b.abs(); } if b==0 { return a.abs(); } a = a.abs(); b = b.abs(); while b!=0 { let t = b; b = a % b; a = t; } a }
fn lcm_i64(a: i64, b: i64) -> i64 { if a==0 || b==0 { 0 } else { (a / gcd_i64(a,b)).saturating_mul(b).abs() } }

fn gcd_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() { return Value::Integer(0); }
    let mut acc: Option<i64> = None;
    for v in args {
        match v { Value::Integer(n) => { acc = Some(match acc { None => n.abs(), Some(a)=> gcd_i64(a, n) }); }, _ => return Value::Expr { head: Box::new(Value::Symbol("GCD".into())), args: vec![] } }
    }
...
fn lcm_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    let mut acc: Option<i64> = None;
    for v in args {
        match v { Value::Integer(n) => { acc = Some(match acc { None => n.abs(), Some(a)=> lcm_i64(a, n) }); }, _ => return Value::Expr { head: Box::new(Value::Symbol("LCM".into())), args: vec![] } }
    }
    ...
    match args.as_slice() {
        [Value::Integer(n)] if *n >= 0 => {
            ...
            for k in 2..=(*n as i128) {
                ...
                if acc > i64::MAX as i128 { return Value::Expr { head: Box::new(Value::Symbol("Factorial".into())), args: vec![Value::Integer(*n)] }; }
            }
            Value::Integer(acc as i64)
        }
        [v] => Value::Expr { head: Box::new(Value::Symbol("Factorial".into())), args: vec![v.clone()] },
        other => Value::Expr { head: Box::new(Value::Symbol("Factorial".into())), args: other.to_vec() },
    }
}
    ...
    match args.as_slice() {
        [Value::Integer(n), Value::Integer(k)] if *n >= 0 && *k >= 0 && *k <= *n => {
            ...
            for i in 1..=k {
                ...
                num /= g; den /= g;
                if num > i64::MAX as i128 { return Value::Expr { head: Box::new(Value::Symbol("Binomial".into())), args: vec![Value::Integer(n0), Value::Integer(k0)] }; }
            }
            if den != 1 { let g = gcd_i128(num, den); return rat_value((num/g) as i64, (den/g) as i64); }
            Value::Integer(num as i64)
        }
        [a,b] => Value::Expr { head: Box::new(Value::Symbol("Binomial".into())), args: vec![a.clone(), b.clone()] },
        other => Value::Expr { head: Box::new(Value::Symbol("Binomial".into())), args: other.to_vec() },
    }
}

fn gcd_i128(mut a: i128, mut b: i128) -> i128 { a = a.abs(); b = b.abs(); while b != 0 { let t = b; b = a % b; a = t; } a }

fn to_degrees_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        [v] => match to_f64_scalar(v) { Some(x) => Value::Real(x * 180.0 / std::f64::consts::PI), _ => map_unary_packed("ToDegrees", v.clone(), |x| x * 180.0 / std::f64::consts::PI) },
        other => Value::Expr { head: Box::new(Value::Symbol("ToDegrees".into())), args: other.to_vec() },
    }
}

fn to_radians_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        [v] => match to_f64_scalar(v) { Some(x) => Value::Real(x * std::f64::consts::PI / 180.0), _ => map_unary_packed("ToRadians", v.clone(), |x| x * std::f64::consts::PI / 180.0) },
        other => Value::Expr { head: Box::new(Value::Symbol("ToRadians".into())), args: other.to_vec() },
    }
}
    ...
    match args.as_slice() {
        [x, Value::List(bounds)] if bounds.len()==2 => {
            ...
            match (to_f64_scalar(x), min_v, max_v) {
                ...
                _ => Value::Expr { head: Box::new(Value::Symbol("Clip".into())), args: vec![x.clone(), Value::List(bounds.clone())] },
            }
        }
...
fn signum_fn(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    match args.as_slice() {
        ...
        [v] => match to_f64_scalar(v) {
            Some(x) => Value::Integer(if x > 0.0 { 1 } else if x < 0.0 { -1 } else { 0 }),
            None => map_unary_packed("Signum", v.clone(), |x| if x>0.0 { 1.0 } else if x<0.0 { -1.0 } else { 0.0 }),
        },
        other => Value::Expr { head: Box::new(Value::Symbol("Signum".into())), args: other.to_vec() },
    }
}
