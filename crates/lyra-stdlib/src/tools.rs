...
fn next_idemp() -> i64 {
    let a = IDEMP_SEQ.get_or_init(|| std::sync::atomic::AtomicI64::new(1));
...
macro_rules! schema_str {
    () => {
        $crate::tools::schema_type_value("string")
    };
}
#[macro_export]
macro_rules! schema_int {
    () => {
        $crate::tools::schema_type_value("integer")
    };
}
#[macro_export]
macro_rules! schema_num {
    () => {
        $crate::tools::schema_type_value("number")
    };
}
#[macro_export]
macro_rules! schema_bool {
    () => {
        $crate::tools::schema_type_value("boolean")
    };
}
#[macro_export]
macro_rules! schema_arr {
    ($items:expr) => {
        $crate::tools::schema_array_value($items)
    };
}
...
fn validate_against_schema(schema: &Value, provided: &Value) -> (bool, Vec<Value>) {
    ...
    fn get_field<'a>(m: &'a HashMap<String, Value>, k: &str) -> Option<&'a Value> {
        m.get(k)
    }
    match (schema, provided) {
        (Value::Assoc(s), Value::Assoc(p)) => {
            // type: object
            if let Some(Value::String(t)) = get_field(s, "type") {
                if t != "object" { /* ignore other root types */ }
            }
            // required
                                    ...
                                    match t.as_str() {
                                        "array" => {
                                            ...
                                            if let Value::List(items) = v {
                                                if let Some(Value::Assoc(item_schema)) =
                                                    pm.get("items")
                                                {
                                                    if let Some(Value::String(it_t)) =
        ...
        (Value::Assoc(s), other) => {
            if let Some(Value::String(t)) = s.get("type") {
                if !check_type(t, other, "$", &mut errors) {
                    ok = false;
                }
            }
        }
...
fn tools_register(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsRegister".into())), args };
    }
    let mut reg = tool_reg().lock().unwrap();
...
fn tools_unregister(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsUnregister".into())), args };
    }
    let id = match &args[0] {
...
fn stdlib_default_specs(ev: &mut Evaluator) -> Vec<Value> {
    ...
    let names: std::collections::HashSet<String> = builtin_cards(ev)
        .into_iter()
        .filter_map(|v| if let Value::Assoc(m) = v { get_str(&m, "name") } else { None })
        .collect();
    let mut specs: Vec<Value> = Vec::new();
    let _add = |name: &str, summary: &str, params: Vec<&str>, tags: Vec<&str>| {
    ...
    if names.contains("StringJoinWith") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringTrimLeft") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringTrimRight") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringTrimPrefix") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringTrimSuffix") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringContains") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringReverse") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringPadLeft") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringPadRight") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringRepeat") {
        specs.push(tool_spec!(
    ...
    if names.contains("EqualsIgnoreCase") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringFromChars") {
        specs.push(tool_spec!(
    ...
    if names.contains("RegexIsMatch") {
        specs.push(tool_spec!(
    ...
    if names.contains("RegexFindAll") {
        specs.push(tool_spec!(
    ...
    if names.contains("RegexReplace") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringFormat") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringFormatMap") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringInterpolate") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringInterpolateWith") {
        specs.push(tool_spec!(
    ...
    if names.contains("TemplateRender") {
        specs.push(tool_spec!(
    ...
    if names.contains("HtmlUnescape") {
        specs.push(tool_spec!(
    ...
    if names.contains("UrlFormEncode") {
        specs.push(tool_spec!(
    ...
    if names.contains("UrlFormDecode") {
        specs.push(tool_spec!(
    ...
    if names.contains("JsonUnescape") {
        specs.push(tool_spec!(
    ...
    if names.contains("StringTruncate") {
        specs.push(tool_spec!(
...
fn tools_list(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() > 1 {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsList".into())), args };
    }
    let filter =
        if args.len() == 1 { ev.eval(args[0].clone()) } else { Value::Assoc(HashMap::new()) };
    ...
    let mut seen: std::collections::HashSet<String> = out
        .iter()
        .filter_map(|v| if let Value::Assoc(m) = v { get_str(m, "id") } else { None })
        .collect();
    ...
    let (eff_filter, caps_filter, tag_filter): (
        ...
        Value::Assoc(f) if !f.is_empty() => (
            match f.get("effects") {
                Some(Value::List(vs)) => {
                    vs.iter().filter_map(value_to_string).map(|s| s.to_lowercase()).collect()
                }
                _ => Default::default(),
            },
            match f.get("capabilities") {
                Some(Value::List(vs)) => {
                    vs.iter().filter_map(value_to_string).map(|s| s.to_lowercase()).collect()
                }
                _ => Default::default(),
        ...
        .filter(|v| {
            if let Value::Assoc(m) = v {
                ...
                if !want_caps.is_empty() {
                    ...
                    if let Some(Value::List(effs_v)) = m.get("effects") {
                        for e in effs_v {
                            if let Some(s) = value_to_string(e) {
                                effs.insert(s.to_lowercase());
                            }
                        }
                    }
                    if !effs.is_subset(want_caps) {
                        return false;
                    }
                }
                ...
                if !eff_filter.is_empty() {
                    ...
                    if let Some(Value::List(effs)) = m.get("effects") {
                        for e in effs {
                            if let Some(s) = value_to_string(e) {
                                have.insert(s.to_lowercase());
                            }
                        }
                    }
                    if !eff_filter.is_subset(&have) {
                        return false;
                    }
                }
                // tags filter
                if !tag_filter.is_empty() {
                    ...
                    if let Some(Value::List(tags)) = m.get("tags") {
                        for t in tags {
                            if let Some(s) = value_to_string(t) {
                                have.insert(s.to_lowercase());
                            }
                        }
                    }
                    if tag_filter.is_disjoint(&have) {
                        return false;
                    }
                }
                true
...
fn tools_set_capabilities(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsSetCapabilities".into())), args };
    }
    let caps: Option<std::collections::HashSet<String>> = match &args[0] {
        Value::List(vs) => {
            Some(vs.iter().filter_map(|v| value_to_string(v)).map(|s| s.to_lowercase()).collect())
        }
        _ => None,
    };
...
fn tools_get_capabilities(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if !args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsGetCapabilities".into())), args };
    }
    let mu = CAPABILITIES.get_or_init(|| Mutex::new(None));
...
fn tools_cards(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() > 2 {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsCards".into())), args };
    }
    ...
    let limit = if args.len() == 2 {
        match ev.eval(args[1].clone()) {
            Value::Integer(i) => i.max(1) as usize,
            _ => 20,
        }
    } else {
        20
    };
    let all = match tools_list(ev, vec![]) {
        Value::List(vs) => vs,
        _ => vec![],
    };
    ...
    let next =
        if end < all.len() { Value::String(end.to_string()) } else { Value::Symbol("Null".into()) };
    Value::Assoc(
        vec![("items".to_string(), Value::List(slice)), ("next_cursor".to_string(), next)]
            .into_iter()
            .collect(),
    )
}
...
fn tools_describe(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsDescribe".into())), args };
    }
    let key = match &args[0] {
        Value::String(s) | Value::Symbol(s) => s.clone(),
        _ => return Value::Assoc(HashMap::new()),
    };
    let reg = tool_reg().lock().unwrap();
...
fn tools_search(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsSearch".into())), args };
    }
    let q = match &args[0] {
        Value::String(s) | Value::Symbol(s) => s.to_lowercase(),
        _ => String::new(),
    };
    let topk = if args.len() >= 2 {
        if let Value::Integer(k) = args[1].clone() {
            k.max(1) as usize
        } else {
            10
        }
    ...
    let mut pool: Vec<Value> = match tools_list(ev, vec![]) {
        Value::List(vs) => vs,
        _ => vec![],
    };
    ...
    for v in pool.drain(..) {
        let mut score: i64 = 0;
        if let Value::Assoc(m) = &v {
            if let Some(s) = get_str(m, "name").or_else(|| get_str(m, "id")) {
                if s.to_lowercase().contains(&q) {
                    score += 5;
                }
            }
            if let Some(s) = get_str(m, "summary") {
                if s.to_lowercase().contains(&q) {
                    score += 3;
                }
            }
            if let Some(Value::List(tags)) = m.get("tags") {
                for t in tags {
                    if let Some(ts) = value_to_string(t) {
                        if ts.to_lowercase().contains(&q) {
                            score += 2;
                        }
                    }
                }
...
fn tools_resolve(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsResolve".into())), args };
    }
    let q = match &args[0] {
        Value::String(s) | Value::Symbol(s) => s.to_lowercase(),
        _ => String::new(),
            ...
            if let Value::Assoc(m) = v {
                if let Some(id) = get_str(&m, "id").or_else(|| get_str(&m, "name")) {
                    let hay = format!(
                        "{} {} {}",
                        id,
                        get_str(&m, "summary").unwrap_or_default(),
                        get_str(&m, "name").unwrap_or_default()
                    )
                    .to_lowercase();
                    let mut score = 0i64;
                    if id.to_lowercase() == q {
                        score += 10;
                    }
                    if hay.contains(&q) {
                        score += 5;
            ...
            .map(|(id, score)| {
                Value::Assoc(
                    vec![
                        ...
                        ("score".to_string(), Value::Integer(score)),
                    ]
                    .into_iter()
                    .collect(),
                )
...
fn tools_invoke(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsInvoke".into())), args };
    }
    let key = match &args[0] {
        Value::String(s) | Value::Symbol(s) => s.clone(),
    ...
    let call_args: Vec<Value> = match &provided {
        ...
        Value::Assoc(m) => {
            ...
            if let Some(Value::List(vs)) = m.get("args") {
                vs.clone()
            } else {
                vec![Value::Assoc(m.clone())]
            }
        }
    ...
    if let Value::Assoc(m) = &opts {
        if let Some(Value::String(s)) = m.get("Cache") {
            if s == "session" {
                cache_enabled = true;
            }
        }
        if let Some(Value::String(s)) = m.get("IdempotencyKey") {
            id_key = Some(s.clone());
        }
    }
    if cache_enabled || id_key.is_some() {
        let mut hasher = DefaultHasher::new();
        if let Some(idk) = id_key.clone() {
            idk.hash(&mut hasher);
        } else {
            name.hash(&mut hasher);
            // Include provided assoc/list as part of key by pretty formatting (session-only stability acceptable)
            let key_v = if let Value::Assoc(_) = provided {
                provided.clone()
            } else {
                Value::List(call_args.clone())
            };
            let s = lyra_core::pretty::format_value(&key_v);
            s.hash(&mut hasher);
        }
        let h = hasher.finish();
        if let Some(v) = tool_cache().lock().unwrap().get(&h) {
            return v.clone();
        }
...
fn tools_dry_run(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsDryRun".into())), args };
    }
    let key = match &args[0] {
        Value::String(s) | Value::Symbol(s) => s.clone(),
        other => match ev.eval(other.clone()) {
    ...
    if let Some(Value::Assoc(m)) = spec_opt {
        if let Some(Value::Assoc(cost)) = m.get("cost_estimate") {
            estimates = Value::Assoc(cost.clone());
        }
        let params: Vec<String> = match m.get("params") {
            Some(Value::List(vs)) => vs.iter().filter_map(value_to_string).collect(),
            _ => vec![],
        };
        let defaults: HashMap<String, Value> = match m.get("param_defaults") {
            Some(Value::Assoc(am)) => am.clone(),
            _ => HashMap::new(),
        };
        if let Value::Assoc(input) = provided.clone() {
            ...
            if let Some(Value::Assoc(schema)) = m.get("input_schema") {
                let (_ok, mut errs) = validate_against_schema(
                    &Value::Assoc(schema.clone()),
                    &Value::Assoc(input.clone()),
                );
                if !_ok {
                    ok = false;
                }
        ...
        } else if let Value::List(vs) = provided {
            normalized = vs;
        }
    } else {
        // Unknown spec; pass through
        normalized = match provided {
            Value::List(vs) => vs,
            Value::Assoc(m) => vec![Value::Assoc(m)],
            v => vec![v],
        };
    }
    let out = Value::Assoc(
...
fn tools_export_openai(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if !args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsExportOpenAI".into())), args };
    }
    let list = match tools_list(ev, vec![]) {
        Value::List(vs) => vs,
        _ => vec![],
    };
    let mut out: Vec<Value> = Vec::new();
...
fn tools_export_bundle(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if !args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("ToolsExportBundle".into())), args };
    }
...
pub fn register_tools_filtered(ev: &mut Evaluator, pred: &dyn Fn(&str) -> bool) {
    ...
    register_if(
        ...
        pred,
        "ToolsExportOpenAI",
        tools_export_openai as NativeFn,
        Attributes::empty(),
    );
    register_if(
        ev,
        pred,
        "ToolsExportBundle",
        tools_export_bundle as NativeFn,
        Attributes::empty(),
    );
    register_if(
        ev,
        ...
        tools_set_capabilities as NativeFn,
        Attributes::LISTABLE,
    );
    register_if(
        ev,
        ...
        tools_get_capabilities as NativeFn,
        Attributes::empty(),
    );
}
