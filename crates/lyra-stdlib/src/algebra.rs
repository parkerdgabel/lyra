...
fn simplify(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 { return Value::expr(Value::symbol("Simplify"), args); }
    simp(args[0].clone())
}
    ...
    match v {
        Value::Expr { head, args } => match *head {
            Value::Symbol(ref s) if s == "Plus" => {
                ...
                for a in args.into_iter().map(simp) {
                    if let Value::Expr { head: h2, args: a2 } = &a {
                        if let Value::Symbol(ref t) = **h2 {
                            if t == "Plus" { flat.extend(a2.clone()); continue; }
                        }
                    }
                ...
                if let Some(n) = acc_num { if !is_zero(&n) { flat.insert(0, n); } }
                match flat.len() {
                    0 => Value::Integer(0),
                    1 => flat.pop().unwrap(),
            ...
            Value::Symbol(ref s) if s == "Times" => {
                ...
                for a in args.into_iter().map(simp) {
                    if let Value::Expr { head: h2, args: a2 } = &a {
                        if let Value::Symbol(ref t) = **h2 {
                            if t == "Times" { flat.extend(a2.clone()); continue; }
                        }
                    }
                    if let Some(n) = try_numeric(&a) {
                        acc_num = match (acc_num, n) {
                ...
                if let Some(n) = acc_num {
                    if is_zero(&n) { return Value::Integer(0); }
                    if !is_one(&n) { flat.insert(0, n); }
                }
                match flat.len() {
                    0 => Value::Integer(1),
                    1 => flat.pop().unwrap(),
...
fn expand(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 { return Value::expr(Value::symbol("Expand"), args); }
    expd(args[0].clone())
}
                ...
                for a in args.into_iter().map(expd) {
                    ...
                    match a {
                        Value::Expr { head: h2, args: a2 } if matches!(*h2, Value::Symbol(ref t) if t=="Plus") => {
                            for t in terms.iter() {
                                for addend in &a2 {
                                    new_terms.push(simp(Value::expr(Value::symbol("Times"), vec![t.clone(), addend.clone()])));
                                }
                            }
                        }
                        other => {
                            for t in terms.iter() {
                                new_terms.push(simp(Value::expr(Value::symbol("Times"), vec![t.clone(), other.clone()])));
                            }
                        }
                    }
                    terms = new_terms;
                }
                if terms.len()==1 { terms.pop().unwrap() } else { Value::expr(Value::symbol("Plus"), terms) }
            }
            Value::Symbol(ref s) if s == "Power" => {
                match args.as_slice() {
                    [base, Value::Integer(n)] if *n >= 0 && *n <= 8 => {
                        ...
                        for _ in 0..*n { res = expd(Value::expr(Value::symbol("Times"), vec![res, base.clone()])); }
                        res
                    }
                    _ => Value::expr(*head, args.into_iter().map(expd).collect()),
                }
            }
            Value::Symbol(ref s) if s == "Plus" => Value::expr(Value::symbol("Plus"), args.into_iter().map(expd).collect()),
            _ => Value::expr(*head, args.into_iter().map(expd).collect()),
        },
        other => other,
    }
...
fn expand_all(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len()!=1 { return Value::expr(Value::symbol("ExpandAll"), args); }
...
fn collect_by(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.is_empty() { return Value::expr(Value::symbol("CollectTermsBy"), args); }
    let expr = args[0].clone();
    let mut vars: Option<Vec<String>> = None;
    let mut order_by: String = "lex".into();
    if args.len() >= 2 {
        if let Value::List(vs) = &args[1] { vars = Some(vs.iter().filter_map(|v| as_symbol_name(v)).collect()); }
    }
    if args.len() >= 3 {
        if let Value::Assoc(m) = &args[2] {
            if let Some(Value::String(s))|Some(Value::Symbol(s)) = m.get("OrderBy") { order_by = s.clone(); }
        }
    }
...
fn collect_mono_with_order(expr: Value, vars: &[String], order_by: &str) -> Value {
    ...
    fn accum(vars: &[String], v: &Value, map: &mut std::collections::BTreeMap<Vec<usize>, i64>) -> bool {
        match v {
            Value::Integer(n) => { add_term(map, *n, vec![0; vars.len()]); true }
            Value::Expr { head, args } if matches!(&**head, Value::Symbol(s) if s=="Plus") => {
                for a in args { if !accum(vars, a, map) { return false; } }
                true
            }
            _ => {
                ...
                let factors: Vec<Value> = match v {
                    Value::Expr { head, args } if matches!(&**head, Value::Symbol(s) if s=="Times") => args.clone(),
                    other => vec![other.clone()],
                };
                for f in factors {
                    match f {
                        Value::Integer(n) => { coeff = coeff.saturating_mul(n); }
                        Value::Symbol(s) => {
                            if let Some(idx) = vars.iter().position(|x| x==&s) { deg[idx] = deg[idx].saturating_add(1); }
                            else { rest.push(Value::Symbol(s)); }
                        }
                        Value::Expr { head, args } => {
                                    ...
                                    if let [base, exp] = args.as_slice() {
                                        if let Value::Symbol(s) = base {
                                            if let Some(idx) = vars.iter().position(|x| x==s) {
                                                if let Value::Integer(n) = exp { if *n>=0 { deg[idx] = deg[idx].saturating_add(*n as usize); continue; } }
                                            }
                                        }
                                    }
                                }
                ...
                if rest.is_empty() { add_term(map, coeff, deg); true }
                else { false }
            }
        }
    }
    if !accum(vars, &expr, &mut map) { return expr; }
    ...
    for (deg, c) in items.into_iter() {
        ...
        for (i, d) in deg.into_iter().enumerate() {
            if d==0 { continue; }
            if d==1 { factors.push(Value::symbol(vars[i].clone())); }
            else { factors.push(Value::expr(Value::symbol("Power"), vec![Value::symbol(vars[i].clone()), Value::Integer(d as i64)])); }
        }
        terms.push(mul_list(factors));
    }
    if terms.is_empty() { Value::Integer(0) } else if terms.len()==1 { terms.pop().unwrap() } else { Value::expr(Value::symbol("Plus"), terms) }
}

fn collect_symbols(v: &Value, out: &mut std::collections::BTreeSet<String>) {
    match v {
        Value::Symbol(s) => { out.insert(s.clone()); }
        Value::Expr { head, args } => {
...
fn cancel(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    // Cancel[(num)/(den), x?]
    if args.is_empty() { return Value::expr(Value::symbol("Cancel"), args); }
    let (num, den) = match &args[0] {
        Value::Expr { head, args: ab } if matches!(**head, Value::Symbol(ref s) if s=="Divide") && ab.len()==2 => (ab[0].clone(), ab[1].clone()),
    ...
    'outer: for n in nf.iter() {
        for (j, d) in df.iter().enumerate() {
            if !used[j] && n == d { used[j]=true; continue 'outer; }
        }
        nf2.push(n.clone());
    }
    for (j, d) in df.iter().enumerate() { if !used[j] { df2.push(d.clone()); } }
    let num_r = mul_list(nf2);
    let den_r = mul_list(df2);
    if is_one(&den_r) { simp(num_r) } else { Value::expr(Value::symbol("Divide"), vec![simp(num_r), simp(den_r)]) }
}

fn factor_list(v: Value, var: Option<&str>) -> Vec<Value> {
    // Use our basic Poly factoring (content and quadratic) and flatten Times
    let fv = match var { Some(_) => v.clone(), None => v.clone() };
...
fn flatten_times(v: &Value, out: &mut Vec<Value>) {
    match v {
        Value::Expr { head, args } if matches!(**head, Value::Symbol(ref s) if s=="Times") => {
            for a in args { flatten_times(a, out); }
        }
...
fn factor_basic_with_var(v: Value, var: Option<&str>) -> Value {
    ...
    if let Some(mut p) = Poly::from_expr(&v, var) {
        p.factor_content();
        if let Some((p1,p2)) = p.factor_quadratic() { return mul_list(vec![p1.to_expr(), p2.to_expr()]); }
        return p.to_expr();
    }
    v
}
...
fn apart(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len()<1 { return Value::expr(Value::symbol("Apart"), args); }
    let var = args.get(1).and_then(|v| as_symbol_name(v)).unwrap_or_else(|| "x".into());
    let (num, den) = match &args[0] {
        Value::Expr { head, args: ab } if matches!(**head, Value::Symbol(ref s) if s=="Divide") && ab.len()==2 => (ab[0].clone(), ab[1].clone()),
        other => return other.clone(),
    };
    let cnum = match num { Value::Integer(n) => n, _ => return Value::expr(Value::symbol("Apart"), args) };
    let mut den_factors = Vec::new();
    flatten_times(&factor_basic_with_var(den, Some(&var)), &mut den_factors);
    if den_factors.len() != 2 { return Value::expr(Value::symbol("Apart"), args); }
    let (a1,b1) = match linear_coeffs(&den_factors[0], &var) { Some(t)=>t, None=> return Value::expr(Value::symbol("Apart"), args) };
    let (a2,b2) = match linear_coeffs(&den_factors[1], &var) { Some(t)=>t, None=> return Value::expr(Value::symbol("Apart"), args) };
    // Solve cnum = A*(a2 x + b2) + B*(a1 x + b1) -> match x and constant terms
    // Coef of x: A*a2 + B*a1 = 0; Constant: A*b2 + B*b1 = cnum
    let det = a1*b2 - a2*b1;
    if det == 0 { return Value::expr(Value::symbol("Apart"), args); }
...
fn linear_coeffs(v: &Value, var: &str) -> Option<(i64, i64)> {
    ...
    match v {
        ...
        Value::Expr { head, args } if matches!(**head, Value::Symbol(ref s) if s=="Plus") => {
            ...
            match (&args[0], &args[1]) {
                (Value::Expr { head: h2, args: a2 }, b) | (b, Value::Expr { head: h2, args: a2 }) if matches!(**h2, Value::Symbol(ref t) if t=="Times") => {
                    ...
                    if !ok { return None; }
                    let bconst = match b { Value::Integer(n) => *n, _ => return None };
                    Some((coef, bconst))
                }
...
fn solve(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len()!=2 { return Value::expr(Value::symbol("Solve"), args); }
    ...
    if var_names.len()!=vars.len() { return Value::expr(Value::symbol("Solve"), args); }
    let n = var_names.len();
    let m = eqs.len();
    if m==0 || n==0 { return Value::List(vec![]); }
    ...
    for (i, e) in eqs.iter().enumerate() {
        if let Value::Expr { head, args } = e {
            if matches!(**head, Value::Symbol(ref s) if s=="Equal") && args.len()==2 {
                ...
                if decompose_linear(&expd, &var_names, &mut coeffs, &mut const_term).is_none() { return Value::expr(Value::symbol("Solve"), args.to_vec()); }
                for j in 0..n { a[i][j] = coeffs[j]; }
                b[i] = -const_term;
                continue;
            }
        }
    ...
    if let Some(xs) = gaussian_elimination(a, b) {
        ...
        for (j, name) in var_names.iter().enumerate() {
            rules.push(Value::expr(Value::symbol("Rule"), vec![Value::symbol(name.clone()), Value::Real(xs[j])]))
        }
        Value::List(rules)
    } else { Value::expr(Value::symbol("Solve"), args) }
}

fn decompose_linear(v: &Value, vars: &[String], out: &mut [f64], c: &mut f64) -> Option<()> {
    match v {
        Value::Integer(n) => { *c += *n as f64; Some(()) }
        Value::Real(x) => { *c += *x; Some(()) }
        Value::Expr { head, args } if matches!(**head, Value::Symbol(ref s) if s=="Minus") => {
            // a - b => a + (-1)*b
            match args.as_slice() {
                [a, b] => { decompose_linear(a, vars, out, c)?; decompose_linear(&Value::expr(Value::symbol("Times"), vec![Value::Integer(-1), b.clone()]), vars, out, c) }
                [a] => decompose_linear(&Value::expr(Value::symbol("Times"), vec![Value::Integer(-1), a.clone()]), vars, out, c),
                _ => None,
            }
        }
        Value::Expr { head, args } if matches!(**head, Value::Symbol(ref s) if s=="Plus") => {
            for a in args { decompose_linear(a, vars, out, c)?; }
            Some(())
        }
        Value::Expr { head, args } if matches!(**head, Value::Symbol(ref s) if s=="Times") => {
            // Expect numeric * var
            match args.as_slice() {
                [Value::Integer(n), Value::Symbol(s)] => { if let Some(idx)=vars.iter().position(|x| x==s) { out[idx]+=*n as f64; return Some(()); } }
                [Value::Real(x), Value::Symbol(s)] => { if let Some(idx)=vars.iter().position(|x| x==s) { out[idx]+=*x; return Some(()); } }
                [Value::Symbol(s), Value::Integer(n)] => { if let Some(idx)=vars.iter().position(|x| x==s) { out[idx]+=*n as f64; return Some(()); } }
                [Value::Symbol(s), Value::Real(x)] => { if let Some(idx)=vars.iter().position(|x| x==s) { out[idx]+=*x; return Some(()); } }
                _ => {}
            }
            None
        }
        Value::Symbol(s) => { if let Some(idx)=vars.iter().position(|x| x==s) { out[idx]+=1.0; Some(()) } else { None } }
        _ => None,
    }
}
...
fn to_add_mul(v: Value) -> Value {
    match v.clone() {
        Value::Expr { head, args } => match *head {
            Value::Symbol(ref s) if s=="Minus" => {
                match args.as_slice() {
                    [a, b] => Value::expr(Value::symbol("Plus"), vec![to_add_mul(a.clone()), Value::expr(Value::symbol("Times"), vec![Value::Integer(-1), to_add_mul(b.clone())])]),
                    [a] => Value::expr(Value::symbol("Times"), vec![Value::Integer(-1), to_add_mul(a.clone())]),
...
fn gaussian_elimination(mut a: Vec<Vec<f64>>, mut b: Vec<f64>) -> Option<Vec<f64>> {
    ...
    for row in 0..n {
        // find pivot
        let mut pivot = row;
        while pivot < m && a[pivot][col].abs() < 1e-12 { pivot += 1; }
        if pivot==m { return None; }
        a.swap(row, pivot); b.swap(row, pivot);
        // normalize
        let div = a[row][col]; if div.abs()<1e-12 { return None; }
        for j in col..n { a[row][j] /= div; }
        b[row] /= div;
        // eliminate others
        for i in 0..m { if i!=row { let f=a[i][col]; if f.abs()>1e-12 { for j in col..n { a[i][j]-=f*a[row][j]; } b[i]-=f*b[row]; }} }
        if col+1 < n { col+=1; } else { break; }
    }
...
fn roots(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    // Roots[poly, x]
    if args.len()!=2 { return Value::expr(Value::symbol("Roots"), args); }
    let var = match as_symbol_name(&args[1]) { Some(s)=>s, None=>return Value::expr(Value::symbol("Roots"), args) };
    let v = to_add_mul(expd(simp(args[0].clone())));
    let Some(p) = Poly::from_expr(&v, Some(&var)) else { return Value::expr(Value::symbol("Roots"), args) };
    ...
    match deg {
        ...
        1 => {
            ...
            if b.abs() < 1e-12 { return Value::List(vec![]); }
        ...
        2 => {
            ...
            if disc >= 0.0 {
                let s = disc.sqrt();
                Value::List(vec![Value::Real((-b - s)/(2.0*a)), Value::Real((-b + s)/(2.0*a))])
            } else {
                // complex roots
                let s = (-disc).sqrt();
                let re = -b/(2.0*a);
                let im = s/(2.0*a);
                Value::List(vec![Value::Complex{re:Box::new(Value::Real(re)), im:Box::new(Value::Real(-im))}, Value::Complex{re:Box::new(Value::Real(re)), im:Box::new(Value::Real(im))}])
            }
        }
...
fn diff(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    // D[expr, x]
    if args.len()!=2 { return Value::expr(Value::symbol("D"), args); }
    let expr = args[0].clone();
    let var = args[1].clone();
    let var_name = match as_symbol_name(&var) { Some(s) => s, None => return Value::expr(Value::symbol("D"), vec![expr, var]) };
    d(expr, &var_name)
}

fn d(expr: Value, var: &str) -> Value {
    match expr {
        Value::Integer(_) | Value::Real(_) | Value::Rational{..} | Value::BigReal(_) => Value::Integer(0),
        Value::Symbol(ref s) if s == var => Value::Integer(1),
        Value::Symbol(_) => Value::Integer(0),
        Value::Expr { head, args } => match *head {
            Value::Symbol(ref s) if s == "Plus" => Value::expr(Value::symbol("Plus"), args.into_iter().map(|a| d(a, var)).collect()),
            Value::Symbol(ref s) if s == "Times" => {
                ...
                for i in 0..n {
                    ...
                    for (j, a) in args.iter().enumerate() {
                        if i == j { prod.push(d(a.clone(), var)); }
                        else { prod.push(a.clone()); }
                    }
                ...
                match args.as_slice() {
                    [base, exp] => match exp {
                        Value::Integer(n) if *n >= 0 => {
                            ...
                            mul_list(vec![nval, Value::expr(Value::symbol("Power"), vec![base.clone(), n1]), d(base.clone(), var)])
                        }
                        _ => Value::expr(Value::symbol("Times"), vec![expr_of("Log", vec![base.clone()]), d(base.clone(), var), Value::expr(Value::symbol("Power"), vec![base.clone(), exp.clone()])]),
                    },
                    _ => Value::expr(Value::symbol("D"), vec![Value::expr(*head, args), Value::symbol(var.to_string())]),
                }
            }
            _ => Value::expr(Value::symbol("D"), vec![Value::expr(*head, args), Value::symbol(var.to_string())]),
        },
...
fn expr_of(name: &str, args: Vec<Value>) -> Value { Value::expr(Value::symbol(name.to_string()), args) }
fn mul_list(mut args: Vec<Value>) -> Value {
    // drop neutral 1s
    args.retain(|a| !is_one(a));
    if args.is_empty() { Value::Integer(1) } else if args.len()==1 { args.pop().unwrap() } else { Value::expr(Value::symbol("Times"), args) }
}

fn as_symbol_name(v: &Value) -> Option<String> { if let Value::Symbol(s) = v { Some(s.clone()) } else { None } }
...
fn try_numeric(v: &Value) -> Option<Value> {
    match v { Value::Integer(_) | Value::Real(_) | Value::Rational {..} | Value::BigReal(_) => Some(v.clone()), _ => None }
}
...
fn add_num(a: Value, b: Value) -> Option<Value> {
    match (a, b) {
        ...
        (Value::Rational{num:n1,den:d1}, Value::Rational{num:n2,den:d2}) => Some(Value::Rational{num: n1*d2 + n2*d1, den: d1*d2}),
        (Value::Integer(x), Value::Rational{num,den}) | (Value::Rational{num,den}, Value::Integer(x)) => Some(Value::Rational{num: num + x*den, den}),
        (Value::Real(x), Value::Rational{num,den}) | (Value::Rational{num,den}, Value::Real(x)) => Some(Value::Real(x + (num as f64)/(den as f64))),
...
fn mul_num(a: Value, b: Value) -> Option<Value> {
    match (a, b) {
        ...
        (Value::Integer(x), Value::Real(y)) => Some(Value::Real((x as f64)*y)),
        (Value::Real(x), Value::Integer(y)) => Some(Value::Real(x*(y as f64))),
        (Value::Rational{num:n1,den:d1}, Value::Rational{num:n2,den:d2}) => Some(Value::Rational{num: n1*n2, den: d1*d2}),
        (Value::Integer(x), Value::Rational{num,den}) | (Value::Rational{num,den}, Value::Integer(x)) => Some(Value::Rational{num: num * x, den}),
        (Value::Real(x), Value::Rational{num,den}) | (Value::Rational{num,den}, Value::Real(x)) => Some(Value::Real(x * (num as f64)/(den as f64))),
...
impl Poly {
    fn from_expr(expr: &Value, var: Option<&str>) -> Option<Poly> {
        ...
        if !Self::accumulate(expr, &vname, 1, &mut map) { return None; }
        let max_deg = map.keys().copied().max().unwrap_or(0);
        let mut coeffs = vec![0i64; max_deg+1];
        for (k, c) in map { coeffs[k] = c; }
        let mut p = Poly { var: vname, coeffs, content: 1 };
        p.factor_content();
        Some(p)
    }

    fn accumulate(e: &Value, var: &str, sign: i64, map: &mut std::collections::BTreeMap<usize, i64>) -> bool {
        match e {
            Value::Integer(n) => { *map.entry(0).or_default() += sign * *n; true }
            Value::Rational{..} | Value::Real(_) | Value::BigReal(_) => false,
            Value::Symbol(s) => {
                if s == var { *map.entry(1).or_default() += sign; true } else { false }
            }
            Value::Expr { head, args } => match &**head {
                Value::Symbol(h) if h == "Plus" => {
                    for a in args { if !Self::accumulate(a, var, sign, map) { return false; } }
                    true
                }
                Value::Symbol(h) if h == "Times" => {
                    ...
                    for a in args {
                        match a {
                            Value::Integer(n) => { coeff = coeff.saturating_mul(*n); }
                            Value::Symbol(s) if s == var => { deg = deg.saturating_add(1); }
                            Value::Expr { head: h2, args: a2 } => {
                                if let Value::Symbol(pw) = &**h2 {
                                    if pw == "Power" {
                                        if let [base, exp] = a2.as_slice() {
                                            if let Value::Symbol(s) = base { if s==var { if let Value::Integer(n) = exp { if *n>=0 { deg = deg.saturating_add(*n as usize); continue; } } } }
                                        }
                                        return false;
                                    }
                                }
                    ...
                    *map.entry(deg).or_default() += coeff; true
                }
                Value::Symbol(h) if h == "Power" => {
                    if let [base, exp] = args.as_slice() {
                        if let Value::Symbol(s) = base { if s==var { if let Value::Integer(n) = exp { if *n>=0 { *map.entry(*n as usize).or_default() += sign; return true; } } } }
                    }
                    false
                }
                _ => false,
    ...
    fn to_expr(&self) -> Value {
        ...
        for (k, c) in self.coeffs.iter().enumerate() {
            ...
            if k>0 {
                if k==1 { term.push(Value::symbol(self.var.clone())); }
                else { term.push(Value::expr(Value::symbol("Power"), vec![Value::symbol(self.var.clone()), Value::Integer(k as i64)])); }
            }
            terms.push(mul_list(term));
        }
        if terms.is_empty() { Value::Integer(0) } else if terms.len()==1 { terms[0].clone() } else { Value::expr(Value::symbol("Plus"), terms) }
    }

    fn factor_content(&mut self) {
        // extract gcd of coefficients and normalize sign to content
        let mut g: i64 = 0;
        for c in &self.coeffs { g = gcd(g, *c); }
        if g == 0 { self.content = 1; return; }
        self.content = g;
        for c in &mut self.coeffs { *c /= g; }
        // keep content sign positive by moving sign to content
        if self.coeffs.iter().all(|&c| c==0) { self.content = 1; }
    }

    fn factor_quadratic(&self) -> Option<(Poly, Poly)> {
        // ax^2 + bx + c with a!=0; factor over integers if discriminant square
        if self.coeffs.len() < 3 { return None; }
        ...
        let r1n = -b + sd;
        let r2n = -b - sd;
        if r1n % (2*a) != 0 || r2n % (2*a) != 0 { return None; }
        let r1 = r1n / (2*a);
        let r2 = r2n / (2*a);
        // a(x - r1)(x - r2) -> move 'a' into content and keep monic
        let mut p1 = Poly { var: self.var.clone(), coeffs: vec![-r1, 1], content: 1 };
        let mut p2 = Poly { var: self.var.clone(), coeffs: vec![-r2, 1], content: self.content * a };
        p1.factor_content(); p2.factor_content();
        Some((p1, p2))
    }
}
...
fn first_symbol(v: &Value) -> Option<String> {
    match v {
        ...
        Value::Expr { head, args } => {
            if let Some(s) = first_symbol(head) { return Some(s); }
            for a in args { if let Some(s) = first_symbol(a) { return Some(s); } }
            None
        }
        _ => None,
    }
