...
impl Level { fn from_str(s: &str) -> Option<Self> { match s.to_lowercase().as_str() { "trace"=>Some(Level::Trace), "debug"=>Some(Level::Debug), "info"=>Some(Level::Info), "warn"=>Some(Level::Warn), "error"=>Some(Level::Error), _=>None } } fn as_str(&self)->&'static str { match self { Level::Trace=>"trace", Level::Debug=>"debug", Level::Info=>"info", Level::Warn=>"warn", Level::Error=>"error" } } }

#[derive(Clone)]
struct LoggerConf { level: Level, to_stderr: bool, json: bool, file: Option<String>, include_time: bool, include_span: bool }

static LOGGER: OnceLock<Mutex<LoggerConf>> = OnceLock::new();
fn logger() -> &'static Mutex<LoggerConf> { LOGGER.get_or_init(|| Mutex::new(LoggerConf { level: Level::Info, to_stderr: true, json: false, file: None, include_time: true, include_span: false })) }
...
pub fn register_logging_filtered(ev: &mut Evaluator, pred: &dyn Fn(&str)->bool) {
    crate::register_if(ev, pred, "ConfigureLogging", configure_logging as NativeFn, Attributes::empty());
...
fn configure_logging(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len()!=1 { return Value::Expr{ head: Box::new(Value::Symbol("ConfigureLogging".into())), args } }
    let opts = ev.eval(args[0].clone());
    if let Value::Assoc(m) = opts {
        let mut conf = logger().lock().unwrap().clone();
        if let Some(Value::String(s))|Some(Value::Symbol(s)) = m.get("Level") { if let Some(l) = Level::from_str(s) { conf.level = l; } }
        if let Some(Value::String(s))|Some(Value::Symbol(s)) = m.get("Format") { conf.json = s.to_lowercase()=="json"; }
        if let Some(Value::Assoc(file)) = m.get("File") { if let Some(Value::String(p))|Some(Value::Symbol(p)) = file.get("Path") { conf.file = Some(p.clone()); } }
        if let Some(Value::Boolean(b)) = m.get("IncludeTime") { conf.include_time = *b; }
        if let Some(Value::Boolean(b)) = m.get("IncludeSpan") { conf.include_span = *b; }
        // If File.Path set, try opening once to validate
        if let Some(path) = conf.file.clone() { if std::fs::OpenOptions::new().create(true).append(true).open(&path).is_err() { return failure("Log::config", &format!("Cannot open log file: {}", path)); } }
...
fn emit(level: Level, msg: String, meta: Option<HashMap<String, Value>>) -> Result<(), String> {
    ...
    if conf.json {
        ...
        if let Some(t) = ts { obj.insert("ts".into(), serde_json::Value::Number((t).into())); }
        obj.insert("msg".into(), serde_json::Value::String(msg));
        // merge context
        for m in ctx { for (k, v) in m { obj.insert(k, val_to_json(&v)); } }
        if let Some(m) = meta { for (k, v) in m { obj.insert(k, val_to_json(&v)); } }
        let line = serde_json::Value::Object(obj).to_string() + "\n";
        write_out(conf, line.as_bytes())
    } else {
        let mut parts: Vec<String> = Vec::new();
        if let Some(t) = ts { parts.push(format!("{}", t)); }
        parts.push(level.as_str().to_string());
        parts.push(msg);
        if let Some(m) = meta { if !m.is_empty() { parts.push(format!("meta={}", format_value(&Value::Assoc(m)))); } }
        let line = parts.join(" ") + "\n";
        write_out(conf, line.as_bytes())
    }
}
...
fn val_to_json(v: &Value) -> serde_json::Value {
    match v {
        ...
        Value::Assoc(m) => {
            ...
            for (k, vv) in m.iter() { obj.insert(k.clone(), val_to_json(vv)); }
...
fn log_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len()<2 { return Value::Expr{ head: Box::new(Value::Symbol("Log".into())), args } }
    let level_s = match ev.eval(args[0].clone()) { Value::String(s)|Value::Symbol(s)=>s, other=> format_value(&other) };
    let level = Level::from_str(&level_s).unwrap_or(Level::Info);
    let msg = match ev.eval(args[1].clone()) { Value::String(s)|Value::Symbol(s)=>s, other=> format_value(&other) };
    let meta = if args.len()>=3 { match ev.eval(args[2].clone()) { Value::Assoc(m)=>Some(m), _=>None } } else { None };
    match emit(level, msg, meta) { Ok(_)=> Value::Boolean(true), Err(e)=> failure("Log::emit", &e) }
}

fn with_logger(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len()!=2 { return Value::Expr{ head: Box::new(Value::Symbol("WithLogger".into())), args } }
    let ctx = match ev.eval(args[0].clone()) { Value::Assoc(m)=>m, _=> HashMap::new() };
    LOG_CONTEXT.with(|c| c.borrow_mut().push(ctx));
    let out = ev.eval(args[1].clone());
    LOG_CONTEXT.with(|c| { let _ = c.borrow_mut().pop(); });
    out
}

fn set_log_level(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len()!=1 { return Value::Expr{ head: Box::new(Value::Symbol("SetLogLevel".into())), args } }
    let s = match ev.eval(args[0].clone()) { Value::String(s)|Value::Symbol(s)=>s, _=> String::new() };
    if let Some(l) = Level::from_str(&s) { logger().lock().unwrap().level = l; return Value::Boolean(true); }
    failure("Log::config", "Unknown log level")
}

fn get_logger(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if !args.is_empty() { return Value::Expr{ head: Box::new(Value::Symbol("GetLogger".into())), args } }
