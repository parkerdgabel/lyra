...
fn str_of(ev: &mut Evaluator, v: Value) -> String { match ev.eval(v) { Value::String(s)|Value::Symbol(s)=>s, other=> lyra_core::pretty::format_value(&other) } }

fn current_dir(ev: &Evaluator) -> PathBuf {
    if let Some(Value::String(s)) = ev_env_get(ev, "CurrentDir") { PathBuf::from(s) } else { std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")) }
}

fn ev_env_get(ev: &Evaluator, key: &str) -> Option<Value> {
...
fn resolve_module_path(ev: &mut Evaluator, target: &str) -> Option<PathBuf> {
    ...
    let is_pathish = target.starts_with("./") || target.starts_with("../") || target.starts_with('/') || target.ends_with(".lyra");
    if is_pathish {
        let base = current_dir(ev);
        let p = if Path::new(target).is_absolute() { PathBuf::from(target) } else { base.join(target) };
        return Some(p);
    }
...
fn using(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() { return Value::Expr { head: Box::new(Value::Symbol("Using".into())), args } }
    ...
    let path = match resolve_module_path(ev, &target) { Some(p)=>p, None=> return lyra_core::value::Value::Assoc(HashMap::from([(String::from("message"), Value::String(format!("Module not found: {}", target))), (String::from("tag"), Value::String(String::from("Module::using")))])) };
    let abs = if path.is_absolute() { path } else { fs::canonicalize(&path).unwrap_or(path) };
    // read file
    let content = match fs::read_to_string(&abs) { Ok(s)=>s, Err(e)=> return failure("Module::using", &e.to_string()) };
    // capture keys before
    ...
    match p.parse_all() {
        Ok(exprs) => { for e in exprs { let _ = ev.eval(e); } }
        Err(e) => { return failure("Module::using", &format!("parse: {:?}", e)); }
    }
    ...
    if let Some(Value::List(xs)) = ev.get_env("__exports_declared") {
        ...
        for v in xs { if let Value::String(s)|Value::Symbol(s)=v { names.push(s.clone()); } }
        if !names.is_empty() { new_syms = names; }
        ev.unset_env("__exports_declared");
    }
    for name in &new_syms { exports.push((name.clone(), Value::Symbol(name.clone()))); }
...
fn exported(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len()!=1 { return Value::Expr { head: Box::new(Value::Symbol("Exported".into())), args } }
    match ev.eval(args[0].clone()) {
        Value::List(xs) => { ev.set_env("__exports_declared", Value::List(xs)); Value::Boolean(true) }
        other => Value::Expr { head: Box::new(Value::Symbol("Exported".into())), args: vec![other] }
    }
}

fn module_info(ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    let mut m = HashMap::new();
    if let Some(v) = ev.get_env("CurrentFile") { m.insert("Path".into(), v); }
    if let Some(v) = ev.get_env("CurrentDir") { m.insert("Dir".into(), v); }
    if let Some(v) = ev.get_env("ProjectRoot") { m.insert("ProjectRoot".into(), v); }
    Value::Assoc(m)
}

fn resolve_relative(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len()!=1 { return Value::Expr { head: Box::new(Value::Symbol("ResolveRelative".into())), args } }
