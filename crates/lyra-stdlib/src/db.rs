...
fn conn_reg() -> &'static Mutex<HashMap<i64, ConnectionState>> {
    CONN_REG.get_or_init(|| Mutex::new(HashMap::new()))
}
fn next_conn_id() -> i64 {
    let a = NEXT_CONN_ID.get_or_init(|| std::sync::atomic::AtomicI64::new(1));
...
pub fn db_table_columns(conn_id: i64, table: &str) -> Option<Vec<String>> {
    ...
    drop(reg);
    match st.kind {
        ConnectorKind::Mock => st.mock_tables.get(table).and_then(|rows| {
            rows.iter().find_map(|r| match r {
                Value::Assoc(m) => Some(m.keys().cloned().collect::<Vec<_>>()),
                _ => None,
            })
        }),
        #[cfg(feature = "db_sqlite")]
...
fn list_duckdb_columns(dsn: &str, table: &str) -> duckdb::Result<Vec<String>> {
    ...
    let mut out = Vec::new();
    while let Some(row) = rows.next()? {
        if let Ok(Some(name)) = row.get::<usize, Option<String>>(0) {
            out.push(name);
        }
    }
    Ok(out)
...
fn connect(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    let (dsn, kind) = match ev.eval(args[0].clone()) {
        Value::String(s) | Value::Symbol(s) => {
            ...
            } else if s.starts_with("duckdb:") || s.starts_with("duckdb://") {
                #[cfg(feature = "db_duckdb")]
                {
                    ConnectorKind::DuckDb
                }
        ...
        Value::Assoc(m) => {
            let dsn = match m.get("DSN") {
                Some(Value::String(s)) | Some(Value::Symbol(s)) => s.clone(),
                _ => "mock://".to_string(),
            };
            let kind = if dsn.starts_with("mock:") || dsn.starts_with("mock://") {
                ConnectorKind::Mock
            ...
            } else if dsn.starts_with("duckdb:") || dsn.starts_with("duckdb://") {
                #[cfg(feature = "db_duckdb")]
                {
                    ConnectorKind::DuckDb
                }
        ...
        other => {
            return Value::Expr {
                head: Box::new(Value::Symbol("Connect".into())),
                args: vec![other],
            }
        }
    };
    ...
    #[cfg(feature = "db_sqlite")]
    let mut sqlite_conn: Option<std::sync::Arc<std::sync::Mutex<rusqlite::Connection>>> = None;
    #[cfg(feature = "db_sqlite")]
    {
        if matches!(kind, ConnectorKind::Sqlite) {
            if let Ok(c) = sqlite_open(&dsn) {
                sqlite_conn = Some(std::sync::Arc::new(std::sync::Mutex::new(c)));
            }
        }
    }
    #[cfg(feature = "db_duckdb")]
    let mut duckdb_conn: Option<std::sync::Arc<std::sync::Mutex<duckdb::Connection>>> = None;
    #[cfg(feature = "db_duckdb")]
    {
        if matches!(kind, ConnectorKind::DuckDb) {
            if let Ok(c) = duckdb_open(&dsn) {
                duckdb_conn = Some(std::sync::Arc::new(std::sync::Mutex::new(c)));
            }
        }
    }
    conn_reg().lock().unwrap().insert(
        id,
        ConnectionState {
            dsn,
            kind,
...
fn disconnect(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("Disconnect".into())), args };
    }
    let id = match get_conn(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("Disconnect".into())), args },
    };
...
fn ping(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("Ping".into())), args };
    }
    let id = match get_conn(&args[0]) {
        Some(_) => true,
        None => false,
...
fn list_tables(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("ListTables".into())), args };
    }
    let id = match get_conn(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("ListTables".into())), args },
    };
    let reg = conn_reg().lock().unwrap();
...
fn register_table(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 3 {
        return Value::Expr { head: Box::new(Value::Symbol("RegisterTable".into())), args };
    }
    let id = match get_conn(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("RegisterTable".into())), args },
    };
    let table = match ev.eval(args[1].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => {
            return Value::Expr {
                head: Box::new(Value::Symbol("RegisterTable".into())),
                args: vec![args[0].clone(), other, args[2].clone()],
            }
        }
    };
    let rows = match ev.eval(args[2].clone()) {
        Value::List(vs) => vs,
        other => {
            return Value::Expr {
                head: Box::new(Value::Symbol("RegisterTable".into())),
                args: vec![args[0].clone(), Value::String(table), other],
            }
        }
    };
    let mut reg = conn_reg().lock().unwrap();
    let st = match reg.get_mut(&id) {
        Some(s) => s,
        None => return Value::Expr { head: Box::new(Value::Symbol("RegisterTable".into())), args },
    };
...
fn table_to_dataset(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 2 {
        return Value::Expr { head: Box::new(Value::Symbol("Table".into())), args };
    }
    let conn_id = match get_conn(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("Table".into())), args },
    };
    let name = match ev.eval(args[1].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => {
            return Value::Expr {
                head: Box::new(Value::Symbol("Table".into())),
    ...
    let call = Value::Expr {
        ...
        args: vec![Value::Integer(conn_id), Value::String(name)],
    };
    ev.eval(call)
}
...
fn sql_query(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    match st.kind {
        ConnectorKind::Mock => {
            ...
            if parts.len() >= 4
                && parts[0].eq_ignore_ascii_case("select")
                && parts[1] == "*"
...
fn sql_to_rows(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 2 {
        return Value::Expr { head: Box::new(Value::Symbol("__SQLToRows".into())), args };
    }
    let conn_id = match &args[0] {
        Value::Integer(id) => *id,
        _ => return Value::Expr { head: Box::new(Value::Symbol("__SQLToRows".into())), args },
    };
    ...
    let st = match reg.get(&conn_id) {
        Some(s) => s.clone(),
        None => return Value::Expr { head: Box::new(Value::Symbol("__SQLToRows".into())), args },
    };
    drop(reg);
...
fn cur_reg() -> &'static Mutex<HashMap<i64, CursorState>> {
    CUR_REG.get_or_init(|| Mutex::new(HashMap::new()))
}
fn next_cur_id() -> i64 {
    let a = NEXT_CUR_ID.get_or_init(|| std::sync::atomic::AtomicI64::new(1));
    a.fetch_add(1, std::sync::atomic::Ordering::Relaxed)
}
...
fn sql_cursor(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("SQLCursor".into())), args };
    }
    let conn_id = match get_conn(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("SQLCursor".into())), args },
    };
    let mut sql = match ev.eval(args[1].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => {
            return Value::Expr {
                head: Box::new(Value::Symbol("SQLCursor".into())),
                args: vec![args[0].clone(), other],
            }
        }
    };
    if args.len() >= 3 {
        if let Value::Assoc(m) = ev.eval(args[2].clone()) {
            sql = substitute_params(&sql, &m);
        }
    }
    let fetch_size = if args.len() >= 4 {
        if let Value::Assoc(m) = ev.eval(args[3].clone()) {
            if let Some(Value::Integer(n)) = m.get("FetchSize") {
                *n
            } else {
                1000
            }
...
fn fetch_cursor(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("Fetch".into())), args };
    }
    let id = match get_cursor(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("Fetch".into())), args },
    };
    let n = if args.len() >= 2 {
...
fn close_cursor(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("Close".into())), args };
    }
    if let Some(id) = get_cursor(&args[0]) {
        cur_reg().lock().unwrap().remove(&id);
...
fn insert_rows(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 3 {
        return Value::Expr { head: Box::new(Value::Symbol("InsertRows".into())), args };
    }
    let conn_id = match get_conn(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("InsertRows".into())), args },
    };
    let table = match ev.eval(args[1].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => {
            return Value::Expr {
                head: Box::new(Value::Symbol("InsertRows".into())),
                args: vec![args[0].clone(), other, args[2].clone()],
            }
        }
    };
    ...
    let rows = match &rows_v {
        ...
        _ => {
            return Value::Expr {
                head: Box::new(Value::Symbol("InsertRows".into())),
                args: vec![args[0].clone(), Value::String(table), rows_v],
            }
        }
    };
    let reg = conn_reg().lock().unwrap();
    ...
    for r in &rows {
        if let Value::Assoc(m) = r {
            ...
            let vals: Vec<String> = cols
                .iter()
                .map(|c| value_to_sql_literal(m.get(c).unwrap_or(&Value::Symbol("Null".into()))))
                .collect();
    ...
    match st.kind {
        ...
        ConnectorKind::Sqlite => {
            if let Some(conn) = st.sqlite_conn.as_ref() {
                ...
                if !st.in_tx {
                    let _ = guard.execute_batch("BEGIN");
                }
                let res = guard
                    .execute_batch(&sql)
                    .map(|_| Value::Integer(count))
                    .unwrap_or(Value::Integer(0));
                if !st.in_tx {
                    let _ = guard.execute_batch("COMMIT");
                }
                return res;
            }
            Value::Integer(0)
        ...
        ConnectorKind::DuckDb => {
            if let Some(conn) = st.duckdb_conn.as_ref() {
                ...
                if !st.in_tx {
                    let _ = guard.execute_batch("BEGIN TRANSACTION");
                }
                let res = guard
                    .execute_batch(&sql)
                    .map(|_| Value::Integer(count))
...
fn upsert_rows(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 3 {
        return Value::Expr { head: Box::new(Value::Symbol("UpsertRows".into())), args };
    }
    let conn_id = match get_conn(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("UpsertRows".into())), args },
    };
    let table = match ev.eval(args[1].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => {
            return Value::Expr {
                head: Box::new(Value::Symbol("UpsertRows".into())),
                args: vec![args[0].clone(), other, args[2].clone()],
            }
        }
    };
    ...
    let keys: Vec<String> = if args.len() >= 4 {
        if let Value::Assoc(m) = ev.eval(args[3].clone()) {
            if let Some(Value::List(ks)) = m.get("Keys") {
                ks.iter()
        ...
        ConnectorKind::Mock => {
            ...
            if let Some(state) = reg.get_mut(&conn_id) {
                ...
                for r in rows {
                    if let Value::Assoc(m) = &r {
                        if !keys.is_empty() {
                            ent.retain(|row| match row {
                                Value::Assoc(mm) => !keys.iter().all(|k| mm.get(k) == m.get(k)),
                                _ => true,
                            });
                        }
                        ent.push(r);
        ...
        ConnectorKind::Sqlite => {
            ...
            for r in rows {
                if let Value::Assoc(m) = r {
                            ...
                            .map(|k| {
                                format!(
                                    ...
                                    value_to_sql_literal(
                                        m.get(k).unwrap_or(&Value::Symbol("Null".into()))
                                    )
                                )
                            })
                            .collect();
                    ...
                    let vals: Vec<String> =
                        cols.iter().map(|c| value_to_sql_literal(m.get(c).unwrap())).collect();
                    sql.push_str(&format!(
                        "INSERT INTO {} ({}) VALUES ({}); ",
                        table,
                        cols.join(", "),
            ...
            if let Some(conn) = st.sqlite_conn.as_ref() {
                let _ = conn.lock().unwrap().execute_batch(&sql);
            }
            Value::Integer(inserted)
        }
        #[cfg(feature = "db_duckdb")]
...
fn write_dataset(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 3 {
        return Value::Expr { head: Box::new(Value::Symbol("WriteDataset".into())), args };
    }
    ...
    let rows = match ds {
        Value::Assoc(_) | Value::List(_) | Value::Expr { .. } => {
            ev.eval(Value::Expr { head: Box::new(Value::Symbol("Collect".into())), args: vec![ds] })
        }
        other => other,
    };
    if let Value::String(tname) | Value::Symbol(tname) = table {
        if mode.eq_ignore_ascii_case("Overwrite") {
            ...
            let conn_id = match get_conn(&conn) {
                Some(id) => id,
                None => {
                    return Value::Expr {
                        head: Box::new(Value::Symbol("WriteDataset".into())),
                        args,
                    }
                }
            };
            ...
            match st.kind {
                ConnectorKind::Mock => {
                    let mut reg = conn_reg().lock().unwrap();
                    if let Some(state) = reg.get_mut(&conn_id) {
                        state.mock_tables.insert(tname.clone(), Vec::new());
                    }
                }
                #[cfg(feature = "db_sqlite")]
                ConnectorKind::Sqlite => {
                    if let Some(c) = st.sqlite_conn.as_ref() {
                        let _ = c.lock().unwrap().execute(&format!("DELETE FROM {}", tname), []);
                    }
                }
                #[cfg(feature = "db_duckdb")]
                ConnectorKind::DuckDb => {
...
fn begin_tx(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("Begin".into())), args };
    }
    let id = match get_conn(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("Begin".into())), args },
    };
    let mut reg = conn_reg().lock().unwrap();
    if let Some(st) = reg.get_mut(&id) {
        match st.kind {
            ...
            #[cfg(feature = "db_sqlite")]
            ConnectorKind::Sqlite => {
                if let Some(c) = &st.sqlite_conn {
                    let _ = c.lock().unwrap().execute_batch("BEGIN");
                    st.in_tx = true;
                    return Value::Boolean(true);
                }
            }
            #[cfg(feature = "db_duckdb")]
            ConnectorKind::DuckDb => {
                if let Some(c) = &st.duckdb_conn {
                    let _ = c.lock().unwrap().execute_batch("BEGIN TRANSACTION");
                    st.in_tx = true;
                    return Value::Boolean(true);
                }
            }
...
fn commit_tx(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("Commit".into())), args };
    }
    let id = match get_conn(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("Commit".into())), args },
    };
    let mut reg = conn_reg().lock().unwrap();
    if let Some(st) = reg.get_mut(&id) {
        match st.kind {
            ...
            ConnectorKind::Sqlite => {
                if let Some(c) = &st.sqlite_conn {
                    let _ = c.lock().unwrap().execute_batch("COMMIT");
                    st.in_tx = false;
                    return Value::Boolean(true);
                }
            }
...
fn rollback_tx(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("Rollback".into())), args };
    }
    let id = match get_conn(&args[0]) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("Rollback".into())), args },
    };
    let mut reg = conn_reg().lock().unwrap();
    if let Some(st) = reg.get_mut(&id) {
        match st.kind {
            ...
            ConnectorKind::Sqlite => {
                if let Some(c) = &st.sqlite_conn {
                    let _ = c.lock().unwrap().execute_batch("ROLLBACK");
                    st.in_tx = false;
                    return Value::Boolean(true);
                }
            }
...
fn exec_query(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    match st.kind {
        ...
        ConnectorKind::Sqlite => {
            if let Some(c) = st.sqlite_conn.as_ref() {
                if let Some(pm) = &_params_map {
                    ...
                    let ok = c.lock().unwrap().execute_batch(&sql).is_ok();
                    Value::Boolean(ok)
                }
            } else {
                Value::Boolean(false)
        ...
        ConnectorKind::DuckDb => {
            if let Some(c) = st.duckdb_conn.as_ref() {
                let sql2 =
                    if let Some(pm) = &_params_map { substitute_params(&sql, pm) } else { sql };
                let ok = c.lock().unwrap().execute_batch(&sql2).is_ok();
                Value::Boolean(ok)
            } else {
                Value::Boolean(false)
            }
...
#[cfg(feature = "db_duckdb")]
fn fetch_duckdb_rows_prepared(
    dsn: &str,
    sql: &str,
    params: &HashMap<String, Value>,
) -> duckdb::Result<Vec<Value>> {
