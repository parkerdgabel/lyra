...
fn ml_reg() -> &'static Mutex<HashMap<i64, ModelState>> {
    ML_REG.get_or_init(|| Mutex::new(HashMap::new()))
}
fn next_id() -> i64 {
...
fn model_handle(id: i64) -> Value {
    Value::assoc(vec![("__type", Value::String("MLModel".into())), ("id", Value::Integer(id))])
}
fn get_model_id(v: &Value) -> Option<i64> {
    if let Value::Assoc(m) = v {
        if matches!(m.get("__type"), Some(Value::String(s)) if s=="MLModel") {
...
#[allow(dead_code)]
fn to_list(ev: &mut Evaluator, v: Value) -> Vec<Value> {
    match ev.eval(v) {
        Value::List(xs) => xs,
        other => vec![other],
    }
...
#[allow(dead_code)]
fn is_assoc(v: &Value) -> bool {
    matches!(v, Value::Assoc(_))
}
...
fn z_for_conf(level: f64) -> f64 {
    ...
    if (level - 0.90).abs() < 1e-6 {
        1.6449
    } else if (level - 0.95).abs() < 1e-6 {
        1.96
    } else if (level - 0.99).abs() < 1e-6 {
        2.575
    } else {
        1.96
    }
...
fn build_preproc_from_pairs(
    pairs: &[(Value, Value)],
    features_opt: Option<Vec<String>>,
        ...
        match x {
            Value::Assoc(m) => {
                for (k, v) in m {
                    ...
                    if let Some(fs) = &features_opt {
                        if !fs.contains(k) {
                            continue;
                        }
                    }
                    if !feature_cols.contains(k) {
                        feature_cols.push(k.clone());
                    }
                    match v {
                        Value::Integer(n) => {
                            *num_sums.entry(k.clone()).or_insert(0.0) += *n as f64;
                            *num_cnts.entry(k.clone()).or_insert(0.0) += 1.0;
                        }
                        Value::Real(x) => {
                            *num_sums.entry(k.clone()).or_insert(0.0) += *x;
                            *num_cnts.entry(k.clone()).or_insert(0.0) += 1.0;
                        }
                        other => {
                            let key = pretty_key(other);
                            let row = cat_counts.entry(k.clone()).or_insert_with(HashMap::new);
                            *row.entry(key).or_insert(0) += 1;
                        }
                    }
                }
            }
    ...
    for k in &numeric_cols {
        let mean =
            num_sums.get(k).cloned().unwrap_or(0.0) / num_cnts.get(k).cloned().unwrap_or(1.0);
        num_stats.insert(k.clone(), (mean, 1.0));
    }
    let mut cat_vocab: HashMap<String, Vec<String>> = HashMap::new();
    for (k, row) in cat_counts {
        let mut cats: Vec<(String, i64)> = row.into_iter().collect();
        cats.sort_by_key(|(_, c)| -c);
        let vocab: Vec<String> = cats.into_iter().map(|(s, _)| s).take(100).collect();
        cat_vocab.insert(k, vocab);
    }
    // Dim: numeric (1 each) + sum(one-hot sizes)
    let mut dim = numeric_cols.len();
    for v in cat_vocab.values() {
        dim += v.len();
    }
...
fn featurize(pre: &Preproc, x: &Value) -> Vec<f64> {
    ...
    for c in &pre.numeric_cols {
        let val = match x {
            Value::Assoc(m) => m
                .get(c)
                .and_then(|v| as_f64(v))
                .unwrap_or(pre.num_stats.get(c).map(|(m, _)| *m).unwrap_or(0.0)),
            Value::List(vs) => {
                let i = c.strip_prefix('f').and_then(|s| s.parse::<usize>().ok()).unwrap_or(0);
                vs.get(i)
                    .and_then(|v| as_f64(v))
                    .unwrap_or(pre.num_stats.get(c).map(|(m, _)| *m).unwrap_or(0.0))
            }
        ...
        out[idx] = if std > 0.0 { (val - mean) / std } else { val - mean };
        idx += 1;
    }
    // Categorical
    for c in &pre.categorical_cols {
        ...
        let val_key = match x {
            Value::Assoc(m) => {
                m.get(c).map(|v| pretty_key(v)).unwrap_or_else(|| "__MISSING__".into())
            }
            Value::List(vs) => {
                let i = c.strip_prefix('f').and_then(|s| s.parse::<usize>().ok()).unwrap_or(0);
                vs.get(i).map(|v| pretty_key(v)).unwrap_or_else(|| "__MISSING__".into())
            }
            _ => "__MISSING__".into(),
        };
        for (j, cat) in vocab.iter().enumerate() {
            out[idx + j] = if *cat == val_key { 1.0 } else { 0.0 };
        }
        idx += vocab.len();
    }
    out
...
fn dot(w: &Vec<f64>, x: &Vec<f64>) -> f64 {
...
fn classify(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("Classify".into())), args };
    }
                ...
                for it in items {
                    if let Value::Expr { head, args } = it {
                        if matches!(*head, Value::Symbol(ref s) if s=="Rule") && args.len() == 2 {
                            labels.push(args[1].clone());
                        }
                    }
                }
            }
            Value::Assoc(_) => {
                let target = get_option(&opts, "Target").and_then(|v| match v {
        ...
        match train {
            ...
            Value::Assoc(_) => {
                ...
                if let Some(tcol) = target {
                    if let Some(rows) = collect_dataset_rows(ev, &train) {
                        for m in rows {
                            let y = m.get(&tcol).cloned().unwrap_or(Value::Symbol("Null".into()));
                            pairs.push((Value::Assoc(m), y));
                        }
                    }
                }
            }
    ...
    for (_, y) in &pairs {
        let k = pretty_key(y);
        if !class_values.iter().any(|v| pretty_key(v) == k) {
            class_values.push(y.clone());
        }
    }
    let use_logistic = method.eq_ignore_ascii_case("logistic")
        || (method.eq_ignore_ascii_case("automatic")
            && class_values.len() > 1
            && class_values.len() <= 10
            && !pairs.is_empty());
    if use_logistic {
        ...
        let mut w_all: Vec<Vec<f64>> = Vec::new();
        let mut b_all: Vec<f64> = Vec::new();
        for c in &class_values {
            let mut w = vec![0.0; pre.dim];
            let mut b = 0.0;
            for _ in 0..epochs {
                let mut grad_w = vec![0.0f64; pre.dim];
        ...
        let id = next_id();
        ml_reg().lock().unwrap().insert(
            id,
            ModelState {
                task: "classification".into(),
                method: "Logistic".into(),
                classes: Some(class_values),
                class_counts: None,
                majority: None,
                mean: None,
                var: None,
                train_size: pairs.len(),
                preproc: Some(pre),
                lin_w: None,
                lin_b: None,
                resid_std: None,
        ...
        let body = Value::expr(
            Value::symbol("MLApply"),
            vec![model_handle(id), Value::slot(None), Value::slot(Some(2))],
        );
    ...
    for (_, y) in pairs {
        let k = pretty_key(&y);
        if !classes.iter().any(|c| pretty_key(c) == k) {
            classes.push(y.clone());
        }
        let c = *counts.get(&k).unwrap_or(&0);
        if majority.as_ref().map(|(_, m)| c > *m).unwrap_or(true) {
            majority = Some((y, c));
        }
    }
    let id = next_id();
...
fn predict(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("Predict".into())), args };
    }
    ...
    if pos.len() == 1 {
        ...
        match train {
                    ...
                    if let Value::Expr { head, args } = it {
                        if matches!(*head, Value::Symbol(ref s) if s=="Rule") && args.len() == 2 {
                            match ev.eval(args[1].clone()) {
                                Value::Integer(n) => ys.push(n as f64),
                                Value::Real(x) => ys.push(x),
                                _ => {}
                            }
                        }
                    }
            ...
            Value::Assoc(_) => {
                ...
                let target = get_option(&opts2, "Target").and_then(|v| match v {
                    Value::String(s) | Value::Symbol(s) => Some(s.clone()),
                    _ => None,
                });
                if let Some(tcol) = target {
    ...
    let method = get_option(&opts2, "Method")
        .and_then(|v| match v {
            Value::String(s) | Value::Symbol(s) => Some(s.clone()),
            _ => None,
        })
        ...
        match train {
            Value::List(items) => {
                for it in items {
                    if let Value::Expr { head, args } = it {
                        if matches!(*head, Value::Symbol(ref s) if s=="Rule") && args.len() == 2 {
                            let y = as_f64(&ev.eval(args[1].clone()));
                            if let Some(yf) = y {
                                pairs.push((args[0].clone(), yf));
                            }
                        }
                    }
                }
            ...
            Value::Assoc(_) => {
                ...
                if let Some(tcol) = target {
                    if let Some(rows) = collect_dataset_rows(ev, &train) {
                        for m in rows {
                            if let Some(yv) = m.get(&tcol).and_then(|v| as_f64(v)) {
                                pairs.push((Value::Assoc(m), yv));
                            }
                        }
                    }
                }
    ...
    if !pairs.is_empty()
        && (method.eq_ignore_ascii_case("linear") || method.eq_ignore_ascii_case("automatic"))
    {
        let pre = build_preproc_from_pairs(
            &pairs.iter().map(|(x, y)| (x.clone(), Value::Real(*y))).collect::<Vec<_>>(),
            None,
            None,
        );
        ...
        let mut w = vec![0.0f64; pre.dim];
        let mut b = 0.0f64;
        if pre.dim == 0 {
            // intercept-only: set to mean directly
    ...
    let var = ys.iter().map(|x| (x - mean) * (x - mean)).sum::<f64>() / n.max(1.0);
    let id = next_id();
    ml_reg().lock().unwrap().insert(
        id,
        ModelState {
...
fn cluster(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("Cluster".into())), args };
    }
    let id = next_id();
    ml_reg().lock().unwrap().insert(
        id,
        ModelState {
            task: "clustering".into(),
            method: "Baseline".into(),
            classes: None,
            class_counts: None,
            majority: None,
...
fn feature_extract(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("FeatureExtract".into())), args };
    }
...
fn dimension_reduce(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("DimensionReduce".into())), args };
    }
...
fn ml_apply(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("MLApply".into())), args };
    }
    let model_v = ev.eval(args[0].clone());
    let x = args[1].clone();
    let opts =
        if args.len() >= 3 { ev.eval(args[2].clone()) } else { Value::Assoc(HashMap::new()) };
    let id = match get_model_id(&model_v) {
        Some(id) => id,
        None => {
            return Value::Expr {
                head: Box::new(Value::Symbol("MLApply".into())),
                args: vec![model_v, x],
            }
        }
    };
    ...
    let st = match reg.get(&id) {
        Some(s) => s.clone(),
        None => return Value::Expr { head: Box::new(Value::Symbol("MLApply".into())), args },
    };
    ...
    if let Value::List(xs) = ev.eval(x.clone()) {
        ...
        for xi in xs {
            out.push(ml_apply(ev, vec![model_v.clone(), xi, Value::Assoc(opts_map.clone())]));
        }
        return Value::List(out);
    }
    match st.task.as_str() {
        "classification" => {
            let output = get_option(&opts_map, "Output")
                .and_then(|v| match v {
                    Value::String(s) | Value::Symbol(s) => Some(s.clone()),
                    _ => None,
                })
            ...
            if st.method.eq_ignore_ascii_case("Logistic") {
                let pre = match &st.preproc {
                    Some(p) => p.clone(),
                    None => return Value::Symbol("Null".into()),
                };
                let cls_w = match &st.cls_w {
                    Some(w) => w.clone(),
                    None => vec![],
                };
                let cls_b = match &st.cls_b {
                    Some(b) => b.clone(),
                    None => vec![],
                };
                ...
                for (w, b) in cls_w.iter().zip(cls_b.iter()) {
                    scores.push(1.0 / (1.0 + (-(dot(w, &xv) + *b)).exp()));
                }
                // normalize to sum=1 if multi-class
                let sum: f64 = scores.iter().sum();
                let probs_norm: Vec<f64> = if classes.len() > 1 && sum > 0.0 {
                ...
                if output.eq_ignore_ascii_case("Probabilities") {
                    ...
                    for (c, p) in classes.iter().zip(probs_norm.iter()) {
                        probs.insert(pretty_key(c), Value::Real(*p));
                    }
                    return Value::Assoc(probs);
                }
                if output.eq_ignore_ascii_case("TopK") {
                    let k =
                        get_option(&opts_map, "K").and_then(|v| as_f64(v)).unwrap_or(3.0) as usize;
                    let mut pairs: Vec<(String, f64)> = classes
                        .iter()
                        .zip(probs_norm.iter())
                        .map(|(c, p)| (pretty_key(c), *p))
                        .collect();
                    pairs
                ...
                if output.eq_ignore_ascii_case("DecisionValue") {
                    if probs_norm.len() == 1 {
                        return Value::Real(probs_norm[0]);
                    }
                    let mut assoc: HashMap<String, Value> = HashMap::new();
                    for (c, p) in classes.iter().zip(probs_norm.iter()) {
                        assoc.insert(pretty_key(c), Value::Real(*p));
                    }
                    return Value::Assoc(assoc);
                }
                // Default label: argmax probability
                if let Some((idx, _)) = probs_norm
                    .iter()
                    .enumerate()
                    .max_by(|a, b| a.1.partial_cmp(b.1).unwrap_or(std::cmp::Ordering::Equal))
                {
                    return classes.get(idx).cloned().unwrap_or(Value::Symbol("Null".into()));
                }
            ...
            if output.eq_ignore_ascii_case("Probabilities") {
                ...
                if let (Some(classes), Some(counts)) = (st.classes.clone(), st.class_counts.clone())
                {
                    ...
                    for c in classes {
                        let k = pretty_key(&c);
                        let p = counts.get(&k).map(|v| (*v as f64) / total).unwrap_or(0.0);
                        probs.insert(k, Value::Real(p));
                    }
                }
                Value::Assoc(probs)
            } else {
                st.majority.clone().unwrap_or(Value::Symbol("Null".into()))
            }
        }
        "regression" => {
            let output = get_option(&opts_map, "Output")
            ...
            if st.method.eq_ignore_ascii_case("Linear") {
                ...
                let w = st.lin_w.clone().unwrap_or(vec![]);
                let b = st.lin_b.unwrap_or(0.0);
                let xv = featurize(&pre, &x);
                let yhat = dot(&w, &xv) + b;
                if output.eq_ignore_ascii_case("PredictionInterval") {
                    let cl = get_option(&opts_map, "ConfidenceLevel")
                        .and_then(|v| as_f64(v))
                        .unwrap_or(0.9);
                    let z = z_for_conf(cl);
...
fn ml_property(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("MLProperty".into())), args };
    }
    ...
    let model_v = if get_model_id(&arg0).is_some() {
        arg0.clone()
    } else {
        match &arg0 {
            Value::PureFunction { body, .. } => match &**body {
                Value::Expr { head, args }
                    if matches!(&**head, Value::Symbol(s) if s=="MLApply") && !args.is_empty() =>
                {
                    args[0].clone()
                }
                _ => arg0.clone(),
    ...
    let id = match get_model_id(&model_v) {
        Some(id) => id,
        None => return Value::Expr { head: Box::new(Value::Symbol("MLProperty".into())), args },
    };
    let reg = ml_reg().lock().unwrap();
    let st = match reg.get(&id) {
        Some(s) => s.clone(),
        ...
        Value::String(s) | Value::Symbol(s) => {
            ...
            match key.as_str() {
                "properties" => Value::List(
                    vec!["Method", "Task", "TrainingSize", "Classes"]
                        .into_iter()
...
fn classify_measurements(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() != 2 {
        return Value::Expr { head: Box::new(Value::Symbol("ClassifyMeasurements".into())), args };
    }
    ...
    let cm_rows: Vec<Value> = cm
        .into_iter()
        .map(|(tk, row)| {
            let mut inner: HashMap<String, Value> = HashMap::new();
            for (pk, c) in row {
                inner.insert(pk, Value::Integer(c));
            }
...
fn predict_measurements(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() != 2 {
        return Value::Expr { head: Box::new(Value::Symbol("PredictMeasurements".into())), args };
    }
...
fn ml_cross_validate(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("MLCrossValidate".into())), args };
    }
    ...
    let metric = get_option(&opts, "Metric")
        .and_then(
            ...
            for it in items {
                if let Value::Expr { head, args } = it {
                    if matches!(*head, Value::Symbol(ref s) if s=="Rule") && args.len() == 2 {
                        ...
                        if let Some(yf) = as_f64(&yv) {
                            pairs_reg.push((args[0].clone(), yf));
                        } else {
                            pairs_cls.push((args[0].clone(), yv));
                        }
                    }
                }
            }
    ...
    for (train_idx, valid_idx) in folds {
        if !pairs_reg.is_empty() {
            let tr: Vec<Value> = train_idx
                .iter()
                .map(|&i| {
                    Value::expr(
                        Value::symbol("Rule"),
                        vec![pairs_reg[i].0.clone(), Value::Real(pairs_reg[i].1)],
                    )
                })
                .collect();
            let model = ev.eval(Value::expr(
                Value::symbol("Predict"),
                vec![
                    Value::List(tr.clone()),
                    Value::assoc(vec![
                        ("Method", Value::String("Linear".into())),
                        ("Epochs", Value::Integer(epochs_opt as i64)),
                    ]),
                ],
            ));
            let valid: Vec<Value> = valid_idx
                .iter()
                .map(|&i| {
                    Value::expr(
                        Value::symbol("Rule"),
            ...
            if let Value::Assoc(m) = meas {
                if let Some(Value::Real(rmse)) = m.get("RMSE") {
                    scores.push(*rmse);
                }
            }
        } else {
            let tr: Vec<Value> = train_idx
                ...
                .map(|&i| {
                    Value::expr(
                        Value::symbol("Rule"),
                        vec![pairs_cls[i].0.clone(), pairs_cls[i].1.clone()],
                    )
                })
                .collect();
            let model = ev.eval(Value::expr(
                Value::symbol("Classify"),
                vec![
                    Value::List(tr.clone()),
                    Value::assoc(vec![
                        ("Method", Value::String("Logistic".into())),
                        ("Epochs", Value::Integer(epochs_opt as i64)),
                    ]),
                ],
            ));
            ...
            if let Value::Assoc(m) = meas {
                if let Some(Value::Real(acc)) = m.get("Accuracy") {
                    scores.push(*acc);
                }
            }
        }
...
fn ml_tune(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("MLTune".into())), args };
    }
    let (pos, opts) = parse_opts(ev, &args);
    if pos.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("MLTune".into())), args };
    ...
    let l2_vals: Vec<f64> = match search {
        Value::Assoc(m) => m
            .get("L2")
            .and_then(|v| match v {
                Value::List(xs) => Some(xs.iter().filter_map(|x| as_f64(x)).collect()),
                _ => None,
            })
            .unwrap_or(vec![0.0, 1e-2, 1e-1]),
        _ => vec![0.0],
    };
    let k = get_option(&opts, "K").and_then(|v| as_f64(v)).unwrap_or(3.0) as usize;
    let epochs_opt = get_option(&opts, "Epochs").and_then(|v| as_f64(v)).unwrap_or(30.0) as usize;
    let mut best_score = std::f64::INFINITY;
    let mut best_model = Value::Symbol("Null".into());
    let mut report: Vec<Value> = Vec::new();
    match train {
        Value::List(items) => {
            // Decide task by target type in first item
            let mut is_reg = false;
            if let Some(Value::Expr { head, args }) = items.first() {
                if matches!(&**head, Value::Symbol(s) if s=="Rule") && args.len() == 2 {
                    is_reg = as_f64(&ev.eval(args[1].clone())).is_some();
                }
            }
            for l2 in l2_vals {
                if is_reg {
                    ...
                    if let Value::Assoc(m) = cv {
                        if let Some(Value::Real(s)) = m.get("score") {
                            score = *s;
                        }
                    }
                    report.push(Value::assoc(vec![
                        ("L2", Value::Real(l2)),
...
pub fn register_ml_filtered(ev: &mut Evaluator, pred: &dyn Fn(&str) -> bool) {
    ...
    register_if(
        ...
        classify_measurements as NativeFn,
        Attributes::empty(),
    );
    register_if(
        ev,
        pred,
        "PredictMeasurements",
        predict_measurements as NativeFn,
        Attributes::empty(),
    );
