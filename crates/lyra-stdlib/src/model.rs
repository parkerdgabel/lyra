...
fn model(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("Model".into())), args };
    }
    match &args[0] {
        Value::String(s) | Value::Symbol(s) => {
            let mut m = HashMap::new();
            m.insert("Id".into(), Value::String(s.clone()));
            let provider = if let Some((p, _)) = s.split_once(":") {
                p.to_string()
            } else {
                "mock".to_string()
            };
...
fn models_list(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    Value::List(vec![Value::Assoc(HashMap::from([
...
fn extract_messages(m: &Value) -> Vec<(String, String)> {
    match m {
        Value::List(items) => items
            .iter()
            .filter_map(|it| {
                if let Value::Assoc(a) = it {
                    let role = a.get("role").and_then(str_of)?;
                    let content = a.get("content").and_then(str_of)?;
                    Some((role, content))
                } else {
                    None
                }
...
pub fn chat(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("Chat".into())), args };
    }
    ...
    let mode = ev
        .get_env("ModelsMode")
        .and_then(|v| if let Value::String(s) | Value::Symbol(s) = v { Some(s) } else { None })
        .unwrap_or_else(|| "mock".into());
    if mode == "auto" && model_id.starts_with("openai:") {
        if let Some(res) = chat_openai(&model_id, &messages) {
            return res;
        }
    }
    // Mock behavior: echo last user content or compose a trivial response
    let mut response = String::new();
    if let Some((_, last)) = messages.iter().rev().find(|(r, _)| r == "user") {
    ...
    let span_start = Value::Expr {
        ...
        args: vec![
            ...
            Value::Assoc(HashMap::from([
                ...
                (
                    "TokensIn".into(),
                    Value::Integer(messages.iter().map(|(_, c)| c.len() as i64).sum()),
                ),
            ])),
        ],
    };
    ...
    if stream {
        ...
        for t in tokens {
            let _ = ev.eval(Value::Expr {
                head: Box::new(Value::Symbol("Puts".into())),
                args: vec![Value::String(t.to_string())],
            });
        }
    }
    ...
    let _ = ev.eval(Value::Expr {
        head: Box::new(Value::Symbol("CostAdd".into())),
        args: vec![Value::Real(0.0)],
    });
...
fn complete(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("Complete".into())), args };
    }
    let mut prompt = String::new();
    // Accept Prompt in assoc or as direct string
    if args.len() == 1 {
        if let Some(s) = str_of(&args[0]) {
            prompt = s;
        }
    }
    for a in &args {
    ...
    let _ = ev.eval(Value::Expr {
        head: Box::new(Value::Symbol("Span".into())),
        args: vec![Value::String("model:Complete".into())],
    });
...
pub fn embed(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    #[cfg(feature = "net_https")]
    if mode == "auto" {
        if let Some(v) = openai_embed(&texts) {
            return v;
        }
    }
    ...
    Value::List(
        texts
            ...
            .map(|t| {
                ...
                Value::List(vec![
                    Value::Real(n as f64),
                    Value::Real((n % 7) as f64),
                    Value::Real(((n * 3) % 11) as f64),
                ])
            })
            .collect(),
    )
