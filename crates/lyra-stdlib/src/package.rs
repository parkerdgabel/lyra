...
pub fn register_package(ev: &mut Evaluator) {
    ...
    ev.register(
        ...
        list_installed_packages_fn as NativeFn,
        Attributes::empty(),
    );
...
fn set_env(ev: &mut Evaluator, name: &str, v: Value) {
    let _ = ev.eval(Value::Expr {
        ...
        args: vec![Value::Symbol(name.into()), v],
    });
}

fn default_package_path() -> Vec<String> {
    vec!["./.lyra/packages".to_string(), "./packages".to_string()]
}
...
fn write_loaded_packages(ev: &mut Evaluator, m: std::collections::HashMap<String, Value>) {
    set_env(ev, "$LoadedPackages", Value::Assoc(m));
}
...
fn write_module_exports(ev: &mut Evaluator, names: Vec<String>) {
    set_env(ev, "$ModuleExports", Value::List(names.into_iter().map(Value::String).collect()));
}
...
fn unset_var(ev: &mut Evaluator, name: &str) {
    let _ = ev.eval(Value::Expr {
        head: Box::new(Value::Symbol("Unset".into())),
        args: vec![Value::Symbol(name.into())],
    });
}
...
fn write_pkg_exports(ev: &mut Evaluator, m: std::collections::HashMap<String, Vec<String>>) {
    ...
    for (k, vs) in m {
        out.insert(k, Value::List(vs.into_iter().map(Value::String).collect()));
    }
...
fn using_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    let (name, _ver_req) = match ev.eval(args[0].clone()) {
        ...
        Value::Assoc(m) => {
            ...
            match nm {
                Some(s) => (
                    s,
                    m.get("version").or_else(|| m.get("Version")).and_then(|v| {
    ...
    let pkg_path = match found {
        Some(p) => p,
        None => {
            return failure(
                "Package::notFound",
                &format!("Package '{}' not found on $PackagePath", name),
            )
        }
    };
    ...
    let import_spec = if args.len() >= 2 {
        match ev.eval(args[1].clone()) {
            Value::Assoc(m) => {
                ...
                if let Some(v) = m.get("Import").or_else(|| m.get("import")) {
                    spec.insert("import".into(), v.clone());
                }
                if let Some(v) = m.get("Except").or_else(|| m.get("except")) {
                    spec.insert("except".into(), v.clone());
                }
                Value::Assoc(spec)
            }
                ...
                (Some(Value::Symbol(s)), _) | (Some(Value::String(s)), _) if s == "All" => {
                    if let Some(exports) = exports_opt {
                        ...
                        if let Some(Value::List(exl)) = exc {
                            let mut exset: std::collections::HashSet<String> =
                                std::collections::HashSet::new();
                            for v in exl {
                                if let Value::String(s) = v {
                                    exset.insert(s.clone());
                                } else if let Value::Symbol(s) = v {
                                    exset.insert(s.clone());
                                }
                            }
                        ...
                        if let Some(Value::List(vs)) = exc {
                            a.insert("except".into(), Value::List(vs.clone()));
                        }
                        Value::Assoc(a)
                    }
                }
...
fn unuse_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return uneval("Unuse", args);
    }
    let name = match ev.eval(args[0].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => return uneval("Unuse", vec![other]),
    };
        ...
        match v {
            ...
            Value::Assoc(m) => {
                ...
                if let Some(Value::Boolean(true)) = m.get("all") {
                    let except: std::collections::HashSet<String> = m
                        .get("except")
                        .and_then(|v| match v {
                            Value::List(vs) => Some(vs.clone()),
                            _ => None,
                        })
                        .map(|vs| {
                            vs.into_iter()
...
fn reload_package_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return uneval("ReloadPackage", args);
    }
    let name = match ev.eval(args[0].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => return uneval("ReloadPackage", vec![other]),
    };
...
fn with_package_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() != 2 {
        return uneval("WithPackage", args);
    }
    let name = match ev.eval(args[0].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => return uneval("WithPackage", vec![other, args[1].clone()]),
    };
...
fn begin_module_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return uneval("BeginModule", args);
    }
    let name = match ev.eval(args[0].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => return uneval("BeginModule", vec![other]),
    };
...
fn export_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return uneval("Export", args);
    }
    let mut exports = get_module_exports(ev);
    match ev.eval(args[0].clone()) {
        Value::List(vs) => {
            for v in vs {
                if let Value::Symbol(s) | Value::String(s) = v {
                    exports.push(s);
                }
            }
        }
...
fn private_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    // Placeholder: no-op for now
    Value::Boolean(true)
}

fn current_module_fn(ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    get_env(ev, "$ModuleContext")
}
...
fn set_module_path_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return uneval("SetModulePath", args);
    }
...
fn package_info_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    let name_opt = args.get(0).map(|v| ev.eval(v.clone())).and_then(|v| match v {
        ...
        for base in paths {
            ...
            if p.is_dir() {
                ...
                if p.join("lyra.toml").exists() {
                    if let Some(ver) = read_manifest_version(&p.join("lyra.toml")) {
                        assoc.insert("version".into(), Value::String(ver));
                    }
                }
...
fn package_version_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() != 1 {
        return uneval("PackageVersion", args);
    }
...
fn imported_symbols_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() == 1 {
        let name = match ev.eval(args[0].clone()) {
            Value::String(s) | Value::Symbol(s) => s,
            _ => return uneval("ImportedSymbols", args),
        };
        if let Value::Assoc(m) = get_env(ev, "$ImportedSymbols") {
            return m.get(&name).cloned().unwrap_or(Value::List(vec![]));
        }
        Value::List(vec![])
    } else if args.is_empty() {
        get_env(ev, "$ImportedSymbols")
    } else {
        uneval("ImportedSymbols", args)
    }
}

fn loaded_packages_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if !args.is_empty() {
        return uneval("LoadedPackages", args);
    }
...
fn register_exports_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.is_empty() {
        return uneval("RegisterExports", args);
    }
    let (name, ex_list) = match ev.eval(args[0].clone()) {
        Value::Assoc(m) => {
            ...
            match (nm, ex) {
                (Some(Value::String(n)) | Some(Value::Symbol(n)), Some(v)) => {
                    (n.clone(), v.clone())
                }
                _ => return uneval("RegisterExports", vec![Value::Assoc(m)]),
            }
        }
        Value::String(s) | Value::Symbol(s) => {
            if args.len() < 2 {
...
fn package_exports_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() == 1 {
        let name = match ev.eval(args[0].clone()) {
            Value::String(s) | Value::Symbol(s) => s,
            _ => return uneval("PackageExports", args),
        };
        if let Some(vs) = m.get(&name) {
            return Value::List(vs.iter().cloned().map(Value::String).collect());
        }
...
fn new_package_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return uneval("NewPackage", args);
    }
    let name = match ev.eval(args[0].clone()) {
        Value::String(s) | Value::Symbol(s) => s,
        other => return uneval("NewPackage", vec![other]),
    };
    let path = PathBuf::from(&name);
    let (dir_path, pkg_name_opt) = if path.components().count() > 1 {
        (path, None)
    } else {
        (PathBuf::from(&name), Some(name.clone()))
    };
    if let Err(e) = fs::create_dir_all(dir_path.join("src")) {
        return failure("Package::scaffold", &e.to_string());
    }
    // Write minimal manifest
    let manifest_path = dir_path.join("lyra.toml");
    let manifest = if let Some(ref n) = pkg_name_opt {
        format!("[package]\nname = \"{}\"\nversion = \"0.1.0\"\n", n)
    } else {
        "[package]\nversion = \"0.1.0\"\n".into()
    };
...
fn new_module_fn(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return uneval("NewModule", args);
    }
    let pkg_path = match ev.eval(args[0].clone()) {
        Value::String(s) | Value::Symbol(s) => PathBuf::from(s),
        other => return uneval("NewModule", vec![other, args[1].clone()]),
    };
    let name = match ev.eval(args[1].clone()) {
        ...
        other => {
            return uneval(
                "NewModule",
                vec![Value::String(pkg_path.to_string_lossy().into()), other],
            )
        }
    };
        ...
        for line in s.lines() {
            ...
            if l.starts_with("version") {
                if let Some(idx) = l.find('=') {
                    return Some(l[idx + 1..].trim().trim_matches('"').to_string());
                }
            }
        }
    }
...
fn pm_na(fun: &str) -> Value {
    failure("PackageManagerNotAvailable", &format!("{} requires lyra-pm; not yet available", fun))
}

fn build_package_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("BuildPackage")
}
fn test_package_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("TestPackage")
}
fn lint_package_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("LintPackage")
}
fn pack_package_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("PackPackage")
}
fn generate_sbom_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("GenerateSBOM")
}
fn sign_package_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("SignPackage")
}
fn publish_package_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("PublishPackage")
}
fn install_package_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("InstallPackage")
}
fn update_package_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("UpdatePackage")
}
fn remove_package_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("RemovePackage")
}
fn login_registry_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("LoginRegistry")
}
fn logout_registry_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("LogoutRegistry")
}
fn whoami_registry_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("WhoAmI")
}
fn package_audit_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("PackageAudit")
}
fn package_verify_fn(_ev: &mut Evaluator, _args: Vec<Value>) -> Value {
    pm_na("PackageVerify")
}
