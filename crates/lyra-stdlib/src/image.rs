...
fn encode_image(
    ...
    match fmt.to_lowercase().as_str() {
        ...
        "jpeg" | "jpg" => {
            let q = opts
                .get("quality")
                .or_else(|| opts.get("Quality"))
                .and_then(|v| if let Value::Integer(i) = v { Some(*i as u8) } else { None })
                .unwrap_or(85);
            img.write_to(&mut std::io::Cursor::new(&mut buf), image::ImageOutputFormat::Jpeg(q))
                .map_err(|e| e.to_string())?;
        }
        // WebP encoding support in image may vary; not guaranteed. Return error for now.
        "webp" => {
            return Err("WebP encode not supported yet".into());
        }
        other => {
...
fn wrap_bytes_output(
    ...
    let enc = opts
        .and_then(|m| {
            m.get("Output").and_then(|v| {
                if let Value::Assoc(mm) = v {
                    mm.get("Encoding").cloned()
                } else {
                    None
                }
            })
...
fn image_decode(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("ImageDecode".into())), args };
    }
    let img = match decode_image(ev, args[0].clone()) {
        Ok(i) => i,
        Err(e) => return failure("Image::decode", &format!("ImageDecode: {}", e)),
    };
    ...
    if fmt == "raw" {
        ...
        match encode_image(
            &img,
            &std::collections::HashMap::from([(String::from("format"), Value::String(fmt))]),
        ) {
...
fn image_from_raw_assoc(
    ...
    let h = m
        .get("height")
        .or_else(|| m.get("Height"))
        .and_then(|v| if let Value::Integer(i) = v { Some(*i as u32) } else { None })
        .ok_or_else(|| "Missing height".to_string())?;
    let ch = m
        .get("channels")
        .or_else(|| m.get("Channels"))
        .and_then(|v| if let Value::String(s) = v { Some(s.to_lowercase()) } else { None })
        .unwrap_or_else(|| "rgba8".into());
    if ch != "rgba8" {
        return Err("Only rgba8 supported".into());
    }
    let bytes_s = m
        .get("bytes")
        .or_else(|| m.get("Bytes"))
    ...
    let enc = m
        .get("encoding")
        .or_else(|| m.get("Encoding"))
        .and_then(|v| if let Value::String(s) = v { Some(s.to_lowercase()) } else { None })
        .unwrap_or_else(|| "base64url".into());
    let raw = match enc.as_str() {
        "base64url" | "base64" | "b64" => base64url_decode(&bytes_s),
        _ => Err("Unsupported raw encoding".into()),
    }?;
    if raw.len() != (w as usize) * (h as usize) * 4 {
        return Err("Raw length mismatch".into());
    }
    let img =
        image::RgbaImage::from_raw(w, h, raw).ok_or_else(|| "Invalid raw buffer".to_string())?;
...
fn image_encode(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("ImageEncode".into())), args };
    }
    let enc_opts = match ev.eval(args[1].clone()) {
        Value::Assoc(m) => m,
        _ => std::collections::HashMap::new(),
    };
    // Input may be raw assoc or another image input
    let img = match ev.eval(args[0].clone()) {
        Value::Assoc(m) => match image_from_raw_assoc(&m) {
            Ok(i) => i,
            Err(_) => match decode_image(ev, Value::Assoc(m)) {
                Ok(i) => i,
                Err(e) => return failure("Image::decode", &format!("ImageEncode: {}", e)),
            },
        },
        other => match decode_image(ev, other) {
            Ok(i) => i,
            Err(e) => return failure("Image::decode", &format!("ImageEncode: {}", e)),
        },
    };
...
fn image_info(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() {
        return Value::Expr { head: Box::new(Value::Symbol("ImageInfo".into())), args };
    }
...
fn image_canvas(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() != 1 {
        return Value::Expr { head: Box::new(Value::Symbol("ImageCanvas".into())), args };
    }
    let opts = match &args[0] {
        Value::Assoc(m) => m,
        _ => return Value::Expr { head: Box::new(Value::Symbol("ImageCanvas".into())), args },
    };
    let w = opts
        .get("width")
    ...
    match encode_image(
        &dynimg,
        &HashMap::from([(String::from("format"), Value::String(String::from("png")))]),
    ) {
        Ok(bytes) => Value::String(base64url_encode(&bytes)),
...
fn image_resize(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("ImageResize".into())), args };
    }
    ...
    let fit = opts
        .get("fit")
        .or_else(|| opts.get("Fit"))
        .and_then(|v| if let Value::String(s) = v { Some(s.to_lowercase()) } else { None })
        .unwrap_or_else(|| "contain".into());
    let w_opt = opts.get("width").or_else(|| opts.get("Width")).and_then(|v| {
    ...
    let h_opt = opts.get("height").or_else(|| opts.get("Height")).and_then(|v| {
        if let Value::Integer(i) = v {
            Some(*i as u32)
        } else {
            None
    ...
    let kernel = opts
        .get("kernel")
        .or_else(|| opts.get("Kernel"))
        .and_then(|v| if let Value::String(s) = v { Some(s.to_lowercase()) } else { None })
        .unwrap_or_else(|| "lanczos3".into());
    let filter = match kernel.as_str() {
        "nearest" => image::imageops::FilterType::Nearest,
        "mitchell" => image::imageops::FilterType::CatmullRom,
        _ => image::imageops::FilterType::Lanczos3,
    };
    let img = match decode_image(ev, args[0].clone()) {
        Ok(i) => i,
        Err(e) => return failure("Image::decode", &format!("ImageResize: {}", e)),
    };
    let (ow, oh) = img.dimensions();
    let target_w = w_opt.unwrap_or_else(|| {
        if let Some(h) = h_opt {
            ((ow as f32) * (h as f32 / oh as f32)) as u32
        } else {
            ow
        }
    });
    let target_h = h_opt.unwrap_or_else(|| {
        if let Some(w) = w_opt {
            ((oh as f32) * (w as f32 / ow as f32)) as u32
        } else {
            oh
        }
    });
    let out_img = if fit == "cover" {
        ...
        image::DynamicImage::ImageRgba8(
            image::imageops::crop_imm(&resized, x, y, target_w.min(rw), target_h.min(rh))
                .to_image(),
        )
    } else {
        // contain
...
fn image_convert(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("ImageConvert".into())), args };
    }
    let img = match decode_image(ev, args[0].clone()) {
        Ok(i) => i,
        Err(e) => return failure("Image::decode", &format!("ImageConvert: {}", e)),
    };
...
fn image_thumbnail(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("ImageThumbnail".into())), args };
    }
    ...
    let (tw, th, mut opts) = match ev.eval(args[1].clone()) {
        ...
        Value::Assoc(m) => {
            let w = m
                ...
                .and_then(|v| if let Value::Integer(i) = v { Some(*i as u32) } else { None })
                .or_else(|| {
                    m.get("width").or_else(|| m.get("Width")).and_then(|v| {
                        if let Value::Integer(i) = v {
                            Some(*i as u32)
                        } else {
                            None
            ...
            let h = m
                ...
                .and_then(|v| if let Value::Integer(i) = v { Some(*i as u32) } else { None })
                .or_else(|| {
                    m.get("height").or_else(|| m.get("Height")).and_then(|v| {
                        if let Value::Integer(i) = v {
                            Some(*i as u32)
                        } else {
                            None
...
fn image_transform(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("ImageTransform".into())), args };
    }
    let mut img = match decode_image(ev, args[0].clone()) {
        Ok(i) => i,
        Err(e) => return failure("Image::decode", &format!("ImageTransform: {}", e)),
    };
    let pipeline = match ev.eval(args[1].clone()) {
        Value::Assoc(m) => m,
        _ => std::collections::HashMap::new(),
    };
        ...
        for opv in ops {
            if let Value::Assoc(op) = opv {
                let name = op
                    ...
                    .or_else(|| op.get("Op"))
                    .and_then(
                        |v| if let Value::String(s) = v { Some(s.to_lowercase()) } else { None },
                    )
                    .unwrap_or_default();
                let args_map = op
                    .get("args")
                ...
                match name.as_str() {
                    "resize" => {
                        ...
                        let fit = args_map
                            ...
                            .unwrap_or_else(|| "contain".into());
                        let w_opt =
                            args_map.get("width").or_else(|| args_map.get("Width")).and_then(|v| {
                                if let Value::Integer(i) = v {
                                    Some(*i as u32)
                                } else {
                                    None
                        ...
                        let h_opt =
                            args_map.get("height").or_else(|| args_map.get("Height")).and_then(
                                |v| if let Value::Integer(i) = v { Some(*i as u32) } else { None },
                            );
                        let (ow, oh) = img.dimensions();
                        let target_w = w_opt.unwrap_or_else(|| {
                            if let Some(h) = h_opt {
                        ...
                        img = if fit == "cover" {
                            let scale =
                                f32::max(target_w as f32 / ow as f32, target_h as f32 / oh as f32);
                            let rw = ((ow as f32) * scale).round().max(1.0) as u32;
                            let rh = ((oh as f32) * scale).round().max(1.0) as u32;
                            let resized = img.resize(rw, rh, filter);
                            let x = if rw > target_w { (rw - target_w) / 2 } else { 0 };
                            ...
                            let scale =
                                f32::min(target_w as f32 / ow as f32, target_h as f32 / oh as f32);
                            let rw = ((ow as f32) * scale).round().max(1.0) as u32;
                            let rh = ((oh as f32) * scale).round().max(1.0) as u32;
                            img.resize(rw, rh, filter)
                        };
                    ...
                    "flip" => {
                        let horiz = args_map
                            .get("horizontal")
                            .or_else(|| args_map.get("Horizontal"))
                            .and_then(|v| if let Value::Boolean(b) = v { Some(*b) } else { None })
                            .unwrap_or(false);
                        let vert = args_map
                            .get("vertical")
                            .or_else(|| args_map.get("Vertical"))
                            .and_then(|v| if let Value::Boolean(b) = v { Some(*b) } else { None })
                            .unwrap_or(false);
                        if horiz {
                            img = img.fliph();
    ...
    let enc_opts = pipeline
        .get("encoding")
        .or_else(|| pipeline.get("Encoding"))
        .and_then(|v| if let Value::Assoc(m) = v { Some(m.clone()) } else { None })
        .unwrap_or_default();
    match encode_image(&img, &enc_opts) {
...
fn image_crop(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("ImageCrop".into())), args };
    }
    let img = match decode_image(ev, args[0].clone()) {
        Ok(i) => i,
        Err(e) => return failure("Image::decode", &format!("ImageCrop: {}", e)),
    };
    let opts = match ev.eval(args[1].clone()) {
        Value::Assoc(m) => m,
        _ => std::collections::HashMap::new(),
    ...
    let out_img = if let Some(Value::Assoc(r)) = opts.get("rect").or_else(|| opts.get("Rect")) {
        let x = r
            ...
            .unwrap_or(0)
            .clamp(0, ow as i64) as u32;
        let y = r
            .get("y")
            .or_else(|| r.get("Y"))
            .and_then(|v| if let Value::Integer(i) = v { Some(*i as i64) } else { None })
            .unwrap_or(0)
            .clamp(0, oh as i64) as u32;
        let w = r
            .get("width")
            .or_else(|| r.get("Width"))
            .and_then(|v| if let Value::Integer(i) = v { Some(*i as i64) } else { None })
            .unwrap_or(ow as i64)
            .clamp(1, ow as i64 - x as i64) as u32;
        let h = r
            .get("height")
            .or_else(|| r.get("Height"))
            .and_then(|v| if let Value::Integer(i) = v { Some(*i as i64) } else { None })
            .unwrap_or(oh as i64)
            .clamp(1, oh as i64 - y as i64) as u32;
...
fn image_pad(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("ImagePad".into())), args };
    }
    let img = match decode_image(ev, args[0].clone()) {
        Ok(i) => i,
        Err(e) => return failure("Image::decode", &format!("ImagePad: {}", e)),
    };
    let opts = match ev.eval(args[1].clone()) {
        Value::Assoc(m) => m,
        _ => std::collections::HashMap::new(),
    };
    let tw = opts.get("width").or_else(|| opts.get("Width")).and_then(|v| {
        if let Value::Integer(i) = v {
            Some(*i as u32)
        } else {
            None
    ...
    let th = opts.get("height").or_else(|| opts.get("Height")).and_then(|v| {
        if let Value::Integer(i) = v {
            Some(*i as u32)
        } else {
            None
...
fn image_save(ev: &mut Evaluator, args: Vec<Value>) -> Value {
    ...
    if args.len() < 2 {
        return Value::Expr { head: Box::new(Value::Symbol("ImageSave".into())), args };
    }
    let img = match decode_image(ev, args[0].clone()) {
        Ok(i) => i,
        Err(e) => return failure("Image::decode", &format!("ImageSave: {}", e)),
    };
    let (out_path, mkdirs) = match ev.eval(args[1].clone()) {
        Value::String(s) | Value::Symbol(s) => (s, false),
        Value::Assoc(m) => {
            let p = m.get("path").or_else(|| m.get("Path")).and_then(|v| {
                if let Value::String(s) = v {
                    Some(s.clone())
                } else {
                    None
            ...
            let mk = m
                .get("mkdirs")
                .or_else(|| m.get("Mkdirs"))
                .and_then(|v| if let Value::Boolean(b) = v { Some(*b) } else { None })
                .unwrap_or(false);
            match p {
                Some(pp) => (pp, mk),
                None => return failure("Image::save", "Missing output path"),
            }
        }
    ...
    if !enc_opts.contains_key("format") && !enc_opts.contains_key("Format") {
        if let Some(fmt) = infer_format_from_ext(&out_path) {
            enc_opts.insert("format".into(), Value::String(fmt));
        }
    }
    let bytes = match encode_image(&img, &enc_opts) {
        Ok(b) => b,
        Err(e) => return failure("Image::encode", &format!("ImageSave: {}", e)),
    };
