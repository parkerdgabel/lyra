...
fn sess() -> &'static Mutex<HashMap<String, Vec<Value>>> { SESS.get_or_init(|| Mutex::new(HashMap::new())) }
...
fn key_of(v: &Value) -> Option<String> { match v { Value::String(s)|Value::Symbol(s)=>Some(s.clone()), _=>None } }

// Remember[session_id, data]
fn remember(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len()!=2 { return Value::Expr { head: Box::new(Value::Symbol("Remember".into())), args } }
    let sid = match key_of(&args[0]) { Some(s)=>s, None=>return Value::Boolean(false) };
...
fn recall(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.is_empty() { return Value::Expr { head: Box::new(Value::Symbol("Recall".into())), args } }
    let sid = match key_of(&args[0]) { Some(s)=>s, None=>return Value::List(vec![]) };
    let mut query: Option<String> = None;
    let mut k: usize = 5;
    for a in &args[1..] {
        match a {
            Value::String(s)|Value::Symbol(s) => { query = Some(s.clone()); }
            Value::Assoc(m) => { if let Some(Value::Integer(n)) = m.get("K") { if *n > 0 { k = *n as usize; } } }
            _=>{}
        }
    }
    ...
    if let Some(q) = query {
        out = out.into_iter().filter(|v| match v { Value::String(s)=>s.contains(&q), Value::Assoc(am)=>am.values().any(|vv| matches!(vv, Value::String(ss) if ss.contains(&q))), _=>false }).collect();
    }
    if out.len() > k { out = out.into_iter().rev().take(k).rev().collect(); }
    Value::List(out)
}

fn session_clear(_ev: &mut Evaluator, args: Vec<Value>) -> Value {
    if args.len()!=1 { return Value::Expr { head: Box::new(Value::Symbol("SessionClear".into())), args } }
    let sid = match key_of(&args[0]) { Some(s)=>s, None=>return Value::Boolean(false) };
    let mut m = sess().lock().unwrap();
    m.remove(&sid);
    Value::Boolean(true)
