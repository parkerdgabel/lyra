...
fn matchq_and_patternq() {
    ...
    let r1 = eval_v(
        &mut ev,
        Value::expr(Value::Symbol("MatchQ".into()), vec![Value::Integer(1), pat.clone()]),
    );
    assert_eq!(r1, "True");
    // PatternQ[_Integer]
    ...
    let r3 = eval_v(
        &mut ev,
        Value::expr(Value::Symbol("PatternQ".into()), vec![Value::Symbol("x_Integer".into())]),
    );
    assert_eq!(r3, "True");
    // Non-pattern
    let r4 = eval_v(
    ...
    let pt = Value::expr(
        ...
        vec![
            Value::expr(Value::Symbol("Blank".into()), vec![Value::Symbol("Integer".into())]),
            Value::Symbol("EvenQ".into()),
        ],
    );
    let r5 =
        eval_v(&mut ev, Value::expr(Value::Symbol("MatchQ".into()), vec![Value::Integer(4), pt]));
    assert_eq!(r5, "True");
}
