...
fn gen_sine_raw(sr: u32, ch: u16, secs: f32, freq: f32) -> Value {
    ...
    for n in 0..frames {
        ...
        for _ in 0..ch { samples.extend_from_slice(&s16.to_le_bytes()); }
    }
    Value::Assoc([
...
fn media_pipeline_filter_graph_audio_resample() {
    ...
    let out = ev.eval(Value::expr(Value::Symbol("MediaPipeline".into()), vec![
        Value::Assoc([
            ("inputs".into(), Value::List(vec![wav_b64.clone()])),
            ("filterGraph".into(), Value::Assoc([
                ("chains".into(), Value::List(vec![
                    Value::Assoc([
                        ("in".into(), Value::List(vec![Value::String("0:a".into())])),
                        ("filters".into(), Value::List(vec![
                            Value::Assoc([
                                ("op".into(), Value::String("aresample".into())),
                                ("args".into(), Value::String("sample_rate=12000".into())),
                            ].into_iter().collect()),
                        ])),
            ...
            ("output".into(), Value::Assoc([
                ("format".into(), Value::String("wav".into())),
            ].into_iter().collect())),
        ].into_iter().collect()),
    ]));
