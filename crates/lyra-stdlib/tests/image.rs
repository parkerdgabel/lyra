...
fn image_canvas_info_resize_convert_thumbnail_transform() {
    ...
    let png = ev.eval(Value::expr(
        Value::Symbol("ImageCanvas".into()),
        vec![Value::Assoc(
            [
    ...
    if let Value::Assoc(m) = info {
        ...
        panic!("ImageInfo invalid");
    }

    // Resize to 4x4 cover
    let r = ev.eval(Value::expr(
        Value::Symbol("ImageResize".into()),
        vec![
            png.clone(),
            Value::Assoc(
                [
    ...
    if let Value::Assoc(m) = r_info {
        assert!(matches!(m.get("width"), Some(Value::Integer(n)) if *n==4));
        assert!(matches!(m.get("height"), Some(Value::Integer(n)) if *n==4));
    } else {
        panic!("resize info");
    }

    // Convert to JPEG
    let jpg = ev.eval(Value::expr(
        Value::Symbol("ImageConvert".into()),
        vec![r.clone(), Value::String("jpeg".into())],
    ));
    let _ = b64str(&jpg); // ensure it returns a string
    let j_info = ev.eval(Value::expr(Value::Symbol("ImageInfo".into()), vec![jpg.clone()]));
    if let Value::Assoc(m) = j_info {
        assert!(matches!(m.get("width"), Some(Value::Integer(n)) if *n==4));
    } else {
        panic!("jpeg info");
    }
    ...
    // Thumbnail 2x2
    let th = ev.eval(Value::expr(
        Value::Symbol("ImageThumbnail".into()),
        vec![png.clone(), Value::Integer(2)],
    ));
    let t_info = ev.eval(Value::expr(Value::Symbol("ImageInfo".into()), vec![th]));
    if let Value::Assoc(m) = t_info {
        assert!(matches!(m.get("width"), Some(Value::Integer(n)) if *n==2));
        assert!(matches!(m.get("height"), Some(Value::Integer(n)) if *n==2));
    } else {
        panic!("thumb info");
    }

    // Transform pipeline: contain to 3x5
    let tx = ev.eval(Value::expr(
        Value::Symbol("ImageTransform".into()),
        vec![
            png,
            Value::Assoc(
                [(
    ...
    if let Value::Assoc(m) = txi {
        assert!(matches!(m.get("width"), Some(Value::Integer(n)) if *n<=3));
        assert!(matches!(m.get("height"), Some(Value::Integer(n)) if *n<=5));
    } else {
        panic!("transform info");
    }
}
...
fn image_decode_raw_and_encode_roundtrip() {
    ...
    // Decode to raw
    let raw = ev.eval(Value::expr(
        Value::Symbol("ImageDecode".into()),
        vec![
            base.clone(),
    ...
    let (w, h, bytes) = match &raw {
        Value::Assoc(m) => {
            let w = match m.get("width") {
                Some(Value::Integer(n)) => *n as usize,
                _ => panic!("missing width"),
            };
            let h = match m.get("height") {
                Some(Value::Integer(n)) => *n as usize,
                _ => panic!("missing height"),
            };
            let b = match m.get("bytes") {
...
fn image_crop_and_pad() {
    ...
    let crop_rect = ev.eval(Value::expr(
        Value::Symbol("ImageCrop".into()),
        vec![
            base.clone(),
            Value::Assoc(
                [(
    ...
    if let Value::Assoc(m) = info {
        assert!(matches!(m.get("width"), Some(Value::Integer(n)) if *n==4));
        assert!(matches!(m.get("height"), Some(Value::Integer(n)) if *n==3));
    } else {
        panic!("crop_rect info");
    }

    // Gravity crop to 6x4 northwest
    let crop_g = ev.eval(Value::expr(
        Value::Symbol("ImageCrop".into()),
        vec![
            base.clone(),
            Value::Assoc(
    ...
    if let Value::Assoc(m) = info2 {
        assert!(matches!(m.get("width"), Some(Value::Integer(n)) if *n==6));
        assert!(matches!(m.get("height"), Some(Value::Integer(n)) if *n==4));
    } else {
        panic!("crop_g info");
    }
    ...
    let pad = ev.eval(Value::expr(
        Value::Symbol("ImagePad".into()),
        vec![
            base.clone(),
            Value::Assoc(
                [
    ...
    if let Value::Assoc(m) = pinfo {
        assert!(matches!(m.get("width"), Some(Value::Integer(n)) if *n==10));
        assert!(matches!(m.get("height"), Some(Value::Integer(n)) if *n==10));
    } else {
        panic!("pad info");
    }

    // Pad with downscale to 4x4
    let pad_small = ev.eval(Value::expr(
        Value::Symbol("ImagePad".into()),
        vec![
            base,
            Value::Assoc(
    ...
    if let Value::Assoc(m) = psinfo {
        assert!(matches!(m.get("width"), Some(Value::Integer(n)) if *n==4));
        assert!(matches!(m.get("height"), Some(Value::Integer(n)) if *n==4));
    } else {
        panic!("pad small info");
    }
}
