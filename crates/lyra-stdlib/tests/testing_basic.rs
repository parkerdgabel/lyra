...
fn run_simple_suite(ev: &mut Evaluator) -> Value {
    ...
    let adds = Value::expr(Value::Symbol("AssertEqual".into()), vec![
    ...
    let case1 = Value::expr(Value::Symbol("TestCase".into()), vec![Value::String("adds numbers".into()), adds]);
    let case2 = Value::expr(Value::Symbol("TestCase".into()), vec![Value::String("assert true".into()), ok]);
    let suite = Value::expr(Value::Symbol("TestSuite".into()), vec![Value::String("Sample".into()), Value::list(vec![case1, case2])]);
...
fn failing_case_is_reported() {
    ...
    let adds_wrong = Value::expr(Value::Symbol("AssertEqual".into()), vec![
        Value::expr(Value::Symbol("Plus".into()), vec![Value::Integer(1), Value::Integer(2)]),
        Value::Integer(4),
    ]);
    let case = Value::expr(Value::Symbol("TestCase".into()), vec![Value::String("oops".into()), adds_wrong]);
    let suite = Value::expr(Value::Symbol("TestSuite".into()), vec![Value::String("S".into()), Value::list(vec![case])]);
    let run = Value::expr(Value::Symbol("TestRun".into()), vec![suite]);
    let v = ev.eval(run);
    let txt = format_value(&v);
...
fn assert_equal_with_tolerance() {
    ...
    let case = Value::expr(Value::Symbol("TestCase".into()), vec![
        Value::String("tol".into()),
        Value::expr(Value::Symbol("AssertEqual".into()), vec![Value::Real(1.000000001), Value::Real(1.0), opts]),
    ]);
    let suite = Value::expr(Value::Symbol("TestSuite".into()), vec![Value::String("S".into()), Value::list(vec![case])]);
    let run = Value::expr(Value::Symbol("TestRun".into()), vec![suite]);
    let v = ev.eval(run);
...
fn retries_recorded() {
    ...
    let case = Value::expr(Value::Symbol("TestCase".into()), vec![Value::String("fail".into()), body, opts]);
    let suite = Value::expr(Value::Symbol("TestSuite".into()), vec![Value::String("S".into()), Value::list(vec![case])]);
    let run = Value::expr(Value::Symbol("TestRun".into()), vec![suite]);
    let v = ev.eval(run);
    let txt = format_value(&v);
...
fn fixtures_and_hooks() {
    ...
    let teardown_x = Value::pure_function(Some(vec!["v".into()]), Value::expr(Value::Symbol("AssertEqual".into()), vec![Value::Symbol("v".into()), Value::Integer(41)]));
    let fx_all = Value::expr(Value::Symbol("Fixture".into()), vec![
        Value::String("x".into()), setup_x, teardown_x.clone(), Value::assoc(vec![("Scope", Value::String("all".into()))])
    ]);

    // Fixture y (each): setup returns 5; teardown no-op (Assert[True])
    let setup_y = Value::pure_function(Some(vec![]), Value::Integer(5));
    let teardown_y = Value::pure_function(Some(vec!["_".into()]), Value::expr(Value::Symbol("Assert".into()), vec![Value::Boolean(true)]));
    let fx_each = Value::expr(Value::Symbol("Fixture".into()), vec![
        Value::String("y".into()), setup_y, teardown_y, Value::assoc(vec![("Scope", Value::String("each".into()))])
    ]);

    // Case uses x and y: returns x + y + 1
    let body = Value::expr(Value::Symbol("Plus".into()), vec![
        Value::expr(Value::Symbol("Plus".into()), vec![Value::Symbol("x".into()), Value::Symbol("y".into())]),
        Value::Integer(1)
    ]);
    let case = Value::expr(Value::Symbol("TestCase".into()), vec![Value::String("uses fixtures".into()), body]);
    ...
    let suite = Value::expr(Value::Symbol("TestSuite".into()), vec![
...
fn assert_throws_tag_and_predicate() {
    ...
    let at_tag = Value::expr(Value::Symbol("AssertThrows".into()), vec![thrown.clone(), Value::String("boom".into())]);
