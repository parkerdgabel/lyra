...
fn eval_v(ev: &mut Evaluator, v: lyra_core::value::Value) -> String {
...
fn try_basic_and_default() {
    ...
    let s2 = eval_v(
        ...
        Value::expr(
            Value::Symbol("Try".into()),
            vec![Value::expr(Value::Symbol("Fail".into()), vec![Value::String("oops".into())])],
        ),
    );
...
fn onfailure_and_catch() {
    ...
    let handlers =
        Value::assoc(vec![("x", Value::String("hx".into())), ("_", Value::String("def".into()))]);
    let s2 = eval_v(
        &mut ev,
        Value::expr(
...
fn throw_and_finally() {
    ...
    let thrown = Value::expr(
        Value::Symbol("Throw".into()),
        vec![Value::String("oops".into()), Value::assoc(vec![("code", Value::Integer(1))])],
    );
...
fn catch_rules_list_and_assert_throws() {
    ...
    let rules = Value::list(vec![
        Value::expr(
            Value::Symbol("Rule".into()),
            vec![Value::String("a".into()), Value::String("ha".into())],
        ),
        Value::expr(
            Value::Symbol("Rule".into()),
            vec![Value::String("_".into()), Value::String("def".into())],
        ),
    ]);
    let out = eval_v(
        &mut ev,
        Value::expr(
            Value::Symbol("Catch".into()),
            vec![Value::expr(Value::Symbol("Fail".into()), vec![Value::String("a".into())]), rules],
        ),
    );
    assert_eq!(out, "\"ha\"");
