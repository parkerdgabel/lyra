...
fn crypto_aead_roundtrip() {
    ...
    let env = ev.eval(Value::expr(Value::Symbol("AeadEncrypt".into()), vec![pt.clone(), key.clone()]));
...
fn crypto_sign_verify() {
    ...
    let valid = ev.eval(Value::expr(Value::Symbol("Verify".into()), vec![msg.clone(), sig.clone(), kp.clone()]));
    assert_eq!(valid, Value::Boolean(true));
    let invalid = ev.eval(Value::expr(Value::Symbol("Verify".into()), vec![Value::String("tampered".into()), sig, kp]));
...
fn crypto_hmac() {
    ...
    let mac = ev.eval(Value::expr(Value::Symbol("Hmac".into()), vec![msg.clone(), key.clone(), Value::Assoc([
    ...
    // Verify good and bad
    let ok = ev.eval(Value::expr(Value::Symbol("HmacVerify".into()), vec![msg.clone(), Value::String("key".into()), Value::String(mac_hex.clone()), Value::Assoc([
        ("Alg".into(), Value::String("SHA-256".into())),
        ("InputEncoding".into(), Value::String("hex".into())),
    ].into_iter().collect())]));
    assert_eq!(ok, Value::Boolean(true));
    let bad = ev.eval(Value::expr(Value::Symbol("HmacVerify".into()), vec![msg, Value::String("key".into()), Value::String("00".into()), Value::Assoc([
        ("Alg".into(), Value::String("SHA-256".into())),
        ("InputEncoding".into(), Value::String("hex".into())),
    ].into_iter().collect())]));
...
fn crypto_hkdf_and_argon() {
    ...
    let okm = ev.eval(Value::expr(Value::Symbol("Hkdf".into()), vec![ikm, Value::Assoc([
    ...
    let ph = ev.eval(Value::expr(Value::Symbol("PasswordHash".into()), vec![Value::String("s3cret".into()), Value::Assoc([
    ...
    let ok = ev.eval(Value::expr(Value::Symbol("PasswordVerify".into()), vec![Value::String("s3cret".into()), Value::String(phs.clone())]));
    assert_eq!(ok, Value::Boolean(true));
    let bad = ev.eval(Value::expr(Value::Symbol("PasswordVerify".into()), vec![Value::String("wrong".into()), Value::String(phs)]));
    assert_eq!(bad, Value::Boolean(false));
}
...
fn crypto_jwt_hs256_and_eddsa() {
    ...
    let jwt = ev.eval(Value::expr(Value::Symbol("JwtSign".into()), vec![claims.clone(), Value::String("secret".into()), Value::Assoc([
        ("Alg".into(), Value::String("HS256".into())),
    ].into_iter().collect())]));
    let jwt_s = if let Value::String(s) = jwt { s } else { panic!("JwtSign output not string") };
    let ver = ev.eval(Value::expr(Value::Symbol("JwtVerify".into()), vec![Value::String(jwt_s.clone()), Value::String("secret".into()), Value::Assoc([
        ("ValidateTime".into(), Value::Boolean(false)),
    ].into_iter().collect())]));
    if let Value::Assoc(m) = ver { assert_eq!(m.get("valid"), Some(&Value::Boolean(true))); } else { panic!("JwtVerify result"); }
    // EdDSA roundtrip
    let kp = ev.eval(Value::expr(Value::Symbol("KeypairGenerate".into()), vec![]));
    let jwt2 = ev.eval(Value::expr(Value::Symbol("JwtSign".into()), vec![claims, kp.clone(), Value::Assoc([
        ("Alg".into(), Value::String("EdDSA".into())),
    ].into_iter().collect())]));
    let jwt2s = if let Value::String(s) = jwt2 { s } else { panic!("JwtSign output not string") };
    let ver2 = ev.eval(Value::expr(Value::Symbol("JwtVerify".into()), vec![Value::String(jwt2s), kp, Value::Assoc([
        ("ValidateTime".into(), Value::Boolean(false)),
    ].into_iter().collect())]));
    if let Value::Assoc(m) = ver2 { assert_eq!(m.get("valid"), Some(&Value::Boolean(true))); } else { panic!("JwtVerify result") }
}
