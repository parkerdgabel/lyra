...
fn audio_encode_info_decode_trim_gain() {
    ...
    } else { panic!("AudioInfo invalid"); }

    // Decode to raw
    let raw2 = ev.eval(Value::expr(Value::Symbol("AudioDecode".into()), vec![wav.clone(), Value::Assoc([( "format".into(), Value::String("raw".into()) )].into_iter().collect())]));
    if let Value::Assoc(m) = raw2 { assert!(matches!(m.get("encoding"), Some(Value::String(s)) if s=="s16le")); } else { panic!("raw2 invalid"); }

    // Trim first 20ms and take 50ms
    let trimmed = ev.eval(Value::expr(Value::Symbol("AudioTrim".into()), vec![
        wav.clone(),
        Value::Assoc([
            ("startSec".into(), Value::Real(0.02)),
            ("durationSec".into(), Value::Real(0.05)),
        ].into_iter().collect()),
    ]));
    let tinfo = ev.eval(Value::expr(Value::Symbol("AudioInfo".into()), vec![trimmed]));
    if let Value::Assoc(m) = tinfo {
        let dur = match m.get("durationSec") { Some(Value::Real(f)) => *f, _ => 0.0 };
        assert!(dur > 0.045 && dur < 0.055);
    } else { panic!("trim info invalid"); }

    // Gain -6 dB (linear ~0.5)
    let _gained = ev.eval(Value::expr(Value::Symbol("AudioGain".into()), vec![
        wav,
        Value::Assoc([("db".into(), Value::Real(-6.0))].into_iter().collect()),
    ]));
}
...
fn audio_concat_resample_fade_mix() {
    ...
    let concat = ev.eval(Value::expr(Value::Symbol("AudioConcat".into()), vec![
    ...
    if let Value::Assoc(m) = ci {
        ...
        assert!(matches!(m.get("channels"), Some(Value::Integer(n)) if *n==1));
        // ~0.1s total
        if let Some(Value::Real(d)) = m.get("durationSec") { assert!(*d > 0.09 && *d < 0.11); }
    } else { panic!("concat info"); }

    // Resample
    let rs = ev.eval(Value::expr(Value::Symbol("AudioResample".into()), vec![wav1.clone(), Value::Assoc([( "sampleRate".into(), Value::Integer(12000))].into_iter().collect())]));
    let rsi = ev.eval(Value::expr(Value::Symbol("AudioInfo".into()), vec![rs]));
    if let Value::Assoc(m) = rsi { assert!(matches!(m.get("sampleRate"), Some(Value::Integer(n)) if *n==12000)); } else { panic!("resample info"); }

    // Fade
    let fad = ev.eval(Value::expr(Value::Symbol("AudioFade".into()), vec![wav2.clone(), Value::Assoc([
    ...
    let mono = ev.eval(Value::expr(Value::Symbol("AudioChannelMix".into()), vec![wav2.clone(), Value::Assoc([( "to".into(), Value::String("mono".into()))].into_iter().collect())]));
    let mi = ev.eval(Value::expr(Value::Symbol("AudioInfo".into()), vec![mono]));
    if let Value::Assoc(m) = mi { assert!(matches!(m.get("channels"), Some(Value::Integer(n)) if *n==1)); } else { panic!("mix info"); }

    // Channel mix: mono -> stereo
    let stereo = ev.eval(Value::expr(Value::Symbol("AudioChannelMix".into()), vec![wav1, Value::Assoc([( "to".into(), Value::String("stereo".into()))].into_iter().collect())]));
    let si = ev.eval(Value::expr(Value::Symbol("AudioInfo".into()), vec![stereo]));
    if let Value::Assoc(m) = si { assert!(matches!(m.get("channels"), Some(Value::Integer(n)) if *n==2)); } else { panic!("mix2 info"); }
}
...
fn audio_save_and_convert() {
    ...
    let _ = ev.eval(Value::expr(Value::Symbol("AudioSave".into()), vec![
        wav_b64.clone(),
        Value::Assoc([
    ...
    let wav2 = ev.eval(Value::expr(Value::Symbol("AudioConvert".into()), vec![wav_b64, Value::String("wav".into())]));
    // Probe info
    let info = ev.eval(Value::expr(Value::Symbol("AudioInfo".into()), vec![wav2]));
    if let Value::Assoc(m) = info {
        assert!(matches!(m.get("sampleRate"), Some(Value::Integer(n)) if *n==8000));
    } else { panic!("info after convert"); }
}
