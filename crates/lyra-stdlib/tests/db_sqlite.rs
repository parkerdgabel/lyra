...
fn sqlite_distincton_multikey() {
    ...
    let _ = eval_str(&mut ev, &format!("Exec[{}, \"CREATE TABLE u(id INTEGER, grp INTEGER, v INTEGER)\"]", lyra_core::pretty::format_value(&conn)));
    let rows_u = Value::List(vec![
        Value::assoc(vec![("id", Value::Integer(1)), ("grp", Value::Integer(1)), ("v", Value::Integer(5))]),
        Value::assoc(vec![("id", Value::Integer(1)), ("grp", Value::Integer(1)), ("v", Value::Integer(8))]),
        Value::assoc(vec![("id", Value::Integer(1)), ("grp", Value::Integer(2)), ("v", Value::Integer(3))]),
        Value::assoc(vec![("id", Value::Integer(2)), ("grp", Value::Integer(2)), ("v", Value::Integer(7))]),
    ]);
    let _ = ev.eval(Value::expr(Value::Symbol("InsertRows".into()), vec![conn.clone(), Value::String("u".into()), rows_u]));
    ...
    assert!(do_keys_first_s.contains("\"id\" -> 1") && do_keys_first_s.contains("\"grp\" -> 1") && do_keys_first_s.contains("\"v\" -> 5"));
    // Last per (id,grp)
    ...
    assert!(do_keys_last_s.contains("\"id\" -> 1") && do_keys_last_s.contains("\"grp\" -> 1") && do_keys_last_s.contains("\"v\" -> 8"));
}
