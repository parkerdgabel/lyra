...
fn apply_and_compose() {
    ...
    match r { Value::List(v) => assert_eq!(v, vec![3,7].into_iter().map(Value::Integer).collect::<Vec<_>>()), _ => panic!() }
...
fn nest_and_foldlist() {
    ...
    match r { Value::List(v) => assert_eq!(v.len(), 5), _ => panic!() }
    let r = eval_str("FoldList[Plus, {1,2,3,4}]");
    match r { Value::List(v) => assert_eq!(v, vec![1,3,6,10].into_iter().map(Value::Integer).collect::<Vec<_>>()), _ => panic!() }
}
...
fn fixedpoint_and_through() {
    ...
    match r { Value::Integer(_) => {} _ => panic!() }
    let r = eval_str("Through[{(#&), (#+1)&}, 5]");
    match r { Value::List(v) => { assert_eq!(v.len(), 2); }, _ => panic!() }
}

#[test]
fn operator_forms() {
    let r = eval_str("Map[(#*#)&][{1,2,3}]");
    match r { Value::List(v) => assert_eq!(v, vec![1,4,9].into_iter().map(Value::Integer).collect::<Vec<_>>()), _ => panic!() }
