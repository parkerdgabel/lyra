...
fn docker_pull_run_list_inspect() {
    ...
    let ok = eval_str(
        &mut ev,
        &format!("PullImage[{}, \"alpine:3.19\"]", lyra_core::pretty::format_value(&rt)),
    );
    ...
    let info = eval_str(
        ...
        &format!(
            "InspectContainer[{}, {}]",
            lyra_core::pretty::format_value(&rt),
            lyra_core::pretty::format_value(&run)
        ),
    );
    let info_s = lyra_core::pretty::format_value(&info);
    assert!(
        info_s.contains("\"image\" -> \"alpine:3.19\"")
            || info_s.contains("\"image\" -> \"alpine\"")
    );

    // List containers dataset
    let ds =
        eval_str(&mut ev, &format!("ListContainers[{}]", lyra_core::pretty::format_value(&rt)));
