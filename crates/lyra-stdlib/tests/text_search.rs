...
fn text_search_grep_mode() {
    ...
    write_file(&f1, "alpha\nbeta\nLyra\n");
    let res = ev.eval(Value::expr(Value::Symbol("TextSearch".into()), vec![
...
fn text_search_fuzzy_mode() {
    ...
    stdlib::register_all(&mut ev);
    let res = ev.eval(Value::expr(Value::Symbol("TextSearch".into()), vec![
        Value::Assoc([("text".into(), Value::String("hello lyra\ntext search".into()))].into_iter().collect()),
...
fn text_search_index_mode() {
    ...
    let _ = ev.eval(Value::expr(Value::Symbol("IndexCreate".into()), vec![Value::String(idx.clone())]));
    let _ = ev.eval(Value::expr(Value::Symbol("IndexAdd".into()), vec![
        Value::String(idx.clone()),
        Value::List(vec![
            Value::Assoc([("id".into(), Value::String("1".into())), ("body".into(), Value::String("Lyra search".into()))].into_iter().collect()),
        ]),
    ]));
    let res = ev.eval(Value::expr(Value::Symbol("TextSearch".into()), vec![
...
fn text_search_count_task() {
    ...
    if let Value::Assoc(m) = res { assert!(matches!(m.get("engine"), Some(Value::String(s)) if s=="grep")); } else { panic!("invalid"); }
}
...
fn text_search_replace_task() {
    ...
    let _ = ev.eval(Value::expr(Value::Symbol("TextSearch".into()), vec![
        Value::Assoc([("path".into(), Value::String(f.clone()))].into_iter().collect()),
        Value::Assoc([("literal".into(), Value::String("def".into()))].into_iter().collect()),
        Value::Assoc([("task".into(), Value::String("replace".into())), ("replacement".into(), Value::String("xyz".into())), ("inPlace".into(), Value::Boolean(true))].into_iter().collect()),
    ]));
