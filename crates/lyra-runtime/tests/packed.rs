...
fn packed_map_elementwise() {
    ...
    let pa = ev.eval(Value::Expr { head: Box::new(sym("PackedArray")), args: vec![a] });
    // Map[(x)=>x+1, pa]
    let f = Value::PureFunction { params: Some(vec!["x".into()]), body: Box::new(Value::Expr { head: Box::new(sym("Plus")), args: vec![Value::Symbol("x".into()), Value::Integer(1)] }) };
...
fn packed_broadcast_minus_divide() {
    ...
    let mut ev = Evaluator::new();
    // A shape {2,1}
    let a = Value::List(vec![ Value::List(vec![Value::Integer(1)]), Value::List(vec![Value::Integer(2)]) ]);
    // B shape {1,2}
    let b = Value::List(vec![ Value::List(vec![Value::Integer(10), Value::Integer(20)]) ]);
    let pa = ev.eval(Value::Expr { head: Box::new(sym("PackedArray")), args: vec![a] });
    let pb = ev.eval(Value::Expr { head: Box::new(sym("PackedArray")), args: vec![b] });
    let diff = ev.eval(Value::Expr { head: Box::new(sym("Minus")), args: vec![pb.clone(), pa.clone()] });
    let diff_l = ev.eval(Value::Expr { head: Box::new(sym("PackedToList")), args: vec![diff] });
    assert_eq!(diff_l, Value::List(vec![
    ...
    assert_eq!(quot_l, Value::List(vec![
        Value::List(vec![Value::Real(10.0), Value::Real(20.0)]),
