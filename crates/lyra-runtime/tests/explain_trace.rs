...
fn has_condition_step(steps: &Vec<Value>, head_name: &str, expect: bool) -> bool {
    for st in steps {
        if let Value::Assoc(m) = st {
            if let (Some(Value::String(action)), Some(head), Some(Value::Assoc(data))) = (m.get("action").cloned(), m.get("head").cloned(), m.get("data").cloned()) {
                if action == "ConditionEvaluated" {
                    if let Value::Symbol(hs) = head { if hs == head_name {
                        if let Some(Value::Boolean(b)) = data.get("result") { if *b == expect { return true; } }
                    }}
                }
            }
        }
...
fn explain_records_pattern_test_results() {
    ...
    let lhs = Value::Expr { head: Box::new(sym("PatternTest")), args: vec![ Value::Expr { head: Box::new(sym("Blank")), args: vec![] }, sym("EvenQ") ] };
    ...
    let out = ev.eval(ex);
    let steps = match out { Value::Assoc(m) => match m.get("steps") { Some(Value::List(vs)) => vs.clone(), _ => vec![] }, _ => vec![] };
    assert!(has_condition_step(&steps, "PatternTest", true));

    // Explain[Replace[1, _?EvenQ -> 9]]
    let lhs2 = Value::Expr { head: Box::new(sym("PatternTest")), args: vec![ Value::Expr { head: Box::new(sym("Blank")), args: vec![] }, sym("EvenQ") ] };
    ...
    let steps2 = match out2 { Value::Assoc(m) => match m.get("steps") { Some(Value::List(vs)) => vs.clone(), _ => vec![] }, _ => vec![] };
