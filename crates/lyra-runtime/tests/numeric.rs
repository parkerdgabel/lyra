...
fn rational_arithmetic_plus_times() {
    ...
    // Times[2, 1/3] = 2/3
    let t = Value::Expr {
        head: Box::new(sym("Times")),
        args: vec![Value::Integer(2), Value::Rational { num: 1, den: 3 }],
    };
    let out2 = ev.eval(t);
...
fn complex_addition() {
    ...
    assert_eq!(
        out,
        Value::Complex { re: Box::new(Value::Integer(4)), im: Box::new(Value::Integer(1)) }
    );
}
...
fn rational_minus_divide_power() {
    ...
    // Minus[1/2, 1/3] = 1/6
    let expr = Value::Expr {
        head: Box::new(sym("Minus")),
        args: vec![Value::Rational { num: 1, den: 2 }, Value::Rational { num: 1, den: 3 }],
    };
    let out = ev.eval(expr);
    assert_eq!(out, Value::Rational { num: 1, den: 6 });
    // Divide[1/2, 1/3] = 3/2
    let expr2 = Value::Expr {
        head: Box::new(sym("Divide")),
        args: vec![Value::Rational { num: 1, den: 2 }, Value::Rational { num: 1, den: 3 }],
    };
    let out2 = ev.eval(expr2);
    ...
    let expr3 = Value::Expr {
        head: Box::new(sym("Power")),
        args: vec![Value::Rational { num: 1, den: 2 }, Value::Integer(2)],
    };
...
fn complex_minus_and_scalar_division() {
    ...
    assert_eq!(
        out,
        Value::Complex { re: Box::new(Value::Integer(-2)), im: Box::new(Value::Integer(3)) }
    );
    // (1+2i)/2 = 1/2 + 1i
    ...
    assert_eq!(
        out2,
        Value::Complex {
            re: Box::new(Value::Rational { num: 1, den: 2 }),
            im: Box::new(Value::Integer(1))
        }
    );
}
...
fn complex_division_and_abs() {
    ...
    assert_eq!(
        out,
        Value::Complex {
            re: Box::new(Value::Rational { num: 1, den: 10 }),
            im: Box::new(Value::Rational { num: 7, den: 10 })
        }
    );
    // Abs[3+4i] = 5
